...
```javascript
// Your query embedding (see "Generating Embeddings" section below)
...
### Using Atlas Automated Embeddings
...
This feature is available in Atlas M10+ clusters and supports various embedding providers including OpenAI and Hugging Face.

To use automated embeddings, configure them in your Atlas Search index definition through the Atlas UI or API. Mongoose will then work with the embeddings that Atlas generates automatically.

### Using Third-Party Embedding Models

For application-generated embeddings, you can use any embedding model. Popular options include:
...
Example using a fictional embedding function (you would use your chosen provider's SDK):

```javascript
// Example: Generate embeddings before saving (pseudocode)
async function saveMovieWithEmbedding(movieData) {
  // Generate embedding using your chosen provider
  // This is pseudocode - use the actual SDK for your embedding provider
...
**Note**: Ensure that:
- The embedding dimensions match your index configuration (`numDimensions`)
- You use the same embedding model for both indexing and querying
- Embeddings are normalized if using `dotProduct` similarity
