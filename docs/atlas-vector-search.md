...
const movieSchema = new mongoose.Schema({
  ...
  // Typically 1536 dimensions for OpenAI ada-002 or 1024 for Voyage AI models
  plot_embedding: {
    type: [Number],  // use Buffer if your embeddings are stored as binData
    default: undefined,  // prevent Mongoose from storing an empty array when no embedding is provided
    validate: {
      validator: v => v == null || v.length === 1536,  // must match numDimensions in the index definition
      message: 'plot_embedding must be nullish or exactly 1536 dimensions'
    }
  }
});
...
Once your vector search index is created, and your embeddings are generated (see [Generating Embeddings](#generating-embeddings)), you can perform vector similarity searches using the `$vectorSearch` aggregation stage.
...
const results = await Movie.aggregate([
  {
    $vectorSearch: {
      ...
      path: 'fullplot',
      query: 'shy person finds love',  // plain text, no embedding needed
      numCandidates: 100,
      limit: 10
    }
  },
...
```javascript
// Example: Generate embeddings before saving using Voyage AI
async function saveMovieWithEmbedding(movieData) {
  const response = await fetch('https://ai.mongodb.com/v1/embeddings', {
    ...
    body: JSON.stringify({ model: 'voyage-3', input: movieData.plot })
  });
  ...
  const movie = new Movie({
    ...
    plot: movieData.plot,
    plot_embedding: data.embeddings[0]  // Voyage AI returns { embeddings: [[...]] }
  });
