...
const aggregateTracingChannel = diagnosticsChannel.tracingChannel('mongoose:aggregate');
...
Aggregate.prototype.exec = async function exec() {
  if (!aggregateTracingChannel.hasSubscribers) {
    return execAggregateWithoutTrace.call(this);
  }

  const model = this._model;
  const collection = model?.collection?.name;
  const database = model?.db?.name || this._connection?.client?.db()?.databaseName;
  ...
  return aggregateTracingChannel.tracePromise(execAggregateWithoutTrace, context, this);
};

async function execAggregateWithoutTrace() {
