...
const aggregateChannel = dc?.tracingChannel('mongoose:aggregate');
...
Aggregate.prototype.exec = async function exec() {
  if (aggregateChannel?.hasSubscribers) {
    const ctx = {
      pipeline: this.pipeline(),
      collection: this._model?.collection?.name,
      database: this._model?.db?.name,
      options: this.options
    };

    aggregateChannel.start.publish(ctx);
    try {
      const result = await this.$__exec();
      aggregateChannel.asyncEnd.publish(ctx);
      return result;
    } catch (err) {
      ctx.error = err;
      aggregateChannel.error.publish(ctx);
      aggregateChannel.asyncEnd.publish(ctx);
      throw err;
    }
  }
...
Aggregate.prototype.$__exec = async function exec() {
