...
const { execPreWithFilter, execPostWithFilter } = require('./helpers/filterBuiltInMiddleware');
...
Aggregate.prototype.exec = async function exec() {
  ...
  try {
    await execPreWithFilter(model.hooks, 'aggregate', this, [], this.options);
  } catch (error) {
    return await execPostWithFilter(model.hooks, 'aggregate', this, [null], { error }, this.options);
  }
  ...
  await execPostWithFilter(model.hooks, 'aggregate', this, [result], {}, this.options);
  return result;
};
