...
const { shouldRunMiddleware, isBuiltInMiddleware } = require('./helpers/middleware');
...
Aggregate.prototype.exec = async function exec() {
  ...
  const preFilter = shouldRunMiddleware(this.options, 'pre') ? null : isBuiltInMiddleware;
  const postFilter = shouldRunMiddleware(this.options, 'post') ? null : isBuiltInMiddleware;
  ...
  await model.hooks.execPost('aggregate', this, [result], { filter: postFilter });
  return result;
};
