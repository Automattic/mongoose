...
const shouldSkipMiddleware = require('./helpers/query/shouldSkipMiddleware');
...
Aggregate.prototype.exec = async function exec() {
  ...
  if (shouldSkipMiddleware(this.options, 'pre') === false) {
    try {
      await model.hooks.execPre('aggregate', this);
    } catch (error) {
      if (shouldSkipMiddleware(this.options, 'post')) {
        throw error;
      }
      return await model.hooks.execPost('aggregate', this, [null], { error });
    }
  ...
  if (shouldSkipMiddleware(this.options, 'post') === false) {
    await model.hooks.execPost('aggregate', this, [result], { error: null });
  }
