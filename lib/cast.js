  ...
  while (i--) {
    ...
    if (path === '$or' || path === '$nor' || path === '$and') {
      ...
      try {
        ...
        throw new CastError('string', val, path, error);
      }
      obj[path] = val;
    } else {
      if (!schema) {
      ...
      if (!schematype) {
        ...
        // Do not let scalar SchemaTypes unbox arrays when sanitizeFilter
        // disables implicit $in.
        throw new CastError(schematype.instance, val, path);
      } else {
        obj[path] = schematype.castForQuery(
          null,
          val,
