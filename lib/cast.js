      ...
      if (!schematype) {
        ...
        if (!any$conditionals) {
          ...
          while (k--) {
            ...
            const recurse = iterate(nested) ?? '';
            if (Object.keys(recurse || {}).some(isOperator)) {
              val[$cond] = schematype.castForQuery($cond, recurse, context);
            } else
              if ($cond === '$not') {
...
// This function simply checks if the next value of the given object is also an object.
function checkNested(obj) {
  const vals = Object.values(obj);
  for (let i = 0; i < vals.length; i++) {
    if (typeof vals[i] === 'object' && !Array.isArray(vals[i]) && vals[i] != null) {
      // we check here if the value of the key is not an object, since then it would not meet the criteria for recursion.
      for (const key in vals[i]) {
        if (typeof vals[i][key] !== 'object') {
          return false;
        }
      }
      return true;
