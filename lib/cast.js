      ...
      if (!schematype) {
        ...
        if (!any$conditionals) {
          ...
          while (k--) {
            $cond = ks[k];
            nested = val[$cond];
            console.log('what is nested', nested);
            if (Object.keys(nested || {}).some(isOperator) && checkNested(nested)) {
...
// fix: what happens when obj is { key: { k: val } } gh3719
// need to check if an object that the val is also not an object
function checkNested(obj) {
  const vals = Object.values(obj);
  console.log('what is vals', vals);
