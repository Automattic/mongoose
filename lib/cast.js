 ...
 * @api private
 */
module.exports = function cast(schema, obj, options, context, _pathSeen) {
  ...
  if (_pathSeen && _pathSeen.has(obj)) {
    throw new Error('Mongoose does not support circular references in query filters or updates.');
  }
  ...
  try {
      ...
      if (path === '$or' || path === '$nor' || path === '$and') {
        ...
        for (let k = val.length - 1; k >= 0; k--) {
          ...
          if (discriminatorValue == null) {
            val[k] = cast(schema, val[k], options, context, _pathSeen);
          } else {
            const discriminatorSchema = getSchemaDiscriminatorByValue(context.schema, discriminatorValue);
            val[k] = cast(discriminatorSchema ? discriminatorSchema : schema, val[k], options, context, _pathSeen);
          }
        ...
        val = cast(schema, val, options, context, _pathSeen);
      } else if (path === '$text') {
        ...
        if (!schematype) {
          ...
          if (schematype) {
            ...
            if (schematype.schema) {
              ...
              const ret = cast(schematype.schema, remainingConds, options, context, _pathSeen)[pathLastHalf];
              if (ret === void 0) {
                delete obj[path];
              } else {
                obj[path] = ret;
          ...
          if (isObject(val)) {
            ...
            if (geo) {
              ...
              _cast(value, numbertype, context, _pathSeen);
              continue;
            }
          }
            ...
            while (k--) {
              ...
              if ($cond === '$elemMatch') {
                if (nested && schematype?.schema != null) {
                  cast(schematype.schema, nested, options, context, _pathSeen);
                } else if (nested && schematype?.$isMongooseArray) {
                  if (utils.isPOJO(nested) && nested.$not != null) {
                    cast(schema, nested, options, context, _pathSeen);
                  } else {
    ...
    _pathSeen.delete(obj);
  }
};

function _cast(val, numbertype, context, _pathSeen) {
  if (Array.isArray(val)) {
    val.forEach(function(item, i) {
      if (Array.isArray(item) || isObject(item)) {
        return _cast(item, numbertype, context, _pathSeen);
      }
      val[i] = numbertype.castForQuery(null, item, context);
    });
  } else {
    if (_pathSeen && _pathSeen.has(val)) {
      throw new Error('Mongoose does not support circular references in query filters or updates.');
    }
    _pathSeen || (_pathSeen = new Set());
    _pathSeen.add(val);
    try {
      ...
      while (nearLen--) {
        ...
        const item = val[nkey];
        if (Array.isArray(item) || isObject(item)) {
          _cast(item, numbertype, context, _pathSeen);
          val[nkey] = item;
        } else {
