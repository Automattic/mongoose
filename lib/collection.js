...
Collection.prototype.doQueue = function() {
  for (const method of this.queue) {
    try {
      const fn = typeof method[0] === 'function' ? method[0] : this[method[0]];

      if (typeof fn !== 'function') {
        console.warn(`Collection#doQueue: Method "${method[0]}" not found, skipping.`);
        continue;
      }

      const args = Array.isArray(method[1]) ? method[1] : [];
      fn.apply(this, args);
    } catch (err) {
      console.error(`Collection#doQueue: Error executing "${method[0]}":`, err);
    }
  }
