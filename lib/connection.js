...
Connection.prototype.openUri = function(uri, options, callback) {
  ...
  if (this._destroyCalled && this.readyState === STATES.disconnected) {
 ...
 *
 * @param {Boolean} force
 * @param {Boolean} destroyConnAfterClose
 * @param {Function} callback
 * @api private
 */
Connection.prototype._close = function(force, destroyConnAfterClose, callback) {
  const _this = this;
  const closeCalled = this._closeCalled;
  this._closeCalled = true;
  this._destroyCalled = destroyConnAfterClose;
  if (this.client != null) {
    this.client._closeCalled = true;
    this.client._destroyCalled = destroyConnAfterClose;
  }

  const conn = this;
  switch (this.readyState) {
    case STATES.disconnected:
      if (destroyConnAfterClose && this.base.connections.indexOf(conn) != -1) {
    ...
    case STATES.connected:
      ...
      this.doClose(force, function(err) {
        ...
        if (destroyConnAfterClose && _this.base.connections.indexOf(conn) != -1) {
    ...
    case STATES.connecting:
      this.once('open', function() {
        _this.close(force, destroyConnAfterClose, callback);
      });
      break;
