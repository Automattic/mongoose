...
/**
 * Calls `createCollection()` on each model in a series.
 *
 * @method createCollections
 * @param {Object} [options]
 * @param {boolean} [options.continueOnError=false] Continue after errors and include each error in the result.
 * @param {Boolean|Object} [options.middleware] Controls user `createCollection` middleware for each model.
 * @returns {Promise}
 * @api public
 */
...
Connection.prototype.createCollections = async function createCollections(options = {}) {
  ...
  for (const model of Object.values(this.models)) {
    try {
      const modelOptions = options.middleware === undefined ? {} : { middleware: options.middleware };
      result[model.modelName] = await model.createCollection(modelOptions);
    } catch (err) {
      if (!continueOnError) {
