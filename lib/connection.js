...
Connection.prototype.createCollection = function createCollection(collection, options, cb) {
  ...
  return utils.promiseOrCallback(cb, cb => {
    if ((this.readyState === STATES.connecting || this.readyState === STATES.disconnected) && this._shouldBufferCommands()) {
      this._queue.push({ fn: this.createCollection, ctx: this, args: args.concat([cb]) });
      return;
    }

    this.db.createCollection(collection, options).then(
      res => cb(null, res),
      err => { cb(err); }
    );
  });
...
Connection.prototype.dropCollection = function dropCollection(collection, cb) {
  ...
  return utils.promiseOrCallback(cb, cb => {
    if ((this.readyState === STATES.connecting || this.readyState === STATES.disconnected) && this._shouldBufferCommands()) {
      this._queue.push({ fn: this.dropDatabase, ctx: this, args: args.concat([cb]) });
      return;
    }

    this.db.dropCollection(collection).then(res => cb(null, res), err => cb(err));
  });
};
...
Connection.prototype.dropDatabase = function dropDatabase(cb) {
  ...
  return utils.promiseOrCallback(cb, cb => {
    ...
    this.db.dropDatabase().then(res => cb(null, res), err => cb(err));
  });
};
