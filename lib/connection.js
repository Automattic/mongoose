 ...
 * @returns
 */
Connection.prototype.syncIndexes = async function syncIndexes({ continueOnError } = { continueOnError: false }) {
  const result = {};
  const errorsMap = { };

  for (const model of Object.values(this.models)) {
    try {
      result[model.modelName] = await model.syncIndexes();
    } catch (err) {
      if (!continueOnError) {
  ...
  if (!continueOnError && Object.keys(errorsMap).length) {
    const message = Object.values(errorsMap).map(err => err.message).join(' ');
