...
Connection.prototype.open = function (host, database, port, options, callback) {
  ...
  // authdb
  if (options && options.authdb) {
    this.authdb = options.authdb;
...
Connection.prototype.onOpen = function (callback) {
  ...
  if (self.user && self.pass) {
    ...
    if (self.authdb) {
      connectionOptions.authdb = self.authdb;
    }
    console.log('AUTHENTICATE', connectionOptions);
    self.db.authenticate(self.user, self.pass, connectionOptions, open);
  }
  else
    open();
};
