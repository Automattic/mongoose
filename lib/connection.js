...
/**
 * A wrapper for startSession() followed by session.withTransaction(fn)
 */

Connection.prototype.withSession = async function withSession(opts, fn) {
  const session = await this.startSession(opts);
