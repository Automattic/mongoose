 ...
 * 
 * ####Example
 *    function createMultiTenatConnection(tenantId, modelName, schema) {
  *     mongoose.connect('mongodb://localhost:27017/test');
  *     const dbName = `db_prefix_${tenantId}`;
  *     const dbOptions = {
    *     //If true, create one connection object with multiple calls to useDb(). Default is false
    *     userCache:true 
    *     //If true, db will not listen to the events on original connection. Default is false
    *     noListener:true
  *     }
  *     const db = mongoose.connection.useDb(dbName, dbOptions);
