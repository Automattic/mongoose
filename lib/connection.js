 ...
 * @param {Boolean} [closeAndClean] optional
 * @param {Function} [callback] optional
 * @return {Promise}
 * @api public
 */

Connection.prototype.close = function(force, closeAndClean, callback) {
  if (typeof force === 'function') {
    callback = force;
    force = false;
  }

  if (typeof closeAndClean === 'function') {
    callback = closeAndClean;
    closeAndClean = false;
  }
  ...
  if (closeAndClean !== null) {
    closeAndClean = !!closeAndClean
  }

  return promiseOrCallback(callback, cb => {
    this._close(force, closeAndClean, cb);
  });
};
 ...
 *
 * @param {Boolean} force
 * @param {Boolean} closeAndClean
 * @param {Function} callback
 * @api private
 */
Connection.prototype._close = function(force, closeAndClean, callback) {
  const _this = this;
  ...
  switch (this.readyState) {
    case STATES.disconnected:
      if (closeAndClean && this.base.connections.indexOf(conn) != -1) {
        console.log("BASE CONNECTIONS COUNT BEFORE 1---------> ", this.base.connections.length)
        this.base.connections.splice(this.base.connections.indexOf(conn), 1);
        console.log("BASE CONNECTIONS COUNT AFTER 1---------> ", this.base.connections.length)
      }
      if (closeCalled) {
        callback();
      } else {
    ...
    case STATES.connected:
      ...
      this.doClose(force, function(err) {
        ...
        if (closeAndClean && _this.base.connections.indexOf(conn) != -1) {
          console.log("BASE CONNECTIONS COUNT BEFORE 2---------> ", _this.base.connections.length)
          _this.base.connections.splice(_this.base.connections.indexOf(conn), 1);
          console.log("BASE CONNECTIONS COUNT AFTER 2---------> ", _this.base.connections.length)
    ...
    case STATES.connecting:
      this.once('open', function() {
        ...
        _this.close(false, true, callback);
      });
      break;
    ...
    case STATES.disconnecting:
      this.once('close', function() {
        if (closeAndClean && _this.base.connections.indexOf(conn) != -1) {
          console.log("BASE CONNECTIONS COUNT BEFORE 3---------> ", _this.base.connections.length)
          _this.base.connections.splice(_this.base.connections.indexOf(conn), 1);
          console.log("BASE CONNECTIONS COUNT AFTER 3---------> ", _this.base.connections.length)
        }
        callback();
      });
      break;
