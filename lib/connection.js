...
Connection.prototype.model = function (name, schema, collection) {
  ...
  if (!this.base.modelSchemas[name]) {
    if (name in SchemaDefaults) {
      schema = SchemaDefaults[name];
    }

    if (schema) {
      this.base.modelSchemas[name] = schema;
      for (var i = 0, l = this.base.plugins.length; i < l; i++) {
        schema.plugin(this.base.plugins[i][0], this.base.plugins[i][1]);
      }
    } else {
  ...
  if (!schema) {
    schema = this.base.modelSchemas[name];
  }
