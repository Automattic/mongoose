...
function AggregationCursor(agg) {
  ...
  if (connection) {
    this.cursor = connection.db.aggregate(agg._pipeline, withoutMiddlewareOption(agg.options));
...
function _getRawCursor(model, aggregationCursor, agg) {
  try {
    const cursor = model.collection.aggregate(agg._pipeline, withoutMiddlewareOption(agg.options));
...
/**
 * `middleware` is a Mongoose-only option used to skip user hooks; strip it so
 * it isn't passed through to the MongoDB driver. Clones only when the option
 * is present because `delete` is relatively slow.
 *
 * @param {Object} [aggOptions]
 * @returns {Object} options safe to pass to the driver
 */

function withoutMiddlewareOption(aggOptions) {
  if (aggOptions == null) {
    return {};
  }
  if (!('middleware' in aggOptions)) {
    return aggOptions;
  }
