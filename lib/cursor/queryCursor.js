...
function QueryCursor(query) {
  ...
  this.model = model;
  this.options = {};
  // Another cursor can change the query's selection while pre hooks are pending.
  const middleware = query.options.middleware;
  this._middleware = middleware != null && typeof middleware === 'object' ?
    { pre: middleware.pre, post: middleware.post } : middleware;
  const middlewareOptions = { middleware: this._middleware };
  this._postMiddlewareFilter = buildMiddlewareFilter(middlewareOptions, 'post');
  ...
  const preFilter = buildMiddlewareFilter(middlewareOptions, 'pre');
...
function _nextDoc(ctx, doc, pop, callback) {
  ...
  const { model, _fields, _userProvidedFields } = ctx.query;
  const options = { ...ctx.query.options, middleware: ctx._middleware };
