...
// Nested paths (plain nested objects, as opposed to subdocuments) are
// backed by a lazy view onto the owning document: reading a property off
// of one, like `doc.a.b`, does not copy the value, it re-reads it from the
// document's internal state on every access. That means a value like
// `{ b: doc.a.b }` still holds a live reference to `doc`, not a snapshot.
//
// `Document#$set` clears out the destination path before rebuilding it
// key by key (see below), so if we wait until we get to `a.b` to convert
// that live reference into a plain value, `a` has already been nulled
// out and the conversion reads back `undefined` (gh-16530). To avoid
// that, snapshot any such live documents found anywhere in `val` up
// front, before anything gets cleared.
function resolveLiveNestedDocumentValues(val) {
  if (val == null || typeof val !== 'object') {
    return val;
  }
  if (val.$__ != null) {
    return val.toObject(internalToObjectOptions);
  }
  if (!utils.isPOJO(val)) {
    return val;
  }
  ...
  for (const key of Object.keys(val)) {
    const resolved = resolveLiveNestedDocumentValues(val[key]);
    if (resolved !== val[key]) {
      if (out === val) {
        out = Object.assign({}, val);
      }
      out[key] = resolved;
    }
  }
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (pathType === 'nested' && val) {
    if (typeof val === 'object' && val != null) {
      ...
      val = resolveLiveNestedDocumentValues(val);
