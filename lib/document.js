...
Document.prototype.toObject = function (options) {
	options || (options = {loadVirtuals:false});
	options.minimize = true;
	var cl = clone(this._doc, options);
	var me = this;
	if(options.loadVirtuals && me._schema && me._schema.virtuals)
	Object.keys(me._schema.virtuals).forEach(function(vp){
		if(me._schema.virtuals[vp].getters.length>0){
			var val = me.get(vp);
			if(!((val==undefined)||(val==''))) cl[vp] = val;
		}
	})
	return cl;
};
