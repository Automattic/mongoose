...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  ...
  try {
    ...
    if (validateAllPaths) {
      [paths, doValidateOptionsByPath] = _getAllPathsToValidate(this);
    } else {
    ...
    for (let i = 0; i < paths.length; ++i) {
      ...
      if (shouldValidateModifiedOnly) {
        doValidateOptions.validateModifiedOnly = true;
      }
...
/*!
 * Build the list for `validateAllPaths`. The document array path validates
 * every subdocument, but skips nullish elements. Add those elements separately
 * so their element-level validators still run.
 */

function _getAllPathsToValidate(doc) {
  const paths = Object.keys(doc.$__schema.paths).filter(path =>
    path.charAt(path.length - 1) !== '*' ||
    path.charAt(path.length - 2) !== '$' ||
    path.charAt(path.length - 3) !== '.'
  );
  const doValidateOptionsByPath = {};
  ...
  for (let i = 0, len = paths.length; i < len; ++i) {
    ...
    if (schemaType.$isSchemaMap) {
      ...
      // The document array validates each of its subdocuments, so elements only
      // need their own path listed if the element schematype has validators of
      // its own, like `required` or `[{ type: childSchema, validate: fn }]`.
      if (schemaType.embeddedSchemaType?.validators.length) {
        for (let j = 0; j < val.length; ++j) {
          const elementPath = path + '.' + j;
          paths.push(elementPath);
          doValidateOptionsByPath[elementPath] = { skipSubdocumentValidation: true, index: j };
        }
      }
  ...
  return [paths, doValidateOptionsByPath];
}
...
/*!
 * Builds the list of paths to run validation on, plus per-path options for
 * `doValidate()`.
 *
 * `activePaths` tells us which paths changed (`require`, `init`, `default`,
 * `modify`), but that is neither a complete nor a minimal list of paths to
 * validate, in four separate ways. Each phase below fixes exactly one:
 *
 *  1. Seed     Collect the changed paths, dropping ones that have nothing to
 *              run and ones a subdocument already validates for us.
 *  2. Resolve  Some entries are not schema paths at all: a modified nested
 *              object is stored as one entry (`nest`), but the paths that
 *              have validators are its children (`nest.name`).
 *  3. Cover    Replace changed descendants with their actual subdocument or
 *              document array, which owns validation below that path.
 *  4. Augment  Primitive arrays, required document array elements, and maps
 *              need concrete element or key paths listed individually.
 *
 * Validating a subdocument or document array validates everything underneath
 * it, so `arr` makes `arr.0.name` redundant. Phase 3 uses the actual
 * subdocuments and their modified paths to remove descendants seeded earlier.
 * It runs for nested documents as well, so every document selects containers
 * from its own schema without relying on `$parentSchemaDocArray`.
 *
 * If a document array element is modified, for example `arr.0.name`,
 * array-level validators on `arr` still run. This behavior dates back to gh-6818.
 *
 * `pathsToValidate` / `pathsToSkip` are applied before augment, so that asking
 * for `arr` still validates its elements. Duplicates are fine; both callers
 * skip paths they have already validated.
 */

function _getPathsToValidate(doc, pathsToValidate, pathsToSkip) {
  const schema = doc.$__schema;
  const schemaPaths = schema.paths;
  const singleNestedPaths = schema.singleNestedPaths;
  const activeStates = doc.$__.activePaths.states;
  const doValidateOptions = {};
  // Subdocument and document array paths already in `paths`, which are the
  // schema types that have a `schema` of their own. They run validation on
  // their children, so nothing below one of these should be added.
  const covered = {};
  let paths = new Set();

  // Phase 1: seed. Every path `activePaths` reports as changed, minus the ones
  // with nothing to run and the ones a subdocument validates for us.
  const requireStates = activeStates.require;
  if (requireStates != null) {
    let modifiedPaths = null;
    for (const path in requireStates) {
      const isOwnPath = Object.hasOwn(schemaPaths, path);
      const type = isOwnPath ? schemaPaths[path] : schema.path(path);
      ...
      // `.$*` paths represent map schemas, not actual document paths.
      if (path.charCodeAt(path.length - 1) === STAR_CHAR_CODE && path.endsWith('.$*')) {
        continue;
      }
      const cachedRequired = doc.$__.cachedRequired;
      if (cachedRequired != null && path in cachedRequired && !cachedRequired[path]) {
        continue;
      }
      if (!isOwnPath && type != null &&
          Object.hasOwn(singleNestedPaths, path)) {
        continue;
      }
      paths.add(path);
      if (type != null && type.schema != null) {
        covered[path] = true;
      }
    }
  }
  ...
  for (let i = 0; i < STATES_TO_VALIDATE.length; ++i) {
    ...
    for (const path in statePaths) {
      ...
      const isOwnPath = Object.hasOwn(schemaPaths, path);
      const type = isOwnPath ? schemaPaths[path] : schema.path(path);
      // Paths with no schema type (e.g. sub-paths under Mixed) must still be
      // added, as they trigger validation of the parent Mixed path.
      if (type != null &&
          (_hasNothingToValidate(type) ||
            (!isOwnPath &&
              Object.hasOwn(singleNestedPaths, path)))) {
        continue;
      }
  ...
  if (paths.size === 0 && doc.$__hasOnlyPrimitiveValues()) {
    return [[], doValidateOptions];
  }

  // Phase 2: resolve. A modified nested object is a single entry in
  // `activePaths`, but it is not a schema path and has no validators of its
  // own, so replace it with its children. Children are added as we go, and a
  // Set visits entries added during iteration, so this same loop also resolves
  ...
  for (const path of paths) {
    ...
    for (const key of Object.keys(value)) {
      ...
      paths.add(child);
      if (Object.hasOwn(schemaPaths, child) && schemaPaths[child].schema != null) {
        covered[child] = true;
      }
    }
  }

  // Phase 3: cover. Validate modified single nested documents and document
  // arrays as units, using actual subdocuments to remove redundant descendants.
  const modifiedPaths = doc.modifiedPaths();

  const pathsWithSchemaTypes = Object.entries(schemaPaths);
  // Map of document arrays may have subdocuments that are not in `schemaPaths` -
  // If `team` is a map of document arrays, `team.engineering` is not a schema path
  // but may be a modified document array.
  for (const path of Object.keys(schemaPaths)) {
    const schemaType = schemaPaths[path];
    if (!schemaType.$isSchemaMap || !schemaType.$__schemaType.$isMongooseDocumentArray) {
      continue;
    }
    ...
    for (const key of val.keys()) {
      const mapValuePath = path + '.' + key;
      if (modifiedPaths.includes(mapValuePath)) {
        pathsWithSchemaTypes.push([mapValuePath, schemaType.$__schemaType]);
      }
    }
  }

  for (const [path, schemaType] of pathsWithSchemaTypes) {
    let subdocs;
    if (schemaType.$isSingleNested) {
      ...
      subdocs = doc.$get(path);
      if (!subdocs?.length) {
        continue;
      }
      // If a document array element is modified, for example `arr.0.name`,
      // validate the whole array so array-level validators on `arr` still run.
      // This behavior dates back to gh-6818.
      if (modifiedPaths.includes(path) &&
          (schemaType.validators.length > 0 || covered[path] === true)) {
        if (covered[path] !== true && !Object.hasOwn(activeStates.modify, path)) {
          doValidateOptions[path] = { skipSchemaValidators: true };
        }
        paths.add(path);
        covered[path] = true;
      }
    ...
    // Validating the whole array validates every element, so element paths
    // are redundant. They can be seeded before the array is, so drop them
    // here rather than relying on the array being seen first.
    const arrayCoversElements = schemaType.$isMongooseDocumentArray && covered[path] === true;

    for (const subdoc of subdocs) {
      if (subdoc == null || !subdoc.$basePath) {
        continue;
      }
      const fullPathToSubdoc = schemaType.$isMongooseDocumentArray ?
        path + '.' + subdoc.__index :
        subdoc.$__pathRelativeToParent();
      ...
      if (arrayCoversElements) {
        paths.delete(fullPathToSubdoc);
        for (const modifiedPath of subdoc.modifiedPaths({ includeChildren: true })) {
          paths.delete(fullPathToSubdoc + '.' + modifiedPath);
        }
        if (path !== schemaType.path && Object.hasOwn(activeStates.modify, fullPathToSubdoc)) {
          doValidateOptions[fullPathToSubdoc] = { skipSchemaValidators: false };
        }
      }
      ...
      // The element's index and scope are still recorded even when the array
      // covers it: `_addArrayPathsToValidate` may list the element below.
      if (!arrayCoversElements && covered[path] !== true) {
        paths.add(fullPathToSubdoc);
      }
      covered[fullPathToSubdoc] = true;
      doc.$__.pathsToScopes ??= {};
      doc.$__.pathsToScopes[fullPathToSubdoc] = subdoc.$isDocumentArrayElement ?
        subdoc.__parentArray :
        subdoc.$parent();
      doValidateOptions[fullPathToSubdoc] ??= subdoc.$isDocumentArrayElement ?
        {} :
  ...
  // Phase 4: augment. Arrays and maps only run their own validators, so their
  // elements and keys need to be listed individually.
  // gh-661: if a whole array is modified, make sure to run validation on all
  // the children as well
  _addArrayPathsToValidate(doc, paths, doValidateOptions);

  for (const path of paths) {
    const _pathType = schema.path(path);
  ...
  // The callers index into the result, so hand them an array.
  return [Array.from(paths), doValidateOptions];
}

/*!
 * Optimization: skip paths that cannot produce a validation error, either a
 * primitive with no validators or an array of primitives with no validators on
 * the array or its elements.
 */

function _hasNothingToValidate(type) {
  if (!type.schema &&
      !type.embeddedSchemaType &&
      type.validators.length === 0 &&
      // gh-15957: maps can contain subdocuments that need validation even if
      // the map itself has no validators
      !type.$isSchemaMap &&
      !type.$isSchemaUnion) {
    return true;
  }
  // gh-11380
  return !!type.$isMongooseArray &&
    !type.$isMongooseDocumentArray && // Skip document arrays...
    !type.embeddedSchemaType.$isMongooseArray && // and arrays of arrays
    type.validators.length === 0 &&
    type.embeddedSchemaType.validators.length === 0;
}

function _addArrayPathsToValidate(doc, paths, doValidateOptions) {
  for (const path of paths) {
    ...
    if (!_pathType.$isMongooseArray ||
        // To avoid potential performance issues, skip doc arrays whose elements
        // have no validators of their own. `getPositionalPathType()` may be
        // slow, so avoid it unless we have a case of #6364
        (!Array.isArray(_pathType) &&
          _pathType.$isMongooseDocumentArray &&
          !_pathType.embeddedSchemaType?.validators.length)) {
      continue;
    }
    ...
    if (_pathType.$isMongooseDocumentArray) {
      // We only get here because the document array itself is being validated,
      // which validates every subdocument. Leave the subdocuments to it and run
      // just the element-level validators here, so nothing validates twice.
      const numElements = val?.length ?? 0;
      for (let j = 0; j < numElements; ++j) {
        const elementPath = path + '.' + j;
        paths.add(elementPath);
        doValidateOptions[elementPath] = {
          ...doValidateOptions[path],
          ...doValidateOptions[elementPath],
          skipSubdocumentValidation: true,
          index: j
        };
      }
      continue;
...
function _handlePathsToValidate(paths, pathsToValidate) {
  ...
  // A Set: several paths can map onto the same `parentPaths` entry.
  const ret = new Set();
  for (const path of paths) {
    let isUnderPathToValidate = false;
    let index = path.length;
    // Check if parent of this path is in `pathsToValidate`, e.g. if
    // `profile` is in `pathsToValidate` then we should validate `profile.age`.
    while ((index = path.lastIndexOf('.', index - 1)) !== -1) {
      if (_pathsToValidate.has(path.slice(0, index))) {
        isUnderPathToValidate = true;
        break;
      }
    }

    if (_pathsToValidate.has(path) || isUnderPathToValidate) {
      ret.add(path);
...
Document.prototype.$__validateSync = function(pathsToValidate, options) {
  ...
  for (let i = 0, len = paths.length; i < len; ++i) {
    ...
    const pathOptions = doValidateOptionsByPath[path];
    const err = p.doValidateSync(val, _this, {
      ...(pathOptions != null && typeof pathOptions === 'object' ? pathOptions : null),
      skipSchemaValidators: typeof pathOptions === 'object' ?
        pathOptions.skipSchemaValidators : pathOptions,
      path: path,
