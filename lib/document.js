...
Document.prototype.$__presaveValidate = function presaveValidate(callback) {
  var self = this;
  ...
  if (err) {
    ...
    self.validate(function (err, val) {
      if (err) return promise.reject(err);
      return promise.fulfill();
    });
  }

  var promise2 = promise.then(function Validate() {
    var promise = new Promise;
    if (!docs.length) {
      promise.fulfill();
    }

    var errorSemaphore = false;
    var oks = 0;
    docs.forEach(function (doc) {
      if (errorSemaphore) return;
      //this is not save just a stub for recursion and hooks
      doc.save(function (err, saved) {
        ...
        if (err) {
          ...
          oks++;
          if (oks == docs.length) promise.fulfill(saved);
        }
      });
    });
    return promise;
  ...
  if (callback) promise2.onResolve(callback.bind(self));
  return promise2;
};
...
Document.prototype.$__registerHooksFromSchema = function () {
  ...
  this.hook('save', this.save, function (err) {
  ...
  q.forEach(function (pair) {
    ...
    this[pointCut].apply(this, arguments);
  }, this);
  return this;
};
