 ...
 *
 * @param {Object} [options]
 * @param {string[]} [options.pathsToSave] paths to include in delta generation
 * @api private
 * @method $__delta
 * @memberOf Document
 * @instance
...
Document.prototype.$__delta = function $__delta(options) {
  ...
  const dirty = pathsToSave == null ? allDirty : allDirty.filter(data => isPathIncluded(data.path));
  const optimisticConcurrency = this.$__schema.options.optimisticConcurrency;
  if (optimisticConcurrency) {
    if (Array.isArray(optimisticConcurrency)) {
      const optCon = new Set(optimisticConcurrency);
      const modPaths = this.modifiedPaths();
      const relevantModPaths = pathsToSave == null ? modPaths :
        modPaths.filter(path => isPathIncluded(path));
      if (relevantModPaths.some(path => optCon.has(path))) {
      ...
      const relevantModPaths = pathsToSave == null ? modPaths :
        modPaths.filter(path => isPathIncluded(path));
      if (relevantModPaths.some(path => !excluded.has(path))) {
        this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
      }
    } else {
      this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
