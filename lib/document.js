...
function Document (obj, fields) {
  ...
  this._strictMode = this.options && this.options.strict;
...
/**
 * Determine if this selection has excluded fields.
 * @param {Object} selected
 */

function excluded (selection) {
  var excluded = false;

  if (selection && 'Object' === selection.constructor.name) {
    var keys = Object.keys(selection)
      , ki = keys.length;

    while (ki--) {
      if ('_id' !== keys[ki]) {
        excluded = 0 === selection[keys[ki]];
        break;
      }
    }
  }
