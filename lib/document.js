...
const builtInMiddleware = require('./schema/symbols').builtInMiddleware;
...
function Document(obj, fields, options) {
  ...
  if (this._doc == null) {
    ...
    // Re: gh-6155
    if (defaults) {
      hasPostSetterDefaults = applyDefaults(this, fields, exclude, hasIncludedChildren, true, null, options.middleware == null ?
        applyDefaultsBeforeSettersOptions :
        { ...applyDefaultsBeforeSettersOptions, middleware: options.middleware });
    }
  }
  if (obj) {
    // Skip set hooks
  ...
  if (options.willInit && defaults) {
    ...
    applyDefaults(this, fields, exclude, hasIncludedChildren, false, options.skipDefaults, { middleware: options.middleware });
  }
...
Document.prototype.$__init = function(doc, opts) {
  ...
  applyDefaults(this, this.$__.selected, this.$__.exclude, hasIncludedChildren, false, this.$__.skipDefaults, { middleware: opts.middleware });
  return this;
};
    ...
    if (!schemaType && utils.isPOJO(value)) {
      ...
      if (value === null) {
        ...
        if (schemaType && !wasPopulated && !opts.hydratedPopulatedDocs) {
          ...
          doc[i] = schemaType.cast(value, self, true, undefined, { hydratedPopulatedDocs: true, middleware: opts.middleware });
...
Document.prototype.updateOne = function updateOne(update, options) {
  ...
  queryPreUpdateOne[builtInMiddleware] = true;
  queryPostUpdateOne[builtInMiddleware] = true;
  query.pre(queryPreUpdateOne);
  query.post(queryPostUpdateOne);

  return query;

  async function queryPreUpdateOne() {
    const res = await self._execDocumentPreHooks('updateOne', query.options, [self, update, options]);
    ...
    // Keep pipeline handling without replaying the original call's options.
    query.updateOne({ _id: self._doc._id }, update, { updatePipeline: query._mongooseOptions.updatePipeline });
  ...
  function queryPostUpdateOne() {
    return self._execDocumentPostHooks('updateOne', query.options);
  }
};
...
Document.prototype._execDocumentPostHooks = function _execDocumentPostHooks(opName, options, error) {
  ...
  const middleware = bindPostMiddlewareOptions(this.$__middleware, opName, options);
  return middleware.execPost(opName, this, [this], { error, filter });
};
