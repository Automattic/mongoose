...
let EventEmitter = require('events').EventEmitter, MongooseError = require('./error'), MixedSchema = require('./schema/mixed'), Schema = require('./schema'), ObjectExpectedError = require('./error/objectExpected'), StrictModeError = require('./error/strict'), ValidatorError = require('./schematype').ValidatorError, utils = require('./utils'), clone = utils.clone, isMongooseObject = utils.isMongooseObject, inspect = require('util').inspect, ValidationError = MongooseError.ValidationError, InternalCache = require('./internal'), deepEqual = utils.deepEqual, hooks = require('hooks-fixed'), PromiseProvider = require('./promise_provider'), DocumentArray, MongooseArray, Embedded;
...
function Document(obj, fields, skipId) {
  ...
  if (!schema.options.strict && obj) {
    const _this = this, keys = Object.keys(this._doc);
...
Document.prototype.$__buildDoc = function(obj, fields, skipId) {
  ...
  let paths = Object.keys(this.schema.paths), plen = paths.length, ii = 0;
  ...
  for (; ii < plen; ++ii) {
    ...
    const type = this.schema.paths[p];
    const path = p.split('.');
...
function init(self, obj, doc, prefix) {
  ...
  let keys = Object.keys(obj), len = keys.length, schema, path, i;
...
Document.prototype.$__storeShard = function() {
  ...
  let orig = this.$__.shardval = {}, paths = Object.keys(key), len = paths.length, val;
...
Document.prototype.set = function(path, val, type, options) {
  ...
  let merge = options && options.merge, adhoc = type && type !== true, constructing = type === true, adhocs;
...
Document.prototype.$__set = function(pathToMark, path, constructing, parts, schema, val, priorVal) {
  ...
  let obj = this._doc, i = 0, l = parts.length;

  for (; i < l; i++) {
    const next = i + 1, last = next === l;
...
Document.prototype.$__path = function(path) {
  const adhocs = this.$__.adhocPaths, adhocType = adhocs && adhocs[path];
...
Document.prototype.isSelected = function isSelected(path) {
  if (this.$__.selected) {
    ...
    let paths = Object.keys(this.$__.selected), i = paths.length, inclusive = false, cur;
...
Document.prototype.$__validate = function(callback) {
  ...
  let paths = Object.keys(this.$__.activePaths.states.require).filter(function(path) {
...
function compile(tree, proto, prefix, options) {
  let keys = Object.keys(tree), i = keys.length, limb, key;
    ...
    Object.defineProperty(prototype, prop, {
      ...
      get: function() {
        ...
        if (!this.$__.getters[path]) {
          const nested = Object.create(Object.getPrototypeOf(this), getOwnPropertyDescriptors(this));
...
Document.prototype.$__getAllSubdocs = function() {
  ...
  const subDocs = Object.keys(this._doc).reduce(docReducer.bind(this), []);
...
Document.prototype.$__registerHooksFromSchema = function() {
  ...
  const _this = this;
  const q = _this.schema && _this.schema.callQueue;
  if (!q.length) {
    return _this;
  }

  // we are only interested in 'pre' hooks, and group by point-cut
  const toWrap = q.reduce(function(seed, pair) {
    if (pair[0] !== 'pre' && pair[0] !== 'post' && pair[0] !== 'on') {
      _this[pair[0]].apply(_this, pair[1]);
      return seed;
    }
    const args = [].slice.call(pair[1]);
    const pointCut = pair[0] === 'on' ? 'post' : args[0];
    if (!(pointCut in seed)) {
  ...
  Object.keys(toWrap).forEach(function(pointCut) {
    ...
    const newName = ('$__original_' + pointCut);
    if (!_this[pointCut]) {
      return;
    }
...
Document.prototype.$toObject = function(options, json) {
  const defaultOptions = {transform: true, json: json};
...
function minimize(obj) {
  let keys = Object.keys(obj), i = keys.length, hasKeys, key, val;
...
function applyGetters(self, json, type, options) {
  let schema = self.schema, paths = Object.keys(schema[type]), i = paths.length, path;

  while (i--) {
    path = paths[i];

    let parts = path.split('.'), plen = parts.length, last = plen - 1, branch = json, part;
...
Document.prototype.depopulate = function(path) {
  const populatedIds = this.populated(path);
  if (!populatedIds) {
    return;
  }
  delete this.$__.populated[path];
