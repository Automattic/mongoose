...
Document.prototype.$__validate = function(pathsToValidate, options, callback) {
  ...
  if (Array.isArray(pathsToValidate)) {
    ...
    // make a new variable that is paths intersection !options.pathsToSkip
    const valPaths = new Set(paths);
    for (const path of pathsToSkip) {
      if (valPaths.has(path)) {
        valPaths.delete(path);
      }
    }
    // then send that to be validated
    paths = _handlePathsToSkip(paths, valPaths);
  }
  if (paths.length === 0) {
...
function _handlePathsToSkip(paths, valPaths) {
  const _valPaths = new Set(valPaths);
  const ret = [];
  for (const path of paths) {
    if (_valPaths.has(path)) {
      ret.push(path);
    }
  }
  return ret;
