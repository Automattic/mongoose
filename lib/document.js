 ...
 *  3. Cover    Some paths that need validating are missing: a subdocument
 *              that changed only through one of its children is never marked
 *              modified itself.
 *  4. Augment  Validating an array or map runs only that path's own
 *              validators, so elements and keys must be listed individually.
 *
 * Validating a subdocument or document array validates everything underneath
 * it, so `arr` makes `arr.0.name` redundant. Rather than add redundant paths
 * and filter them out afterwards, phases 1 and 3 never add them: `covered`
 * records the subdocument and document array paths already in the list, and
 * the schema tells us directly which paths live underneath one.
 *
...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  if (requireStates != null) {
    ...
    for (const path in requireStates) {
      ...
      if (!isOwnPath && type != null &&
          _isValidatedBySubdoc(type, schemaPaths, singleNestedPaths, path, paths, covered)) {
        continue;
      }
      paths.add(path);
  ...
  // Phase 3: cover. A subdocument that changed only through one of its children
  // is not marked modified itself, so nothing added it above. Validate the whole
  // subdocument, using the parent array or document as the validator scope.
  // When validating a subdocument this all runs on the top-level document
  // anyway, so there is nothing to add here.
  if (!isNestedValidate) {
    ...
    for (const path of Object.keys(schemaPaths)) {
      ...
      if (schemaType.$isSingleNested) {
        ...
        // gh-6818: if a document array with array-level validators has any
        // modified element, validate the array itself so those validators run.
        if (schemaType.validators.length > 0 && modifiedPaths.includes(path)) {
          paths.add(path);
          covered[path] = true;
        }
      } else {
...
/*!
 * Whether `path` lives underneath a subdocument or document array on this
 * document, which validates it for us. Document arrays are the exception: the
 * array's own validators still need to run, so list the array instead
 * re: gh-6818.
 *
 * Only call this for paths that are not paths of `doc`'s own schema.
 * `$parentSchemaDocArray` says the type belongs to a schema that is used as a
 * document array somewhere, not that it sits under one here - when validating
 * the subdocument itself, its own paths carry that flag too.
 */

function _isValidatedBySubdoc(type, schemaPaths, singleNestedPaths, path, paths, covered) {
  // A document array element (`arr.0`): the array validates every element.
  const parentSchemaType = type.$parentSchemaType;
  if (parentSchemaType != null && parentSchemaType.$isMongooseDocumentArray) {
    return covered[parentSchemaType.path] === true;
  }
  ...
  if (parentDocArray != null) {
    ...
    if (typeof arrayPath === 'string' && schemaPaths[arrayPath] === parentDocArray) {
      if (covered[arrayPath] !== true) {
        paths.add(arrayPath);
        covered[arrayPath] = true;
      }
      return true;
    }
    // `type` can come from a document array nested under a map subdocument.
    // In that case `arrayPath` is relative to the map value's schema, so the
    // top-level document array is not available to validate this path for us.
    return false;
  }
  return Object.hasOwn(singleNestedPaths, path);
}
