...
Document.prototype.$__delta = function $__delta(pathsToSave, pathsToSaveSet) {
  ...
  if (optimisticConcurrency) {
    if (this.$__schema.s.optimisticConcurrencyInclude != null) {
      const optimisticConcurrencyInclude = this.$__schema.s.optimisticConcurrencyInclude;
      const modPaths = this.directModifiedPaths();
      const hasRelevantModPaths = pathsToSave == null ?
        modPaths.find(path => _pathOrAncestorInSet(path, optimisticConcurrencyInclude)) :
        modPaths.find(path => _pathOrAncestorInSet(path, optimisticConcurrencyInclude) && isInPathsToSave(path, pathsToSaveSet, pathsToSave));
      if (hasRelevantModPaths) {
        this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
      }
    } else if (this.$__schema.s.optimisticConcurrencyExclude != null) {
      const excluded = this.$__schema.s.optimisticConcurrencyExclude;
      const modPaths = this.directModifiedPaths();
      const hasRelevantModPaths = pathsToSave == null ?
        modPaths.find(path => !_pathOrAncestorInSet(path, excluded)) :
        modPaths.find(path => !_pathOrAncestorInSet(path, excluded) && isInPathsToSave(path, pathsToSaveSet, pathsToSave));
      if (hasRelevantModPaths) {
        this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
      }
    } else {
 ...
 * For example, _pathOrAncestorInSet('profile.firstName', Set(['profile'])) === true
 */
