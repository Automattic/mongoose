...
function Document(obj, fields, options) {
  ...
  if (obj) {
    ...
    if (this.$__original_set) {
      ...
      this.$set(obj, undefined, options, true);
    }
 ...
 * @param {boolean} [constructing] whether the document is being initialized with constructor data
...
Document.prototype.$set = function $set(path, val, options, constructing) {
  if (typeof options === 'boolean' || typeof options === 'function' || typeof options === 'string') {
    throw new MongooseError('The 3rd argument to set() is now `options` in Mongoose 10 - the `type` argument is no longer supported. `options` cannot be a boolean, function, or string.');
  }
  ...
  if (path == null) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      if (utils.isNonBuiltinObject(valForKey) && pathtype === 'nested') {
        this.$set(pathName, valForKey, options, constructing);
        ...
        if (pathtype === 'real' || pathtype === 'virtual') {
          ...
          this.$set(pathName,
            valForKey.toObject({ transform: false }), options, constructing);
  ...
  if (pathType === 'nested' && val) {
    if (typeof val === 'object' && val != null) {
      ...
      if (!merge) {
        ...
        return this.$set(val, path, options, constructing);
      }
      ...
      for (const key of keys) {
        this.$set(path + '.' + key, val[key], { ...options, _skipMarkModified: true }, constructing);
      }
      if (priorVal != null &&
  ...
  if (val != null && merge && schema.$isSingleNested) {
    ...
    for (const key of keys) {
      this.$set(path + '.' + key, val[key], options, constructing);
    }
...
Document.prototype.get = function(path, options, legacyOptions) {
  if (typeof options === 'boolean' || typeof options === 'function' || typeof options === 'string') {
    throw new MongooseError('The 2nd argument to get() is now `options` in Mongoose 10 - the `type` argument is no longer supported. `options` cannot be a boolean, function, or string.');
  }
  ...
  if (noDottedPath) {
    const obj = Object.hasOwn(this._doc, path) ? this._doc[path] : undefined;
