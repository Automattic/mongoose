...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  // Nested paths are stored in `activePaths` as a single path when the whole
  // nested object is modified, so expand them into their subpaths. Do this
  // before collecting subdocuments so that document arrays underneath nested
  // paths are deduped against their elements below, re: gh-5411.
  for (const pathToCheck of paths) {
    if (doc.$__schema.nested[pathToCheck]) {
      ...
      for (const key of Object.keys(_v)) {
        // Note that this may add a nested path to `paths`, in which case this
        // loop will also expand that nested path's subpaths.
        paths.add(pathToCheck + '.' + key);
      }
    }
  }
