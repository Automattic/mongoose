...
Document.prototype.validate = async function validate(pathsToValidate, options) {
  ...
  try {
    ...
    if (validateAllPaths) {
      paths = Object.keys(this.$__schema.paths);
      // gh-661: if a whole array is modified, make sure to run validation on all
      // the children as well
      for (let i = 0, len = paths.length; i < len; ++i) {
        const path = paths[i];
        const schemaType = this.$__schema.path(path);
        // Skip document arrays: validating the array validates every element,
        // so listing the elements as well validates each subdocument twice.
...
Document.prototype.$__validateSync = function(pathsToValidate, options) {
  ...
  for (let i = 0, len = paths.length; i < len; ++i) {
    ...
    const pathOptions = skipSchemaValidators[path];
