...
const pathMatchesSet = require('./helpers/path/pathMatchesSet');
...
Document.prototype.$__delta = function $__delta(pathsToSave, pathsToSaveSet) {
  ...
  if (optimisticConcurrency) {
    if (Array.isArray(optimisticConcurrency)) {
      ...
      const directPaths = this.directModifiedPaths();
      const hasRelevantModPaths = pathsToSave == null ?
        directPaths.some(path => pathMatchesSet(path, optCon, true)) :
        directPaths.some(path => pathMatchesSet(path, optCon, true) && isInPathsToSave(path, pathsToSaveSet, pathsToSave));
      ...
      const directPaths = this.directModifiedPaths();
      const hasRelevantModPaths = pathsToSave == null ?
        directPaths.some(path => !pathMatchesSet(path, excluded)) :
        directPaths.some(path => !pathMatchesSet(path, excluded) && isInPathsToSave(path, pathsToSaveSet, pathsToSave));
      if (hasRelevantModPaths) {
        this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
      }
    } else {
