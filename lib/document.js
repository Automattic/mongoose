...
function _evaluateRequiredFunctions(doc) {
  Object.keys(doc.$__.activePaths.states.require).forEach(path => {
    ...
    if (typeof p.originalRequiredValue === 'function') {
      p.$cachedRequired = p.originalRequiredValue.call(doc);
    }
  });
}
...
function _getPathsToValidate(doc) {
  ...
  var paths = Object.keys(doc.$__.activePaths.states.require).filter(function(path) {
    ...
    if ('$cachedRequired' in p) {
      return p.$cachedRequired;
    }
    return true;
  });
...
Document.prototype.$toObject = function(options, json) {
  ...
  const schemaOptions = get(this, 'schema.options.' + path, {});
  // merge base default options with Schema's set default options if available.
  // `clone` is necessary here because `utils.options` directly modifies the second input.
  defaultOptions = Object.assign(defaultOptions, clone(baseOptions), clone(schemaOptions.toJSON));
