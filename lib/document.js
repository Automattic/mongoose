 ...
 * @param {Boolean|Object} [options.middleware=true] If `false`, skip all document-level middleware. If an object, can set `pre` or `post` to `false` to skip pre or post document-level middleware respectively.
 * @return {Query}
 * @api public
 * @memberOf Document
 * @instance
...
Document.prototype.updateOne = function updateOne(doc, options) {
  ...
  query.pre(async function queryPreUpdateOne() {
    const res = await self._execDocumentPreHooks('updateOne', options, self);
...
Document.prototype._execDocumentPreHooks = async function _execDocumentPreHooks(opName, options, ...args) {
  if (shouldSkipMiddleware(options, 'pre')) {
    return [options, ...args];
  }
  return this.$__middleware.execPre(opName, this, [options, ...args]);
};
...
Document.prototype._execDocumentPostHooks = async function _execDocumentPostHooks(opName, options, error) {
  if (shouldSkipMiddleware(options, 'post')) {
    if (error) {
      throw error;
    }
    return [this];
