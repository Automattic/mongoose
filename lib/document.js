...
Document.prototype._presave = function _presave(callback) {
  var self = this;

  DocumentArray || (DocumentArray = require('./types/documentarray'));

  var promise1 = new Promise;
  var promise2 = promise1.then(function ValidateArrays() {
    ...
    var arrays = self.$__.activePaths
      .map('init', 'modify', function (i) {
        return self.getValue(i);
      })
      .filter(function (val) {
    ...
    if (!arrays.length) promise.fulfill();
  ...
  promise2.onReject(function (err) {
    ...
    if (self.constructor.listeners('error').length) {
  ...
  var promise3 = promise2.then(function Validate() {
  ...
  promise3.onResolve(callback.bind(self));
  promise3.end();
  promise1.fulfill();
