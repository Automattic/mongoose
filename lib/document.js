...
Document.prototype.$__delta = function $__delta(pathsToSave, pathsToSaveSet) {
  ...
  if (optimisticConcurrency) {
    if (Array.isArray(optimisticConcurrency)) {
      ...
      const hasRelevantModPaths = modPaths.find(path => {
        if (pathsToSave != null && !isInPathsToSave(path, pathsToSaveSet, pathsToSave)) {
          return false;
        }
        if (modPaths.some(p => p !== path && p.startsWith(path + '.'))) {
          return false;
        }
        if (parentPaths(path).some(p => optCon.has(p))) {
          return true;
        }
        for (const p of optCon) {
          if (p.startsWith(path + '.')) {
            return true;
          }
        }
      ...
      const hasRelevantModPaths = modPaths.find(path => {
        ...
        return !parentPaths(path).some(p => excluded.has(p));
      });
      if (hasRelevantModPaths) {
        this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
      }
