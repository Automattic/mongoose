...
/**
 * Wrap pre hooks so we can emit err events
 * on documents regardless of whether or not
 * a callback was passed into .save() or .remove() etc.
 * @private
 */

Document.prototype.__pre = Document.__pre = Document.pre;

Document.prototype.pre =
Document.pre = function mongoosePre (method, async, fn, errback) {
  this.__pre(method, async, fn, errback);
  this.__fix(method);
  return this;
}

/**
 * Fix save and remove hooks so we can handle errors
 * and emit them.
 * @private
 */

Document.__fix =
Document.prototype.__fix = function __fix (method) {
  if (!('save' === method || 'remove' === method)) return

  var proto = this.prototype || this
    , old = proto[method];

  if (old.__fixed) return;

  // wrap hooksjs method
  proto[method] = fixedMethod;
  proto[method].__fixed = 1;

  // hooksjs compat
  proto[method].numAsyncPres = old.numAsyncPres;

  function fixedMethod () {
    var len = arguments.length
      , last = arguments[len - 1]
      , cb = 'function' === typeof last && last
      , args = utils.args(arguments, 0, cb ? len - 1 : len)
      , self = this;

    // substitute our handler for theirs to handle errors
    if (cb) {
      args.push(fn);
    }

    // run the original method passing our callback arg
    return old.apply(this, args);

    // wrap their fn so we can emit errors.
    // if no cb was passed in, the default hook errHandler
    // takes care of emitting for save and remove
    function fn (err) {
      if (err) {
        self.emit('err', err);
      }

      // the users original cb
...
Document.prototype.registerHooks = function () {
  ...
  // wrap remove so regardless of whether or not someone
  // else adds a pre remove the events will be emitted.
  if (this.remove) {
