...
Document.prototype.$__delta = function $__delta(pathsToSave, pathsToSaveSet) {
  ...
  if (optimisticConcurrency) {
    if (optimisticConcurrency instanceof Set) {
      const modPaths = this.directModifiedPaths();
      const hasRelevantModPaths = pathsToSave == null ?
        modPaths.find(path => _pathOrAncestorInSet(path, optimisticConcurrency)) :
        modPaths.find(path => _pathOrAncestorInSet(path, optimisticConcurrency) && isInPathsToSave(path, pathsToSaveSet, pathsToSave));
      if (hasRelevantModPaths) {
        this.$__.version = dirty.length ? VERSION_ALL : VERSION_WHERE;
      }
    } else if (optimisticConcurrency.exclude instanceof Set) {
      const excluded = optimisticConcurrency.exclude;
      const modPaths = this.directModifiedPaths();
      const hasRelevantModPaths = pathsToSave == null ?
...
function operand(self, where, delta, data, val, op) {
  ...
  if (
    self.$__schema.options.optimisticConcurrency === true ||
      self.$__schema.options.optimisticConcurrency instanceof Set ||
      self.$__schema.options.optimisticConcurrency?.exclude instanceof Set
  ) {
    return;
  }
