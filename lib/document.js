 ...
 *     });
 *
 * @param {Array|String|Object} [pathsToValidate] list of paths to validate. If set, Mongoose will validate only the modified paths that are in the given list.
 * If the object pathsToSkip, will validate all paths except the value of pathsToSkip.
...
Document.prototype.validate = function(pathsToValidate, options, callback) {
  ...
  if (arguments.length === 1) {
    if (typeof arguments[0] === 'object' && !Array.isArray(arguments[0]) /*&& !arguments[0].hasOwnProperty('pathsToSkip')*/) {
      options = arguments[0];
      callback = null;
      ...
      options = null;
      pathsToValidate = null;
    } else if (false/*arguments[0].hasOwnProperty('pathsToSkip')*/) {
      skip = true;
    }
    ...
    pathsToValidate = null;
  } else if (false/*typeof pathsToValidate !== 'undefined' && pathsToValidate.hasOwnProperty('pathsToSkip')*/) {
    skip = true;
  }
...
Document.prototype.$__validate = function(pathsToValidate, options, callback, skip) {
  ...
  if (skip) {
    console.log('hi');
    pathsToValidate = pathsToValidate.pathsToSkip;
  }
  if (Array.isArray(pathsToValidate)) {
    paths = _handlePathsToValidate(paths, pathsToValidate, skip);
  }
  if (paths.length === 0) {
...
function _handlePathsToValidate(paths, pathsToValidate, skip) {
  if (skip) {
    // make pathsToValidate = paths intersection !pathsToValidate
    // pathsToValidate = paths.filter((path) => { return pathsToValidate.indexOf(path) < 0; });
  }
