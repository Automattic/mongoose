...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  if (path == null) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      if (someCondition) {
        // Need to add a new condition to someCondition. Current prevailing theory.
        // someCondition alternates between true and false.
        // Currently, set is not "setting" enough as after the create, one entry still stands
        // Setting more than 5 causes failure, however, also causes failure everywhere else when enabled.
        // adding !type had some success
  ...
  // console.log('this is path', path, 'this is parts', parts);
  ...
  for (i = 0; i < parts.length - 1; ++i) {
    ...
    if (!cur) {
      ...
      cur = this.$__getValue(curPath); // I don't think this assignment does anything.
    }
  }
