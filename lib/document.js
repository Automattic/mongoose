...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  // only validate required fields when necessary
  let paths = [];
  for (const path in doc.$__.activePaths.getStatePaths('require')) {
  ...
  for (const state of STATES_TO_VALIDATE) {
    for (const p in doc.$__.activePaths.getStatePaths(state)) {
      ...
      if (_pathType) {
        const splitPath = _pathType.splitPath();
  ...
  if (onlyPrimitiveValues && paths.length === 0) {
...
Document.prototype.$__reset = function reset(skipBackup) {
...
Document.prototype.$__hasOnlyPrimitiveValues = function $__hasOnlyPrimitiveValues() {
  if (this.$__.populated || this.$__.wasPopulated) {
    return false;
  }
  const doc = this._doc;
  if (doc == null) {
  ...
  // Iterate keys directly instead of `Object.values(...).every(...)` to avoid
  // allocating an intermediate values array and a closure on every call. This
  // runs once per document in hot paths like `insertMany()` and `$__reset()`.
