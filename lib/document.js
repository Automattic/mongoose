...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  for (const pathToCheck of paths) {
    if (doc.$__schema.nested[pathToCheck]) {
      const _v = doc.$__getValue(pathToCheck);
      if (_v == null || Array.isArray(_v)) {
        continue;
      }
      for (const key of Object.keys(_v)) {
        const path = pathToCheck + '.' + key;
        // Array paths with validators need to be added here because nested
        // paths are not included in `activePaths` when the parent is modified.
        // Do not add document arrays without array-level validators, because
        // their child documents are validated separately.
        if (Array.isArray(_v[key])) {
          const schemaType = doc.$__schema.path(path);
          if (schemaType?.validators.length === 0) {
            continue;
          }
          doValidateOptions[path] = { skipSubdocumentValidation: true };
        }
        // Note that this may increase the number of paths to
        // validate because we may be adding a nested path here.
        paths.add(path);
      }
    }
  }
