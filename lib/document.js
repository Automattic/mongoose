  ...
  if (!isNestedValidate) {
    ...
    for (const path of Object.keys(schemaPaths)) {
      ...
      for (const subdoc of subdocs) {
        ...
        // Validating the whole array already validates every element, so the
        // element path itself is redundant. Its index and scope are still
        // needed: `_addArrayPathsToValidate` may list the element below.
        if (covered[path] !== true) {
