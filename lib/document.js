...
/**
 * Checks which paths have defaults
 * to then delete them from the doc
 *
 * @param {Object} doc
 */
function $__unapplyDefaults(doc) {
  const keys = Object.keys(doc.$__schema.paths);
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i];
    if (doc.$__schema.paths[key].defaultValue && doc.$__schema.paths[key].instance != 'ObjectID') {
      const property = doc.$__schema.paths[key].path;
      delete doc._doc[property];
    }
  }
}
