...
Document.prototype.$set = function $set(path, val, type, options) {
  console.log('what is path', path, typeof path);
  ...
  if (path == null) {
    ...
    for (let i = 0; i < len; ++i) {
      ...
      if (utils.isNonBuiltinObject(valForKey) && pathtype === 'nested') {
        ...
        if (pathtype === 'real' || pathtype === 'virtual') {
          console.log('This is called first and triggers a recursive call')
    ...
    console.log('does this return?')
    return this;
  }

  let pathType = this.$__schema.pathType(path);
  console.log('what is pathType', pathType)
  let parts = null;
  if (pathType === 'adhocOrUndefined') {
  ...
  console.log('what is shouldSet', shouldSet);
  if (shouldSet) {
    let savedState = null;
