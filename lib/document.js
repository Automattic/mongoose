...
Document.prototype.$__presaveValidate = function $__presaveValidate() {
  var self = this
    , promise = new Promise()
    , retPromise = promise
    , docs = self.$__getArrayPathsToValidate();

  docs.forEach(function (doc) {
    //this is not a real save just a stub for recursion and hooks
  ...
  var err = self.$__.saveError;
  if (err) {
    self.$__.saveError = null;
    promise.reject(err);
  } else {
    self.validate(promise.resolve.bind(promise));
  }
...
Document.prototype.$__registerHooksFromSchema = function () {
  ...
  // register save hook with default error propagation
  self.hook('save', self.save, function (err) {
    // emit on the Model if listening
    if (self.listeners('error').length) {
      self.emit('error', err);
    } else if (self.constructor.listeners && self.constructor.listeners('error').length) {
      self.constructor.emit('error', err);
    } else if (self.listeners && self.listeners('error').length) {
      self.emit('error', err);
    } else if (self.db) {
      // emit on the connection
      if (!self.db.listeners('error').length) {
  ...
  q.forEach(function (pair) {
    var pointCut = pair[0];
    var arguments = pair[1];
    if (self instanceof Embedded && pointCut == 'pre' && arguments[0] === 'save') arguments[0] = '$__save'
    self[pointCut].apply(self, arguments);
  }, self);
  return self;
};
