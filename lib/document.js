...
const buildSelectedInfo = require('./helpers/projection/buildSelectedInfo');
 ...
 * ignore
 *
 * The original (pre gh-16385) `isSelected()` algorithm: re-scans `paths`
 * (`Object.keys(selected)`) from scratch on every call. Used whenever no
 * `selectedInfo` trie has been built yet - see `ensureSelectedInfo()`.
 */

function isSelectedLinearScan(selected, paths, path) {
  let inclusive = null;
...
// Worth building the `selectedInfo` trie once a caller is about to make more
// than this many `isSelected()` calls in a row - see `ensureSelectedInfo()`.
const SELECTED_INFO_BUILD_THRESHOLD = 10;

// Keyed by the `$__.selected` object reference (not content), so documents
// hydrated from the same query - which share that exact object, see
// `queryHelpers.js` `createModel()` - share one trie instead of building
// their own. Assumes `$__.selected` is never mutated after being assigned to
// a document (true today - see `document.js` constructor); mutating it
// after this cache is populated would make lookups for every document
// sharing that reference stale.
const selectedInfoCache = new WeakMap();

function getSelectedInfo(selected) {
  let info = selectedInfoCache.get(selected);
  if (info === undefined) {
    info = buildSelectedInfo(selected);
    selectedInfoCache.set(selected, info);
  }
  return info;
}
 ...
 * ignore
 *
 * Builds (or reuses the shared) `selectedInfo` trie for `doc`, if `doc` is
 * about to see enough `isSelected()` calls to be worth it. Called by
 * internal code that knows its own call count in advance, right before it
 * starts looping and calling `isSelected()` per path - see `applyGetters()`,
 * `_getPathsToValidate()`. Direct `doc.isSelected()` calls from user code
 * have no such call site and so never trigger this - see gh-16385.
 *
 * @param {Document} doc
 * @param {Number} expectedCalls - how many `isSelected()` calls the caller is about to make
 * @api private
 */

function ensureSelectedInfo(doc, expectedCalls) {
  if (doc.$__.selected == null || doc.$__.selectedInfo != null) {
    return;
  }
  if (expectedCalls <= SELECTED_INFO_BUILD_THRESHOLD) {
    return;
  }

  const paths = Object.keys(doc.$__.selected);
  if (paths.length === 1 && paths[0] === '_id') {
    // isSelected() special-cases this without ever touching the trie, so
    // building one here would be pure waste.
    return;
  }

  doc.$__.selectedInfo = getSelectedInfo(doc.$__.selected);
}
...
Document.prototype.isSelected = function isSelected(path) {
  ...
  if (this.$__.selectedInfo == null) {
    ...
    return isSelectedLinearScan(this.$__.selected, paths, path);
  }
  const { inclusive, root } = this.$__.selectedInfo;
  ...
  // walk down to `path`'s own trie node, tracking the shallowest-in-the-
  // projection-object (i.e. minimum insertion index) ancestor path that is
  // itself a selected key along the way
  const parts = path.split('.');
  let node = root;
  let ancestorMatch = null;
  for (const part of parts) {
    if (node.children == null || node.children[part] === undefined) {
      node = null;
      break;
    }
    node = node.children[part];
    if (node.index !== undefined && (ancestorMatch == null || node.index < ancestorMatch.index)) {
      ancestorMatch = node;
    }
  }

  // the earliest (by insertion order) selected key found anywhere strictly
  // beneath `path` in the projection, if any
  const descendantMatch = node ? node.minDescendant : null;

  // mirrors the original per-key loop: whichever relationship (path has a
  // selected ancestor, or path has a selected descendant) belongs to the key
  // that appears first in the projection object wins
  if (ancestorMatch && (!descendantMatch || ancestorMatch.index < descendantMatch.index)) {
    return inclusive;
  }
  if (descendantMatch) {
    return inclusive || descendantMatch.key !== path + '.';
  }
  return !inclusive;
};
...
function _getPathsToValidate(doc, pathsToValidate, pathsToSkip, isNestedValidate) {
  ...
  // only validate required fields when necessary
  const requiredPaths = Object.keys(doc.$__.activePaths.getStatePaths('require'));
  // about to call `doc.$__isSelected()` once per required path below - worth
  // precomputing the `selectedInfo` trie upfront if there are enough of them
  ensureSelectedInfo(doc, requiredPaths.length);
  let paths = new Set(requiredPaths.filter(function(path) {
...
function applyGetters(self, json) {
  ...
  // about to call `self.$__isSelected()` once per schema path below - worth
  // precomputing the `selectedInfo` trie upfront if there are enough of them.
  // `_id`, if present, doesn't count - `isSelected('_id')` short-circuits
  // before ever touching the trie/linear scan.
  const expectedIsSelectedCalls = Object.hasOwn(schema.paths, '_id') ? paths.length - 1 : paths.length;
  ensureSelectedInfo(self, expectedIsSelectedCalls);
