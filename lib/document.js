...
Document.prototype.validate = function(pathsToValidate, options, callback) {
  ...
  if (options && typeof options.pathsToSkip === 'string') {
    options.pathsToSkip = options.pathsToSkip.split(' ');
  }
...
Document.prototype.validateSync = function(pathsToValidate, options) {
  ...
  let pathsToSkip = get(options, 'pathsToSkip');

  if (typeof pathsToValidate === 'string') {
    pathsToValidate = pathsToValidate.split(' ');
  } else if (typeof pathsToSkip === 'string') {
  ...
  if (Array.isArray(pathsToValidate)) {
    ...
    const pathsToValidateByExclusion = _handlePathsToSkip(paths, pathsToSkip);
    paths = _handlePathsToValidate(paths, pathsToValidateByExclusion);
  }
  const validating = {};
