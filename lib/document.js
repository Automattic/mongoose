...
Document.prototype.isSelected = function isSelected(path) {
  ...
  // derived cache for the current `$__.selected` projection, so repeated
  // `isSelected()` calls don't re-scan every projection key each time
  if (this.$__.selectedInfo == null) {
    this.$__.selectedInfo = buildSelectedInfo(this.$__.selected);
  }
  const { inclusive, keySet, root } = this.$__.selectedInfo;
