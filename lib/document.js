  ...
  if ('string' !== typeof path) {
    ...
    if (null === path || undefined === path) {
      ...
      while (i--) {
        ...
        if ((pathtype === 'nested') && (path[key] === null)) {
          //clear out _all_ subproperties when a null-value is passed to a nested property
          var self = this;
          this.schema.eachPath(function (path, Schema) {
            if (path.indexOf(key) === 0) {
              self.set(path, null, constructing);
            }
          });
        }
        if (null != path[key]
...
Document.prototype.toObject = function (options) {
  ...
  if (true === options.transform ||
    ...
    if (opts) {
      options.transform = (typeof options.transform === 'function' ? options.transform : opts.transform);
    }
  }
