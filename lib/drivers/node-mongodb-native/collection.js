...
function iter(i) {
  NativeCollection.prototype[i] = function() {
    ...
    if (this._shouldBufferCommands() && this.buffer) {
      ...
      this.conn.emit('buffer', { _id: opId, collectionName: _this.name, method: i, args: args });
      ...
      callback = function collectionOperationCallback(err, res) {
        if (err != null) {
          _this.conn.emit('operation-end', { _id: opId, collectionName: _this.name, method: i, error: err });
        } else {
          _this.conn.emit('operation-end', { _id: opId, collectionName: _this.name, method: i, result: res });
        }
        return lastArg.apply(this, arguments);
      };
      _args = args.slice(0, args.length - 1).concat([callback]);
    ...
    this.conn.emit('operation-start', { _id: opId, collectionName: this.name, method: i, args: _args });
    ...
    try {
      ...
      if (ret != null && typeof ret.then === 'function') {
        return ret.then(
          res => {
            this.conn.emit('operation-end', { _id: opId, collectionName: this.name, method: i, result: res });
            return res;
          },
          err => {
            this.conn.emit('operation-end', { _id: opId, collectionName: this.name, method: i, error: err });
            throw err;
          }
        );
      }
      ...
      if (typeof callback === 'function') {
        ...
        this.conn.emit('operation-end', { _id: opId, collectionName: this.name, method: i, error: error });
      }
      if (typeof lastArg === 'function') {
        lastArg(error);
      } else {
