...
class CastError extends MongooseError {
  ...
  /*
  toJSON() {
    return { stringValue: this.stringValue, kind: this.kind, value: this.value, path: this.path, reason: this.reason, name: this.name, message: this.message };
  }
  */
  /*!
   * ignore
  ...
  init(type, value, path, reason, schemaType) {
    ...
    this.path = path;
    this.reason = reason;
    // this.name = CastError.prototype.name;
  }
