...
function clone(obj, options, isArrayChild, _pathSeen) {
  if (obj == null) {
    return obj;
  }
  ...
  if (Array.isArray(obj)) {
    return cloneArray(obj, options, _pathSeen);
  }
    ...
    if (options) {
      ...
      if (options.retainDocuments && obj.$isMongooseMap) {
        ...
        for (const [key, value] of obj) {
          ret.$__set(key, clone(value, { ...options, parentDoc: clonedParent }, false, _pathSeen));
        }
        return ret;
      }
    }
  ...
  if (objConstructor) {
    switch (getFunctionName(objConstructor)) {
      case 'Object':
        return cloneObject(obj, options, isArrayChild, _pathSeen);
      case 'Date':
...
function cloneObject(obj, options, isArrayChild, _pathSeen) {
  ...
  for (let i = 0; i < len; ++i) {
    ...
    const val = clone(obj[key], options, false, _pathSeen);
...
function cloneArray(arr, options, _pathSeen) {
  let i = 0;
  ...
  const seen = options?._seen;
  if (!seen) {
    if (_pathSeen && _pathSeen.has(arr)) {
      throw new Error('Mongoose does not support circular references in query filters or updates.');
    }
    _pathSeen || (_pathSeen = new Set());
    _pathSeen.add(arr);
  ...
  for (i = 0; i < len; ++i) {
    ret[i] = clone(arr[i], options, true, _pathSeen);
  }

  if (!seen) {
    _pathSeen.delete(arr);
  }
