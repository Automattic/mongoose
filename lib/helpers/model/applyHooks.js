...
const { buildMiddlewareFilter } = require('../middleware');
...
function applyHooks(model, schema, options) {
  ...
  const originalInit = objToDecorate.$__init;
  objToDecorate.$__init = function(doc, opts) {
    const preFilter = buildMiddlewareFilter(opts, 'pre');
    const postFilter = buildMiddlewareFilter(opts, 'post');

    middleware.execPreSync('init', this, [doc, opts], { filter: preFilter });
    const result = originalInit.call(this, doc, opts);
    middleware.execPostSync('init', this, [result], { filter: postFilter });
