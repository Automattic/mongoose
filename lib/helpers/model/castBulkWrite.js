...
module.exports.castInsertOne = async function castInsertOne(originalModel, insertOne, options) {
  ...
  const doc = new model(insertOne['document'], null, { middleware: options.middleware });
  ...
  await insertOne['document'].$validate({ middleware: options.middleware });
  return insertOne;
};
...
module.exports.castReplaceOne = async function castReplaceOne(originalModel, replaceOne, options) {
  ...
  const doc = new model(replaceOne['replacement'], strict, { skipId: true, middleware: options.middleware });
  ...
  await replaceOne['replacement'].$validate({ middleware: options.middleware });
  replaceOne['replacement'] = replaceOne['replacement'].toBSON();
};
