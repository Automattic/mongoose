...
module.exports = function castBulkWrite(model, op, options) {
  ...
  if (op['insertOne']) {
    ...
    return (callback) => {
      ...
      const doc = new model(op['replaceOne']['replacement'], strict);
      if (model.schema.options.timestamps != null) {
        doc.initializeTimestamps();
      }
      if (options.session != null) {
      ...
      // Make sure to remove `_id`, otherwise you get "_id field cannot be changed"
      op['replaceOne']['replacement'] = doc.toObject(internalToObjectOptions);
      delete op['replaceOne']['replacement']._id;
