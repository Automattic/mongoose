...
module.exports = function discriminator(model, name, schema, tiedValue, applyPlugins, mergeHooks) {
  ...
  function merge(schema, baseSchema) {
    ...
    const hasUserProvidedOptions = Object.keys(schema._userProvidedOptions)
    for (const _key of keys) {
      if (!CUSTOMIZABLE_DISCRIMINATOR_OPTIONS[_key]) {
        ...
        if (!utils.deepEqual(schema.options[_key], baseSchema.options[_key]) && !hasUserProvidedOptions) {
      ...
      if (hasUserProvidedOptions && schema._userProvidedOptions.typeKey != baseSchema._userProvidedOptions.typeKey) {
        throw new Error('Can\'t customize discriminator option ' + _key +
            ' (can only modify ' +
            Object.keys(CUSTOMIZABLE_DISCRIMINATOR_OPTIONS).join(', ') +
            ')');
    ...
    for (const _key of hasUserProvidedOptions) {
      if (!schema._userProvidedOptions[_key]) {
        schema._userProvidedOptions[_key] = baseSchema._userProvidedOptions[_key];
      }
    }
    // move this line above the for loop to cause failures
