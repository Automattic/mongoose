  ...
  function _getModelNames(doc, schema, modelNameFromQuery, model) {
    ...
    if (modelNameFromQuery) {
      ...
      if (typeof refPath === 'function') {
        const res = _getModelNamesFromFunctionRefPath(refPath, doc, schema, options.path, modelSchema, options._queryProjection);
        modelNames = res.modelNames;
        refPath = res.refPath;
      } else {
 ...
 * @private
 */
function _getModelNamesFromFunctionRefPath(refPath, doc, schema, populatePath, modelSchema, queryProjection) {
  const modelNames = [];
  let normalizedRefPath = null;
  ...
  if (schemaPath != null && populatePath.endsWith('.' + schemaPath)) {
    ...
    const subdocsWithPaths = _getSubdocsWithIndexedPaths(doc, subdocPath);

    if (subdocsWithPaths.length > 0) {
      for (const subdocWithPath of subdocsWithPaths) {
        const indexedPath = subdocWithPath.indexedPathPrefix + '.' + schemaPath;
        const subdocRefPath = refPath.call(subdocWithPath.subdoc, subdocWithPath.subdoc, indexedPath);
...
/**
 * Walk nested objects/arrays for a subdocument path and return each resolved
 * subdocument with its root-relative indexed path prefix.
 *
 * @param {Document|Object} doc
 * @param {String} subdocPath
 * @returns {{subdoc: Document|Object, indexedPathPrefix: String}[]}
 * @private
 */
function _getSubdocsWithIndexedPaths(doc, subdocPath) {
  if (!subdocPath) {
    return [];
  }

  const segments = subdocPath.split('.');
  let current = [{ subdoc: doc, indexedPathPrefix: '' }];
  ...
  for (const segment of segments) {
    ...
    for (const item of current) {
      ...
      const source = item.subdoc?._doc ?? item.subdoc;
      if (segment === '$*') {
        if (source instanceof Map) {
          for (const [key, value] of source.entries()) {
            if (value != null) {
              const valuePath = item.indexedPathPrefix.length === 0 ?
                key :
                item.indexedPathPrefix + '.' + key;
              next.push({ subdoc: value, indexedPathPrefix: valuePath });
            }
          }
          continue;
        }
      ...
      const childPath = item.indexedPathPrefix.length === 0 ?
        segment :
        item.indexedPathPrefix + '.' + segment;

      if (Array.isArray(child)) {
        for (let i = 0; i < child.length; ++i) {
          if (child[i] != null) {
            next.push({ subdoc: child[i], indexedPathPrefix: childPath + '.' + i });
          }
        }
      } else if (child != null) {
        next.push({ subdoc: child, indexedPathPrefix: childPath });
      }
    }
