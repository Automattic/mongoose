...
module.exports = function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    try {
      currentOptions = addModelNamesToMap(model, map, available, modelNames, options, currentOptions, data, originalModel, ret, isRefPath, doc);
    } catch (err) {
      return err;
    }
  }
...
function addModelNamesToMap(model, map, available, modelNames, options, currentOptions, data, originalModel, ret, isRefPath, doc) {
  ...
  while (k--) {
    ...
    const Model = originalModel && originalModel[modelSymbol] ?
      originalModel :
    ...
    if (!available[modelName] || currentOptions.perDocumentLimit != null) {
      ...
      if (options.isVirtual && get(options.virtual, 'options.options')) {
        currentOptions.options = utils.clone(options.virtual.options.options);
      }
