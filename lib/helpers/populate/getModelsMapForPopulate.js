...
module.exports = function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    const getters = 'getters' in _populateOptions ?
      ...
      isVirtual && get(virtual, 'options.getters', false);
    ...
    try {
      addModelNamesToMap(model, map, available, modelNames, options, data, originalModel, ret, isRefPath, doc);
    } catch (err) {
      return err;
    }
  }
...
function addModelNamesToMap(model, map, available, modelNames, options, data, originalModel, ret, isRefPath, doc) {
  ...
  while (k--) {
    ...
    if (!available[modelName] || options.perDocumentLimit != null) {
