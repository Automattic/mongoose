...
module.exports = function getSchemaTypes(model, schema, doc, path) {
  ...
  if (discriminatorKey) {
    if (doc != null && doc[discriminatorKey] != null) {
      const discriminator = getDiscriminatorByValue(model, doc[discriminatorKey]);
      schema = discriminator ? discriminator.schema : schema;
    } else if (model.discriminators != null) {
      return Object.values(model.discriminators).
        reduce((arr, model) => arr.concat(getSchemaTypes(model, model.schema, null, path)), []);
    }
  }
