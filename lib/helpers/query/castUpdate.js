...
function walkUpdatePath(schema, obj, op, options, context, filter, pref) {
  ...
  while (i--) {
    ...
    if (getConstructorName(val) === 'Object') {
      ...
      if (schematype && schematype.caster && castOps.has(op)) {
        // embedded doc schema
        if ('$each' in val) {
          hasKeys = true;
          try {
      ...
      } else if ((op === '$currentDate') || (castOps.has(op) && schematype)) {
...
/*!
 * These operators need Objects as value.
 */
...
function castUpdateVal(schema, val, op, $conditional, context, path) {
  ...
  const cond = schema.caster && castOps.has(op) &&
      (utils.isObject(val) || Array.isArray(val));
  if (cond && !overwriteOps.has(op)) {
