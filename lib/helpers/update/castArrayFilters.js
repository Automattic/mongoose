...
module.exports = function castArrayFilters(query) {
  ...
  for (const filter of arrayFilters) {
    ...
    for (const key in filter) {
      ...
      if (updatedPathsByFilter[key] == null  && typeof updatedPathsByFilter[key] == 'object') {
        continue;
      }
      console.log('gh-9468, problem is that it is allowed to get here');
      const dot = key.indexOf('.');
      let filterPath = dot === -1 ?
        updatedPathsByFilter[key] + '.0' :
        updatedPathsByFilter[key.substr(0, dot)] + '.0' + key.substr(dot);
      // add an if statement that if the filterPath has the word undefined, toss it maybe?
      if (filterPath == null) {
