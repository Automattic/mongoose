    ...
    if (v && Array.isArray(v.$in)) {
      v.$in.forEach((v, i) => {
        validatorsToExecute.push(
          schemaPath.doValidate(v, context, { updateValidator: true, middleware: query.options.middleware }).catch(err => {
    ...
    if (schemaPath && schemaPath.$isMongooseDocumentArray) {
      validatorsToExecute.push(
        schemaPath.doValidate(
          ...
          { middleware: query.options.middleware }
        ).catch(err => {
      ...
      for (const atomicUpdate of arrayAtomicUpdates[arrayUpdate]) {
        validatorsToExecute.push(
          schemaPath.doValidate(
            atomicUpdate,
            options?.context === 'query' ? query : null,
            { updateValidator: true, middleware: query.options.middleware }
          ).catch(err => {
            err.path = arrayUpdate;
            validationErrors.push(err);
          })
