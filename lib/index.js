...
var Schema = require('./schema')
  ...
  , PolymorphicSchema = require('./polymorphicschema')
  , Types = require('./types')
  , Query = require('./query')
...
Mongoose.prototype.model = function (name, schema, collection, skipInit) {
  ...
  if (!collection) {
    collection = (schema || this.modelSchemas[name]).set('collection') || utils.toCollectionName(name);
  }
  ...
  if (!this.models[name]) {
    ...
    // also create sub-models for PolymorphicSchemas
    if (schema.schemasByModelName) {
      for (var modelName in schema.schemasByModelName) {
        this.model(PolymorphicSchema.getSubModelName(model, modelName), schema.schemasByModelName[modelName], collection, skipInit);
      }
    }
...
/**
 * Export PolymorphicSchema
 *
 * @api public
 */

exports.PolymorphicSchema = PolymorphicSchema;
