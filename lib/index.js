...
const Schema = require('./schema'), SchemaType = require('./schematype'), VirtualType = require('./virtualtype'), STATES = require('./connectionstate'), Types = require('./types'), Query = require('./query'), Model = require('./model'), Document = require('./document'), utils = require('./utils'), format = utils.toCollectionName, pkg = require('../package.json');
...
const checkReplicaSetInUri = function(uri) {
  if (!uri) {
    return false;
  }
  ...
  if (queryStringStart !== -1) {
    try {
      const obj = querystring.parse(uri.substr(queryStringStart + 1));
