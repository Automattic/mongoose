...
function handleSave (promise, self) {
  return tick(function handleSave (err, res) {
    if (err) return promise.error(err);
    self.emit('save', self);
    promise.complete(self, res);
    promise = null;
    self = null;
  });
}
 ...
 * @param {Object} config object containing two optional parameters: options and
 *                 queryParams to be passed to the save method.
 * @param {Function} fn
 * @api public
 */

Model.prototype.save = function save (config, fn) {
  ...
  if ('undefined' === typeof options.safe && this.options.safe) {
  ...
  if (this.isNew) {
    ...
    if (delta) {
      var criteria = utils.options({ _id: this._doc._id }, queryParams);
      this.collection.update(criteria, delta, options, complete);
    } else {
      complete(null);
    }
  }
