...
async function _populatePath(model, docs, populateOptions) {
  ...
  if (populateOptions.ordered) {
    ...
    for (const arr of params) {
      ...
      allDeferredPopulates = allDeferredPopulates.concat(deferredPopulates);
    }
  } else {
    // By default, populate in parallel
    const promises = [];
    ...
    for (const { docs, deferredPopulates } of await Promise.all(promises)) {
      vals = vals.concat(docs);
      allDeferredPopulates = allDeferredPopulates.concat(deferredPopulates);
    }
  }
  ...
  if (allDeferredPopulates.length > 0) {
    for (const arr of params) {
      const mod = arr[0];
      ...
      if (childDocsToPopulate.length > 0) {
        for (const pop of allDeferredPopulates) {
