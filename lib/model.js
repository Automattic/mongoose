...
const FilterValidationError = require('./error/filterValidation');
...
/**
 * Validates query filter values against this model's schema using
 * [`Model.validate()`](https://mongoosejs.com/docs/api/model.html#Model.validate()).
 *
 * #### Example:
 *
 *     const Model = mongoose.model('Test', Schema({
 ...
 *
 *     // Succeeds
 *     await Model.validateFilter({ status: 'open' });
 *
 *     try {
 *       await Model.validateFilter({ status: 'invalid' });
 *     } catch (err) {
 *       err instanceof mongoose.Error.FilterValidationError; // true
 *       Object.keys(err.errors); // ['status']
 *     }
 *
 * @param {object} conditions query filter object
 * @param {object} [context] context passed to validators
 * @return {Promise<object>} the query filter if validation succeeded
 * @api public
 */

Model.validateFilter = async function validateFilter(conditions, context) {
  ...
  let schema = this.schema;
  if (schema?.discriminators != null && conditions[schema.options.discriminatorKey] != null) {
    schema = getSchemaDiscriminatorByValue(schema, conditions[schema.options.discriminatorKey]) || schema;
  }

  const error = await _validateFilterConditions(this, schema, conditions, context);
  if (error != null) {
    throw error;
  }
...
async function _validateFilterConditions(model, schema, obj, context, error) {
  const keys = Object.keys(obj);

  for (const key of keys) {
    if (key === '$or' || key === '$nor' || key === '$and') {
      for (const sub of obj[key]) {
        error = await _validateFilterConditions(model, schema, sub, context, error);
      }
      continue;
    }

    if (key === '$where' || key === '$expr' || key === '$text' || key === '$comment') {
      continue;
    }
    ...
    if (utils.isPOJO(val) && !Object.keys(val).some(isOperator)) {
      for (const nestedKey of Object.keys(val)) {
        ...
        if (schema.path(nestedPath) == null) {
          continue;
        }
        error = await _validateFilterPathValue(model, schema, nestedPath, val[nestedKey], context, error);
      }
      continue;
    }
    if (schematype == null) {
    ...
    error = await _validateFilterPathValue(model, schema, key, val, context, error);
  }
...
async function _validateFilterPathValue(model, schema, path, val, context, error) {
  ...
  if (utils.isPOJO(val) && Object.keys(val).some(isOperator)) {
    for (const key of Object.keys(val)) {
      ...
      if (key === '$in' || key === '$nin') {
        const arr = Array.isArray(val[key]) ? val[key] : [val[key]];
        for (const v of arr) {
          if (!(v instanceof RegExp)) {
            error = await _validateFilterSingleValue(model, schema, path, v, context, error);
          }
        }
      } else if (key === '$eq' || key === '$ne') {
        if (!(val[key] instanceof RegExp)) {
          error = await _validateFilterSingleValue(model, schema, path, val[key], context, error);
        }
      } else if (key === '$not') {
        error = await _validateFilterPathValue(model, schema, path, val[key], context, error);
      }
    }
    return error;
  }

  return _validateFilterSingleValue(model, schema, path, val, context, error);
}
...
async function _validateFilterSingleValue(model, schema, path, value, context, error) {
  const schemaType = schema.path(path);
  if (schemaType == null) {
    return error;
  }
  ...
  try {
    await schemaType.doValidate(value, context, { path });
  } catch (err) {
    if (err.name !== 'ValidatorError' && err.name !== 'CastError') {
      throw err;
    }
    // `error` is the shared FilterValidationError accumulator, not the caught `err`.
    // Lazily create it on the first known validation failure so the happy path
    // does not allocate, then reuse it for subsequent path errors.
    if (error == null) {
