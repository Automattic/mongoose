...
Model.prototype.save = function (options, fn) {
  ...
  var p1 = p0.all(function () {
    var subDocs = _this.$__getAllSubdocs();
  ...
  p1
    ...
    .then(function (result) {
      ...
      return finalPromise.fulfill(_this, numAffected);
    }
    , function (err) {
      // If the initial insert fails provide a second chance.
      // (If we did this all the time we would break updates)
      if (_this.$__.inserting) {
        _this.isNew = true;
...
Model.ensureIndexes = function ensureIndexes (cb) {
  ...
  var _this = this
  , safe = _this.schema.options.safe;
  ...
  var create = function () {
    ...
    var options = index[1];
    options.safe = safe;
    _this.collection.ensureIndex(index[0], options, tick(function (err) {
...
Model.create = function create (doc, fn) {
  ...
  args.forEach(function (doc) {
    toExecute.push(function (callback) {
      (new _this(doc)).save(function (error, doc) {
        callback(error, doc);
      });
    });
  });
...
Model.populate = function (docs, paths, cb) {
  ...
  while (i--) {
    path = paths[i];
    if ('function' === typeof path.model) _this = path.model;
    populate(_this, docs, path, next);
  }
