...
const saveTracingChannel = diagnosticsChannel.tracingChannel('mongoose:save');
const modelTracingChannel = diagnosticsChannel.tracingChannel('mongoose:model');
...
Model.prototype.save = async function save(options) {
  if (!saveTracingChannel.hasSubscribers) {
    return modelSaveWithoutTrace.call(this, options);
  }

  const model = this.constructor;
  const collection = model?.collection?.name;
  const database = model?.db?.name;
  ...
  return saveTracingChannel.tracePromise(modelSaveWithoutTrace, context, this, options);
};

async function modelSaveWithoutTrace(options) {
