...
Model.prototype.save = function (fn) {
  ...
  var p3 = p2.then(
    function (result) {
      ...
      var numAffected = result ? result.length ? result.length : result : 0;
...
Model.create = function create (doc, fn) {
  ...
  var docs = args.map(function (arg) {return new ModelConstructor(arg);});

  var p1 = new Promise
    , retPromise = p1;
  docs.forEach(function (doc) {
    retPromise = retPromise.then(doc.save.bind(doc));
  });
  p1.fulfill();
  retPromise.onResolve(cb);
  return retPromise;
};
