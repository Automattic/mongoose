...
Model.prototype._where = function _where () {
  ...
  if (this.options.shardkey && 'Object' == this.options.shardkey.constructor.name) {
    var paths = Object.keys(this.options.shardkey)
      , len = paths.length
      , val
    ...
    for (var i = 0; i < len; ++i) {
      ...
      if (isMongooseObject(val)) {
        where[paths[i]] = val.toObject({ depopulate: true })
      } else if (val.valueOf) {
        where[paths[i]] = val.valueOf();
      } else {
        where[paths[i]] = val;
      }
    }
