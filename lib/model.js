...
function handleSave (promise, self) {
  return new Promise(function handleSave (err, result) {
    if (err) {
...
Model.prototype.save = function (fn) {
  ...
  var promise1 = this.presaveValidate();
  promise1.then(function () {
    var promise2 = handleSave(promise, self)
      , options = {}
    ...
    if (self.isNew) {
      ...
      if (!utils.object.hasOwnProperty(obj || {}, '_id')) {
        ...
        return promise2.reject(new Error('document must have an _id before saving'));
      }

      self.$__version(true, obj);
      self.collection.insert(obj, options, function (err, ret) {
        promise2.resolve(err, ret);
      });
      self.$__reset();
      self.isNew = false;
      ...
      if (delta) {
        if (delta instanceof Error) return promise2.reject(delta);
