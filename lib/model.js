...
function _populate (model, docs, options, cb) {
...
function populate (model, docs, options, cb) {
  ...
  for( i in modelsMap ){
    ...
    if (options.options && options.options.limit) {
      options.options.limit = options.options.limit * ids.length;
    }

    findDocsForPopulate(mod, match, select, docs, rawIds, assignmentOpts, cb);
  }
}
...
function getModelsMapForPopulate(model, schema, docs, options){
  ...
  if (refPath) {
    for (i = 0; i < len; i++) {
      doc = docs[i];
      modelName = String(utils.getValue(refPath, doc));
...
function findDocsForPopulate (mod, match, select, docs, rawIds, assignmentOpts, cb){
  mod.Model.find(match, select, mod.options.options, function (err, vals) {
    if (err) return cb(err);

    var lean = mod.options.options && mod.options.options.lean
      , len = vals.length
      , rawOrder = {}
      , rawDocs = {}
      , key
