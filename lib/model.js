...
Model._applyValidation = function( update, cb ){
  ...
  for( var k in update){
    ...
    if( k[0] !== '$' ){
      var toCycle = {}; toCycle[k] = update[matchName] ;  
      op = '$set';
    } else {
      op = k;
      toCycle = update[k];
    ...
    for( var matchName in toCycle){ 

      // Clean up position indexes ('some.4.thing' or 'some.$.thing')
      parts = matchName.replace(/(\.[0-9]+\.)|(\.\$\.)/, '.');
      parts = parts.replace(/(\.[0-9]+$)|(\.\$$)/, '').split('.');

      // console.log("Turned into: " + parts );
      ...
      for (var i = 0, l = parts.length; i < l; i++) {
        ...
        // It's a schema: reset the currPath, set the new schema
        if( currSchema.paths[part] && currSchema.paths[part].schema){
          // console.log(" --  It's a schema!"); 
          currSchema = currSchema.paths[part].schema;
          currPath = '';
        } else {
          // console.log(" --  It's not a schema!"); 
      ...
      var currValue = update[matchName]; // TODO: Fix for unset
      ...
      (function(){ // Make a separate context
        ...
        function explore( obj, schema, path ){
          ...
          // It's a path defined in the current schema:
          // run validation!
          if( schema.path(path )){
             handled = true;
             validatePath( schema, path, matchName + '.' + path, obj );

          // Not a path defined: keep looking into the object's structure
          } else {

            if( typeof( obj ) === 'object' ) {
              for( var k in obj ){
                var v = obj[k];
                explore ( v, schema, path == '' ? k : path + '.' + k );
              }
            }
          } // if( schema.path(path )){
  ...
  function validatePath( schemaType, currPath, matchName, value ){
      ...
      schemaType.paths[currPath].doValidate(value, function (err) {
        if( err ){
          ...
          if ( 'string' === typeof err) {
            err = new ValidatorError(matchName, err);
          }
          errors[matchName] = err;
        }          
        --total || complete();
      }, self );
    });
