...
Model.insertMany = async function insertMany(arr, options) {
  ...
  try {
    [arr] = await execPreWithFilter(this._middleware, 'insertMany', this, [arr], options);
  } catch (error) {
    await execPostWithFilter(this._middleware, 'insertMany', this, [arr], { error }, options);
  }
  ...
  const result = await execPostWithFilter(this._middleware, 'insertMany', this, [docAttributes], {}, options);
  return result[0];
};
...
Model.bulkWrite = async function bulkWrite(ops, options) {
  ...
  try {
    [ops, options] = await execPreWithFilter(this.hooks, 'bulkWrite', this, [ops, options], options);
  } catch (err) {
    if (err instanceof Kareem.skipWrappedFunction) {
      ops = err;
    } else {
      await execPostWithFilter(this.hooks, 'bulkWrite', this, [null], { error: err }, options);
    }
  }
  ...
  if (ordered) {
    ...
    try {
      ...
      await execPostWithFilter(this.hooks, 'bulkWrite', this, [null], { error }, options);
    }
  } else {
    let validOpIndexes = [];
  ...
  await execPostWithFilter(this.hooks, 'bulkWrite', this, [res], {}, options);
...
async function buildPreSavePromise(document, options) {
  const [newOptions] = await execPreWithFilter(document.schema.s.hooks, 'save', document, [options], options);
