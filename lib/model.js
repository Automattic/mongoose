...
function handleSave (promise, self) {
  return tick(function handleSave (err, result) {
    ...
    if (self.__version && !self._inserting) {
      ...
      if (numAffected > 0) {
        var path = self.schema.options.versionKey;
        var version = self.getValue(path);
        self.setValue(path, version + 1);
      } else {
...
function handleMongooseArray (self, where, delta, data, arr) {
  ...
  while (i--) {
    op = ops[i];
    var val = atomics[op];
    var value = isMongooseObject(val)
    ...
    operand(self, where, delta, data, value, op);
  }
}
...
Model.prototype._version = function _version (where, delta) {
  var path = this.schema.options.versionKey;

  if (true === where) {
    // this is an insert
    if (path) this.setValue(path, delta[path] = 0);
    return;
  }
