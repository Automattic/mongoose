...
function populate(model, docs, options, callback) {
  ...
  for (i = 0; i < len; i++) {
    ...
    hasOne = true;
    match.$or = [];
    for (var i = 0; i < mod.docs.length; ++i) {
      var query = {};
      query[mod.foreignFields[i]] = mod.ids[i];
      match.$or.push(query);
    }
  ...
  function _assign(err, vals, mod, assignmentOpts) {
    ...
    assignVals({
      rawIds: mod.ids,
      localFields: mod.localFields,
      foreignFields: mod.foreignFields,
      rawDocs: rawDocs,
      rawOrder: rawOrder,
      docs: mod.docs,
      path: options.path,
...
function assignVals(o) {
  ...
  for (var i = 0; i < docs.length; ++i) {
    if (utils.getValue(o.foreignFields[i], docs[i]) == null) {
      continue;
    }
    utils.setValue(foreignFields[i], rawIds[i], docs[i], setValue);
  }
}
...
function getModelsMapForPopulate(model, docs, options) {
  ...
  for (i = 0; i < len; i++) {
    ...
    var virtual = model.schema.virtuals[options.path];
    ...
    if (doc.$__) {
      doc.populated(options.path, Array.isArray(ret) ? ret.slice() : ret, options);
    }
    ...
    while (k--) {
      ...
      if (!available[modelName]) {
        ...
        available[modelName].localFields.push(localField);
        available[modelName].foreignFields.push(foreignField);
      }
    }
  }
