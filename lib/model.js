...
async function _populatePath(model, docs, populateOptions) {
  ...
  // Handle deferred sub-populate for cases with per-document match functions.
  // We defer sub-populate so sift filtering can compare ObjectIds (not populated docs).
  for (const arr of params) {
    const mod = arr[0];
    const subPopulate = mod.options.populate;
    if (subPopulate && Array.isArray(mod.match)) {
      // Get the assigned (filtered) children from parent docs
      let childDocsToPopulate;
      for (const doc of mod.docs) {
        const childVal = doc.$__ != null ? doc.get(mod.options.path) : doc[mod.options.path];
        if (Array.isArray(childVal)) {
          childDocsToPopulate = childVal;
        } else if (childVal != null) {
          childDocsToPopulate = [childVal];
        }
      }
      if (childDocsToPopulate.length > 0) {
        await mod.model.populate(childDocsToPopulate, subPopulate);
      }
    }
  }
