...
const { bindPostMiddlewareOptions, buildMiddlewareFilter } = require('./helpers/buildMiddlewareFilter');
const { builtInMiddleware } = require('./schema/symbols');
  ...
  try {
    ...
    if (shouldValidateBeforeSave) {
      ...
      const validateOptions = hasValidateModifiedOnlyOption ?
        ...
        {};
      validateOptions.middleware = options.middleware;
...
Model.prototype.save = async function save(options) {
  ...
  return traceSave(async function maybeTracedSave() {
    try {
      ...
      // A post hook may have started another save on this document.
      if (_this.$__.saveOptions === options) {
...
Model.prototype.deleteOne = function deleteOne(options) {
  ...
  if (options.sort instanceof Map) {
    originalOptions.sort = new Map(options.sort);
  }
  ...
  // Keep internal document callbacks active when user hooks are skipped.
  queryPreDeleteOne[builtInMiddleware] = true;
  callSubdocPreHooks[builtInMiddleware] = true;
  skipIfAlreadyDeleted[builtInMiddleware] = true;
  callSubdocPostHooks[builtInMiddleware] = true;
  queryPostDeleteOne[builtInMiddleware] = true;
  query.pre(queryPreDeleteOne);
  query.pre(callSubdocPreHooks);
  query.pre(skipIfAlreadyDeleted);
  query.post(callSubdocPostHooks);
  query.post(queryPostDeleteOne);
  ...
  async function queryPreDeleteOne() {
    ...
    for (const key of Object.keys({ ...originalOptions, ...options })) {
      if (!Object.hasOwn(options, key)) {
        delete query.options[key];
        delete query._mongooseOptions[key];
      } else if (!utils.deepEqual(options[key], originalOptions[key])) {
        changedOptions[key] = options[key];
      }
    }
    query.setOptions(changedOptions);
    query.deleteOne(where);
    if (self.$session() != null && !('session' in query.options)) {
  ...
  function skipIfAlreadyDeleted() {
    if (self.$__.isDeleted) {
      throw new Kareem.skipWrappedFunction();
    }
  }

  function callSubdocPostHooks() {
    const postFilter = buildMiddlewareFilter(query.options, 'post');
 ...
 * @param {boolean} [options.autoCreate] Controls automatic collection creation. Uses the configured default when omitted.
 * @param {boolean|object} [options.middleware=true] Controls user `createCollection` middleware when automatic collection creation is enabled.
 * @param {boolean} [options.middleware.pre=true] set to `false` to skip only pre hooks
 * @param {boolean} [options.middleware.post=true] set to `false` to skip only post hooks
 * @return {Promise}
 * @api public
...
Model.syncIndexes = async function syncIndexes(options) {
  ...
  if (autoCreate) {
    try {
      const createCollectionOptions = options?.middleware === undefined ? undefined : { middleware: options.middleware };
      await this.createCollection(createCollectionOptions);
    } catch (err) {
      if (err != null && (err.name !== 'MongoServerError' || err.code !== 48)) {
        throw err;
      }
  ...
  if (options.ordered) {
    for (let i = 0; i < args.length; i++) {
      try {
        ...
        let toSave = doc;
        if (!(toSave instanceof Model)) {
          toSave = new Model(toSave, null, { middleware: options.middleware });
        }
...
Model.insertOne = async function insertOne(doc, options) {
  ...
  if (!(doc instanceof Model)) {
    doc = new Model(doc, null, { middleware: options?.middleware });
  }
...
Model.watch = function(pipeline, options) {
  ...
  options = options || {};
  const watchOptions = utils.omit(options, ['hydrate', 'middleware']);
  options.model = this;
...
async function _insertMany(arr, options) {
  ...
  function validateDoc(doc, index) {
    ...
    if (!(doc instanceof ThisModel)) {
      ...
      doc = new ThisModel(doc, null, { middleware: options.middleware });
      createdNewDoc = true;
    }
    ...
    const validateOptions = createdNewDoc ? { _skipParallelValidateCheck: true } : {};
    validateOptions.middleware = options.middleware;
    return doc.$validate(validateOptions)
      .then(() => doc)
      .catch(error => {
        if (ordered === false) {
          error.index = index;
 ...
 * @param {boolean} [options.validateModifiedOnly] if true, validate only modified paths
 * @param {boolean} [options.skipValidation=false] if true, skip validation for all documents
 * @param {boolean|object} [options.middleware=true] set to `false` to skip all user-defined middleware
 * @param {boolean} [options.middleware.pre=true] set to `false` to skip only pre hooks
 * @param {boolean} [options.middleware.post=true] set to `false` to skip only post hooks
...
Model.bulkSave = async function bulkSave(documents, options) {
  ...
  for (const document of documents) {
    ...
    document.$__.saveOptions.middleware = options.middleware;
    document.$__.saveOptions.__subdocs = null;
  }
  ...
  // Validation already ran before save middleware.
  const writeOperations = this.buildBulkWriteOperations(documents, { ...options, skipValidation: true });
...
async function buildPreSavePromise(document, options) {
  const shouldValidate = options.validateBeforeSave == null ?
    document.$__schema.options.validateBeforeSave : options.validateBeforeSave;
  if (!options.skipValidation && shouldValidate) {
    const validateOptions = { middleware: options.middleware };
    if (Object.hasOwn(options, 'validateModifiedOnly')) {
      validateOptions.validateModifiedOnly = options.validateModifiedOnly;
    }
    await document.$validate(validateOptions);
  }
...
async function handleSuccessfulWrite(document, options) {
  ...
  const postFilter = buildMiddlewareFilter(options, 'post');
  const middleware = bindPostMiddlewareOptions(document.schema.s.hooks, 'save', options);
  return middleware.execPost('save', document, [document], { filter: postFilter });
}
 ...
 * @param {boolean|object} [options.middleware=true] set to `false` to skip user init hooks
 * @param {boolean} [options.middleware.pre=true] set to `false` to skip only pre-init hooks
 * @param {boolean} [options.middleware.post=true] set to `false` to skip only post-init hooks
...
Model.validate = async function validate(obj, pathsOrOptions, context) {
  ...
  for (const path of paths) {
    ...
    promises.push(
      schemaType.doValidate(val, context, { path: path, middleware: pathsOrOptions?.middleware }).catch(err => {
        error = error || new ValidationError();
        error.addError(path, err);
      })
    );
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  if (typeof name === 'function' && name.prototype instanceof Model) {
    ...
    // generate new class
    model = function model(doc, fields, options) {
      const filter = buildMiddlewareFilter(options, 'pre');
      model.hooks.execPreSync('createModel', doc, [], { filter });
      if (!(this instanceof model)) {
        return new model(doc, fields, options);
      }
      const discriminatorKey = model.schema.options.discriminatorKey;

      if (model.discriminators == null || doc == null || doc[discriminatorKey] == null) {
        Model.call(this, doc, fields, options);
        return;
      }
      ...
      if (Discriminator != null) {
        return new Discriminator(doc, fields, options);
      }
