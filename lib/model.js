...
Model.prototype.$__handleSave = async function $__handleSave(options) {
  const saveOptions = {};
  ...
  if (this.$isNew) {
    ...
    return this[modelCollectionSymbol].insertOne(obj, saveOptions).catch(err => {
      _setIsNew(this, true);
      throw err;
    });
  }
  ...
  if (delta) {
    ...
    _setIsNew(this, false);
    return this[modelCollectionSymbol].updateOne(where, update, saveOptions).then(
      ret => {
        if (ret == null) {
          ret = { $where: where };
        } else {
    ...
    return this.constructor.collection.findOne(where, optionsWithCustomValues)
      .then(documentExists => {
        const matchedCount = !documentExists ? 0 : 1;
        return { $where: where, matchedCount };
      });
  }
};
...
Model.prototype.$__save = async function $__save(options) {
  ...
  let result = null;
  try {
    result = await this.$__handleSave(options);
  } catch (err) {
  ...
  if (writeConcern !== 0) {
    ...
    if (result != null && numAffected <= 0) {
      this.$__undoReset();
      const error = new DocumentNotFoundError(result.$where,
