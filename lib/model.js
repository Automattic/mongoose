...
const createPathsToSaveMatcher = require('./helpers/document/createPathsToSaveMatcher');
...
Model.prototype.$__save = async function $__save(options) {
  ...
  try {
    ...
    if (this.$isNew) {
      ...
      const delta = this.$__delta({ pathsToSave: options.pathsToSave });
      const unsavedState = getUnsavedState(this, options.pathsToSave);
      ...
      if (delta) {
        ...
        this.$__reset();
        restoreUnsavedState(this, unsavedState);
...
function getUnsavedState(doc, pathsToSave) {
  const isPathIncluded = createPathsToSaveMatcher(pathsToSave);
  if (isPathIncluded == null) {
    return null;
  }

  const allDirty = doc.$__dirty();
  const unsavedDirty = allDirty.filter(dirty => isPathIncluded(dirty.path) === false);
  const unsavedDefaultPaths = Object.keys(doc.$__.activePaths.getStatePaths('default'))
    .filter(path => isPathIncluded(path) === false);
...
function restoreUnsavedState(doc, unsavedState) {
  if (unsavedState == null) {
    return;
  }

  for (const dirty of unsavedState.unsavedDirty) {
  ...
  for (const path of unsavedState.unsavedDefaultPaths) {
