...
Model.diffIndexes = function diffIndexes() {
  const toDrop = [];
  const toCreate = [];
  // need this.listIndexes to get all
  // the indexes in mongodb
  // look at cleanIndexes()
  // as it seems it is already done ↓
  // use this.schema.indexes()
  // to get indexes defined on the schema
  this.listIndexes((err, indexes) => {
    console.log('err', err);
    console.log('indexes', indexes);
    console.log(this.schema.indexes());

    // Do const schemaIndexes = this.schema.indexes()[0];
    // for (const key of schemaIndexes)
    // log the keys first to make sure this is the correct line of thought
    // use the isDefaultIdIndex() method to check for that
  });
