...
Model.prototype.$__save = function(options, callback) {
  this.$__handleSave(options, (error, result) => {
    ...
    if (this.$__.version && !this.$__.inserting) {
      var doIncrement = VERSION_INC === (VERSION_INC & this.$__.version);
      this.$__.version = undefined;

      if (numAffected <= 0) {
        // the update failed. pass an error back
        var err = new VersionError(this);
        return callback(err);
      }
      ...
      if (doIncrement) {
        ...
        var version = this.getValue(key) | 0;
...
Model.init = function init(callback) {
  ...
  if (callback) {
    ...
    this.$init.catch = function() {
      this.$caught = true;
      return _catch.apply(this.$init, arguments);
    };
  }
...
Model.geoNear = function(near, options, callback) {
  ...
  if (!near) {
    return utils.promiseOrCallback(callback, cb => {
      cb(new Error('Must pass a near option to geoNear'));
    });
  }
  ...
  return utils.promiseOrCallback(callback, cb => {
    var handler = (err, res) => {
      ...
      for (var i = 0; i < res.results.length; i++) {
        ...
        res.results[i].obj = new this();
        res.results[i].obj.init(temp, init);
      }
    };

    if (options.query != null) {
      options.query = utils.clone(options.query);
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  model.insertMany = function(arr, options, callback) {
    ...
    return utils.promiseOrCallback(callback, cb => {
      model.$__insertMany(arr, options, cb);
    });
  };
