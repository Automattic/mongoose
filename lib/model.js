...
Model.prototype._populate = function populate (schema, oid, fields, fn) {
      ...
      if (!schema && obj[i] && 'Object' === obj[i].constructor.name) {
        ...
        if (obj[i] && schema && populate[path]) {
          if (populate[path].sub) {
            ...
            self._populate(schema, obj[i], populate[path], function (err, doc) {
              if (err) return error(err);
              obj[i] = doc;
              next();
            });
...
Model.prototype._reset = function reset () {
  // clear atomics
