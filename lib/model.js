...
Model.prototype._populate = function populate (schema, oid, query, fn) {
  if (!Array.isArray(oid)) {
    ...
    .findOne(match, query.select, query.options, fn);
  }
  ...
  match._id || (match._id = { $in: oid });

  model.find(match, query.select, query.options, function (err, docs) {
 ...
 *     })
 *
 * @param {Document|Array} doc Either a single document or array of documents to populate.
 * @param {Object} options A hash of key/val (path, options) used for population.
 * @param {Function} cb(err, doc) A callback, executed upon completion. Recieves `err` and the `doc`.
 * @api public
 */

Model.populate = function (doc, paths, cb) {
  assert.equal('function', typeof cb);

  // options is an array of objects [{ path: 'blah' }, { path: 'yep' }]
  // doc may be an array of docs

  // options can specify multiple paths to populate
  // each path has its own query options
  ...
  while (i--) {
    path = paths[i];
    populate(this, doc, path, next);
  }

  function next (err) {
    if (next.err) return;
    if (err) return cb(next.err = err);
    if (--pending) return;
    cb(null, doc);
  }
}
...
function populate (model, docs, options, cb) {
  // TODO doc may be an array of docs


  // get ids + options for path for each document
  ...
  // TODO
  // handle raw docs returned from queries
  // handle multiple docs passed to model.populate
  // bulk process populated documents efficiently
  // all query population is broken right now due to the change in populateOptions
  // suffix support #991
  // #570

  // TODO this may be subpath notation:
  // comments._creator
  // comments.arrayOfIds
  var val = utils.getValue(path, docs);
  console.log('got vals', path, val);

  // nothing to do if there aren't vals to populate
  if (null == val) {
    return process.nextTick(cb);
  }
  ...
  if (!Array.isArray(val)) {
    match || (match = {});
    match._id = val;
    model.findOne(match, select, options.options, function (err, val) {
      if (err) return cb(err);
      utils.setValue(path, val, docs);
      cb();
    });
  ...
  if (0 === val.length || val.every(utils.nullOrUndefined)) {
    return process.nextTick(cb);
  }

  match || (match = {});
  match._id || (match._id = { $in: val });
  model.find(match, select, options.options, function (err, val) {
    if (err) return cb(err);
    console.log(path, val);
    // TODO, setValue probably won't work here, need to get raw docs?
    // to avoid same refs else just share refs to same docs
    utils.setValue(path, val, docs); // TODO handle more than one doc? (docs)
    cb();
  });
}
