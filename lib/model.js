...
Model.useConnection = function useConnection(conn) {
  if (!conn || conn.base?.version !== this.db.base.version) {
    throw new MongooseError('`useConnection()` requires a Mongoose Connection instance.');
  }
  if (this.db) {
  ...
  this.db = conn;
  const collection = conn.collection(this.collection.collectionName, conn.options);
  this.prototype.collection = collection;
  this.prototype.$collection = collection;
  this.prototype[modelCollectionSymbol] = collection;
  this.prototype.db = conn;
  this.prototype[modelDbSymbol] = conn;
  this.collection = collection;
  this.$__collection = collection;
  conn.models[this.modelName] = this;
