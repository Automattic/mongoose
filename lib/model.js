...
Model.validateFilter = async function validateFilter(conditions, context) {
  ...
  await _validateFilterConditions(this, schema, conditions, context);

  return conditions;
};

async function _validateFilterConditions(model, schema, obj, context) {
  const keys = Object.keys(obj);

  for (const key of keys) {
    if (key === '$or' || key === '$nor' || key === '$and') {
      for (const sub of obj[key]) {
        await _validateFilterConditions(model, schema, sub, context);
      }
      continue;
    }
    ...
    if (utils.isPOJO(val) && !Object.keys(val).some(isOperator)) {
      for (const nestedKey of Object.keys(val)) {
        ...
        await _validateFilterPathValue(model, schema, nestedPath, val[nestedKey], context);
      }
      continue;
    }
    ...
    await _validateFilterPathValue(model, schema, key, val, context);
  }
}

async function _validateFilterPathValue(model, schema, path, val, context) {
      ...
      if (key === '$in' || key === '$nin') {
        ...
        for (const v of arr) {
          if (!(v instanceof RegExp)) {
            await _validateFilterSingleValue(model, schema, path, v, context);
          }
        }
      } else if (key === '$eq' || key === '$ne') {
        if (!(val[key] instanceof RegExp)) {
          await _validateFilterSingleValue(model, schema, path, val[key], context);
        }
      } else if (key === '$not') {
        await _validateFilterPathValue(model, schema, path, val[key], context);
      }
    }
    return;
  }

  await _validateFilterSingleValue(model, schema, path, val, context);
}

async function _validateFilterSingleValue(model, schema, path, value, context) {
  const schemaType = schema.path(path);
  if (schemaType == null) {
    return;
  }
