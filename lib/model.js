...
Model._applyUpdateValidation = function( update, cb ){
  ...
  for( var k in update){
    ...
    console.log("toCycle is: %j" , toCycle );
    ...
    for( var keyToUpdate in toCycle){ 
      ...
      console.log("Turned into: " + cleanKeyToUpdate );
      ...
      for (var i = 0, l = cleanKeyToUpdate.length; i < l; i++) {
        ...
        // element: ignore
          ...
          console.log(" --  It's a mixed schema, but the path goes on: ignore!"); 
          dontCheck = true;
          continue;
        } else {
          console.log(" --  Nothing special, add the next chunk, keep looking"); 
        }
      };
      ...
      console.log("=================================================================" );
      console.log("toCycle is: %j; keytoUpdate is: %j, so currValue is %j", toCycle, keyToUpdate, currValue );
    

      (function(){ // Make a separate context

        console.log("OK, entering recursive exploration");

        // It starts with the current path
        explore( currValue, currSchema, currPath, currPath );

        function explore( obj, schema, path, pathIndex ){

          console.log("Explore entered. Obj: '%j', path: '%s', pathIndex: '%s'" , obj, path, pathIndex);

          var pathType = getPathType( schema.path(path));
          console.log('Switch started, pathType: %j' , pathType.type);

          switch (pathType.type) {

            case 'mixed':
              console.log("It's mixed: run validation in case the mixed element had them, but quit at this point");
              validatePath( schema, path, keyToUpdate + '.' + pathIndex, obj );
              return;
            break;
            
            case 'array':
              console.log("It's an array of basic types (not schemas or mixed). Running validation for every element and quitting");
              if( Array.isArray(obj)){
                for( var k in obj ){
                  console.log('Validating %j', obj[k]);
                  validatePath( schema, path, keyToUpdate + '.' + pathIndex + '.' + k, obj[k] );
                }
              }
              return;
            ...
            case 'external':
              console.log('External schema, exit, won\'t validate as even update will simply fail anyway');
              return;
            break;

            case 'schema':
              console.log("It's a schema: change the current schema to the new one, re-running explore without path");
              explore( obj, pathType.object, '', pathIndex );
            break;
              
            case 'present':
              console.log('Path found: run validation, end of search!');
              validatePath( schema, path, keyToUpdate + '.' + pathIndex, obj );
              return;
            break;
            ...
            case 'notFound':
              ...
              if (Array.isArray(obj) ){
                console.log('Value is an array, will explore each item separately, only changing pathIndex');
                for(var k in obj){
                  var p = pathIndex == '' ? k : pathIndex + '.' + k;
                  explore( obj[k], schema, path, p );
                }
              } else if( typeof(obj) == 'object'){
                console.log('Value is an object, will continue looking for paths');
                for( var k in obj ){
                  var p1 = path == '' ? k : path + '.' + k;
                  var p2 = pathIndex == '' ? k : pathIndex + '.' + k;
                  explore( obj[k], schema, p1, p2 );
                }
              } else {
                console.log('Value is not an object nor an array, and it is not in the schema, giving up');
                return;
              }
              break;
          }
  ...
  function validatePath( schemaType, currPath, keyToUpdate, value ){
    ...
    if (validating[currPath]){ console.log("???????????????????"); return; }
    // validating[currPath] = true;
