...
Model.prototype.getPopulationKeys = function getPopulationKeys (query) {
...
Model.prototype.populate = function populate (schema, oid, fields, fn) {
  if (Array.isArray(oid)) {
    if (oid.length) {
      ...
      model.find({ _id: { $in: oid }}, fields, function (err, docs) {
        if (err) return fn(err);

        // put back in original id order
        oid.forEach(function (id, i) {
          arr[i] = docs.filter(function (doc) {
            return id == doc.id;
          })[0];
        });
...
Model.prototype.init = function init (doc, query, fn) {
  ...
  function init (obj, prefix, fn) {
    ...
    function next () {
      ...
      if (!schema && obj[i] && obj[i].constructor.name == 'Object') {
...
Model.prototype._dirty = function _dirty () {
  var self = this;
...
Model.prototype.registerHooks = function registerHooks () {
