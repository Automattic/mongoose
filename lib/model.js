...
const saveChannel = dc?.tracingChannel('mongoose:save');
...
Model.prototype.save = async function save(options) {
  if (saveChannel?.hasSubscribers) {
    const ctx = {
      operation: this.isNew ? 'insert' : 'update',
      collection: this.constructor?.collection?.name,
      database: this.constructor?.db?.name
    };

    saveChannel.start.publish(ctx);
    try {
      const result = await this.$__saveWrapped(options);
      saveChannel.asyncEnd.publish(ctx);
      return result;
    } catch (err) {
      ctx.error = err;
      saveChannel.error.publish(ctx);
      saveChannel.asyncEnd.publish(ctx);
      throw err;
    }
  }

  return this.$__saveWrapped(options);
};
...
Model.prototype.$__saveWrapped = async function save(options) {
 ...
 * @return {Model}
 * @deprecated Use discriminators or create a new model with a different name instead.
 * @api private
 * @memberOf Model
 * @static
 * @method __subclass
...
Model.__subclass = function subclass(conn, schema, collection) {
  utils.warn('Mongoose: Model.__subclass() is deprecated. Use discriminators or ' +
    'create a new model with a different name instead. ' +
    'See: https://mongoosejs.com/docs/deprecations.html#model-__subclass');
