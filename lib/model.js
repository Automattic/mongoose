...
Model.geoSearch = function (conditions, options, callback) {
  if (conditions == undefined || !utils.isObject(conditions)) {
    throw new Error("Must pass conditions to geoSearch");
  }
  ...
  if (!options.near) {
    throw new Error("Must specify the near option in geoSearch");
  }

  if (!Array.isArray(options.near)) {
    throw new Error("near option must be an array [x, y]");
  }
  ...
  this.collection.geoHaystackSearch(options.near[0], options.near[1], options, callback);
};
