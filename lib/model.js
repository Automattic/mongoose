...
const VersionError = MongooseError.VersionError;
const DivergentArrayError = MongooseError.DivergentArrayError;
...
const tick = utils.tick;

const async = require('async');
const PromiseProvider = require('./promise_provider');

const VERSION_WHERE = 1, VERSION_INC = 2, VERSION_ALL = VERSION_WHERE | VERSION_INC;
...
function handleAtomics(self, where, delta, data, value) {
  ...
  let atomics = value._atomics, ops = Object.keys(atomics), i = ops.length, val, op;
...
Model.prototype.$__delta = function() {
  ...
  let where = {}, delta = {}, len = dirty.length, divergent = [], d = 0;
...
Model.discriminator = function discriminator(name, schema) {
  ...
  const key = this.schema.options.discriminatorKey;
  if (schema.path(key)) {
    throw new Error('Discriminator "' + name +
...
Model.remove = function remove(conditions, callback) {
  ...
  const mq = new Query(conditions, {}, this, this.collection);
...
Model.find = function find(conditions, projection, options, callback) {
  ...
  const mq = new Query({}, {}, this, this.collection);
...
Model.where = function where(path, val) {
  void val; // eslint
  // get the mongodb collection object
  const mq = new Query({}, {}, this, this.collection).find({});
...
Model.create = function create(doc, callback) {
  ...
  const promise = new Promise.ES6(function(resolve, reject) {
    if (args.length === 0) {
      process.nextTick(function() {
        cb && cb(null);
        resolve(null);
...
Model.insertMany = function(arr, callback) {
  ...
  return new Promise.ES6(function(resolve, reject) {
    ...
    async.parallel(toExecute, function(error, docs) {
      ...
      const docObjects = docs.map(function(doc) {
...
Model.populate = function(docs, paths, cb) {
  ...
  // normalized paths
  const noPromise = paths && !!paths.__noPromise;
...
function subPopulate(docs, options, cache, cb) {
  ...
  return function(err) {
    ...
    pop.forEach(function(subOptions) {
      ...
      const schema = cache[prefix.substr(0, prefix.length - 1)] ||
...
function populate(model, docs, options, cb) {
  ...
  let i, len = modelsMap.length, mod, match, select, promise, vals = [];

  promise = new Promise(function(err, vals, options, assignmentOpts) {
    if (err) return cb(err);

    let lean = options.options && options.options.lean, len = vals.length, rawOrder = {}, rawDocs = {}, key, val;
