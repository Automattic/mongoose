...
function populate(model, docs, options, callback) {
  ...
  let originalLocalField = '';
  if (model.schema.virtuals[populateOptions.path]) {
    originalLocalField = model.schema.virtuals[populateOptions.path].options.localField;
  }
  if (populateOptions.localField) {
    model.schema.virtuals[populateOptions.path].options.localField = populateOptions.localField;
  }
  ...
  if (originalLocalField) model.schema.virtuals[populateOptions.path].options.localField = originalLocalField;
  ...
  for (let i = 0; i < len; ++i) {
    ...
    hasOne = true;
    if (populateOptions.foreignField) {
