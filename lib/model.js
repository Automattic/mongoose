...
Model.bulkSave = async function bulkSave(documents, options) {
  ...
  const writeOperations = this.buildBulkWriteOperations(documents, options, documentsToWrite);
  ...
  const failedDocuments = new Set(writeErrors.map(writeError => documentsToWrite[writeError.index]));
  if (options.ordered !== false && writeErrors.length > 0) {
    // Ordered writes stop at the first error, leaving later operations unattempted.
    for (let i = writeErrors[0].index + 1; i < documentsToWrite.length; ++i) {
      failedDocuments.add(documentsToWrite[i]);
    }
  }
  const successfulDocuments = documents.filter(document => !failedDocuments.has(document));
 ...
 * @param {boolean} options.timestamps defaults to `null`, when set to false, mongoose will not add/update timestamps to the documents.
 * @param {Array} [documentsToWrite] If provided, receives the document for each generated operation in order.
 * @return {Promise[]} Returns a array of all Promises the function executes to be awaited.
 * @api private
 */

Model.buildBulkWriteOperations = function buildBulkWriteOperations(documents, options, documentsToWrite) {
