...
// DELETE ME
Model.__applyUpdateValidation = function( update,  enableUpdateValidation, cb ){

  var cleanKeyToUpdate, part;
  var currSchema, currPath='', currValue;
  ...
  if(! enableUpdateValidation && ! require('mongoose').get('enableUpdateValidation')) {
  ...
  for( var k in update){
    ...
    l && console.log("toCycle is: %j" , toCycle );
    ...
    for( var keyToUpdate in toCycle){ 
      ...
      for (var i = 0, ll = cleanKeyToUpdate.length; i < ll; i++) {
        ...
        currPath = currPath === '' ? part : currPath + '.' + part;

        l && console.log("currPath is: " + currPath );

        // console.log(currSchema.paths[part]);
        l && console.log(" -- Part: " + part );
        l && console.log(" -- currPath: " + currPath );
      
        var pathType = getPathType( currSchema.paths[currPath]);
        l && console.log("pathType here is: " + pathType.type );
        // It's a straight schema: reset the currPath, set the new schema
      ...
      var currValue = toCycle[keyToUpdate]; // TODO: Fix for unset
      l && console.log("=================================================================" );
      l && console.log("toCycle is: %j; keytoUpdate is: %j, so currValue is %j", toCycle, keyToUpdate, currValue );
      ...
      (function(){ // Make a separate context
        ...
        explore( currValue, currSchema, currPath, keyToUpdate, toCycle, keyToUpdate  );
  ...
  function getPathType(path){
    ...
    if( ! path ) return { type: 'notFound', object: {} };

    // Mixed (1) -- d: [ ] ,
    if( path.caster && path.caster.casterConstructor && path.caster.casterConstructor.name === 'Mixed' ) return { type: 'mixed', object: {} };

    // Mixed (2) -- e: { type: Schema.Types.Mixed}
    if( path.options && path.options.type.name === 'Mixed' ) return { type: 'mixed', object: {} };

    // Not here (1)  --  d: { type: Schema.Types.ObjectId, ref: 'Two' },
    if( path.options && path.options.ref ) return { type: 'external', object: {} };

    // Not here (2)  --  i: [{ type: Schema.Types.ObjectId, ref: 'Two' }]
    if( path.caster && path.caster.options && path.caster.options.ref ) return { type: 'external', object: {} };

    // A schema of EMBEDDED documents  --  f: [Two] ---- f: [ { a: something, b:else } ]
    if( path.schema ) return { type: 'schema', object: path.schema };

    // Aarray of basic types -- [ { type: String } ]
    if( path.caster ) return { type: 'simpleArray', object: {} };
 ...
 * ####Example:
 *
 *     // Create an empty aggregation pipeline, returns an Aggregate
 *     Users.aggregate();
 *
