  ...
  return this.db.base._promiseOrCallback(callback, cb => {
    ...
    this.createCollection(err => {
      ...
      this.cleanIndexes((err, dropped) => {
        console.log('this is dropped', dropped);
        if (err != null) {
          return cb(err);
        }
      ...
      this.diffIndexes((err, result) => {
        console.log('the result', result.toDrop);
        if (err != null) {
          return cb(err);
        }
        this.wipeIndexes(result.toDrop, err => {
          if (err != null) {
            return cb(err);
          }
          this.createIndexes(options, err => {
...
function indexesToDrop(dbIndexes, schemaIndexes) {
  const toDrop = [];
  ...
  for (const index of dbIndexes) {
    ...
    for (const schemaIndex of schemaIndexes) {
      ...
      const options = decorateDiscriminatorIndexOptions(schemaIndex, utils.clone(schemaIndex[1]));
...
Model.wipeIndexes = function wipeIndexes(destroy, callback) {
  console.log('wipe Indexes', destroy);
  _checkContext(this, 'wipeIndexes');
...
Model.diffIndexes = function diffIndexes(callback) {
  const toCreate = [];
  callback = this.$handleCallbackError(callback);
  return this.db.base._promiseOrCallback(callback, cb => {
    cb = this.$wrapCallback(cb);
    this.listIndexes((err, indexes) => {
      // console.log('indexes in diff', indexes)
      if (indexes === undefined) {
        indexes = [];
      }
      indexes = getRelatedDBIndexes(this, indexes);
      const schemaIndexes = getRelatedSchemaIndexes(this, this.schema.indexes());
      const toDrop = indexesToDrop(indexes, schemaIndexes);
      ...
      for (const schemaIndex of schemaIndexes) {
        let found = false;
        const key = schemaIndex[0];
        const options = decorateDiscriminatorIndexOptions(schemaIndexes, utils.clone(schemaIndex[1]));
