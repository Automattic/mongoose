...
Model._applyUpdateValidation = function( update,  enableUpdateValidation, cb ){
  ...
  for( var k in update){
    ...
    // Sets `op` and `toCycle` to the right values, allowing for
    // the fact that $set might not be there
    // { $set: { name: 'Tony', surname: 'Mobily' }, $inc: { age: 1 }, address: '11 Bitton st' } 
    if( k[0] !== '$' ){
      var toCycle = {}; toCycle[k] = update[ k  ] ;  
      op = '$set';
    } else {
      op = k;
      toCycle = update[k];
    ...
    for( var keyToUpdate in toCycle){ 
      ...
      (function(){ // Make a separate context
        ...
        explore( currValue, currSchema, currPath, keyToUpdate );

        function explore( obj, schema, path, pathIndex ){

          l && console.log("Explore entered. Obj: '%j', path: '%s', pathIndex: '%s'" , obj, path, pathIndex);
          ...
          switch (pathType.type) {
            ...
            case 'schema':
              ...
              explore( obj, pathType.object, '', pathIndex );
            break;
            ...
            case 'notFound':
              ...
              if (Array.isArray(obj) ){
                ...
                for(var k in obj){
                  explore( obj[k], schema, path, joinDot( pathIndex, k)  );
                }
              } else if( typeof(obj) == 'object'){
                l && console.log('Value is an object, will continue looking for paths');
                for( var k in obj ){
                  explore( obj[k], schema, joinDot( path, k ), joinDot( pathIndex, k ) );
                }
              } else {
