...
Model = this.Model = Class({
  ...
  merge: function(doc, _parent){
    ...
    keys.forEach(function(i){
      if (_parent[i] && typeof _parent[i] == 'object' && !(Array.isArray(_parent[i]))) this.merge(doc[i], _parent[i]);
  ...
  save: function(fn){
    var self = this;
    if(this.isNew){
      this._collection.save(this._normalize(), function(err, docs){
        self.isNew = false;
        if(fn) fn(err, docs);
      });
    } else {
      this._collection.update({_id: this._id},{$set: this._getDirty()},function(err, docs){
        self._dirty = {};
        if(fn) fn(err, docs);
      });
    }
  ...
  _compileEtters = function(props, getters, setters, prototype, cast, path){
    for (var i in props){
      ...
      (function(props, getters, setters, p, i){
        if (props[i] === null || Array.isArray(props[i])){
