...
Model.geoNear = function (near, options, callback) {
  ...
  if (!near) {
    return process.nextTick(function(){
      callback(new Error("Must pass a near option to geoNear"))
    })
  }
  ...
  if (Array.isArray(near)) {
    if (near.length != 2) {
      return process.nextTick(function(){
        callback(new Error("If using legacy coordinates, must be an array of size 2 for geoNear"));
      });
    }
    x = near[0];
    y = near[1];
    ...
    if (near.type != "Point" || !Array.isArray(near.coordinates)) {
      return process.nextTick(function(){
        callback(new Error("Must pass either a legacy coordinate array or GeoJSON Point to geoNear"));
      })
    }
