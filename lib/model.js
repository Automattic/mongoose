...
Model.prototype.save = function (fn) {
  ...
  var _promise = this.$__presaveValidate().then(function () {
    var innerPromise = new Promise()
      , options = {}

    innerPromise.onFulfill(function handleSaveFulfill (result) {
      self.$__storeShard();
      ...
      if (self.$__.version && !self.$__.inserting) {
        ...
        if (numAffected > 0) {
          ...
          finalPromise.reject(new VersionError);
          return;
        }
      }

      self.emit('save', self, numAffected);
      finalPromise.fulfill(self, numAffected);
    });

    innerPromise.onReject(function handleSaveReject(err) {
      ...
      finalPromise.error(err);
    });


    if (self.schema.options.safe) {
      options.safe = self.schema.options.safe;
    }

    if (self.isNew) {
      // send entire doc
      var obj = self.toObject({ depopulate: 1 });
      ...
      if (!utils.object.hasOwnProperty(obj || {}, '_id')) {
        ...
        return innerPromise.reject(new Error('document must have an _id before saving'));
      }

      self.$__version(true, obj);
      self.collection.insert(obj, options, function (err, ret) {
      ...
      self.$__.inserting = true;

    } else {
      // Make sure we don't treat it as a new object on error,
      // since it already exists
      self.$__.inserting = false;

      var delta = self.$__delta();

      if (delta) {
        if (delta instanceof Error) return innerPromise.reject(delta);
        var where = self.$__where(delta[0]);
        self.$__reset();
        self.collection.update(where, delta[1], options, function (err, ret) {
          innerPromise.resolve(err, ret);
        });
      } else {
        self.$__reset();
        innerPromise.fulfill(self);
      }
