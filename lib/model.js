...
/**
 * Runs validation before save if `validateBeforeSave` option is enabled.
 * This runs regardless of the `middleware` option to ensure data integrity.
 *
 * @param {Object} options - Save options
 * @param {Boolean} [options.validateBeforeSave] - If set, overrides schema-level validateBeforeSave
 * @param {Boolean} [options.validateModifiedOnly] - If true, only validate modified paths
 * @returns {Promise<void>}
 * @memberOf Model
 * @instance
 * @method $__validateBeforeSave
 * @api private
 */
...
Model.prototype.$__save = async function $__save(options) {
  // Validation runs before middleware, regardless of the middleware option.
  // Use `validateBeforeSave: false` to skip validation.
  if (!this.$isSubdocument) {
    try {
      await this.$__validateBeforeSave(options);
    } catch (error) {
...
Model.insertMany = async function insertMany(arr, options) {
  ...
  if (shouldSkipMiddleware(options, 'pre') === false) {
    try {
      [arr] = await this._middleware.execPre('insertMany', this, [arr]);
    } catch (error) {
      if (shouldSkipMiddleware(options, 'post') === false) {
