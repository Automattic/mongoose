...
Model.create = function create (doc, fn) {
  ...
  retPromise.then(function (docs) {
    // check if 'doc' created is an array
    if (doc instanceof Array) {
      promise.fulfill.call(promise, docs);
    } else {
...
function populate (model, docs, options, cb) {
  ...
  Model.find(match, select, options.options, function (err, vals) {
    ...
    //change the path (reduce by one level) if 'repl' (replace) option is true
    !!replace ? path = path.split('.').slice(0, -1).join('.') : '';
