 ...
 * updateOne
 *
 * Issue a mongodb findAndModify update command.
 *
 * Finds a matching document, updates it according to the
 * `update` arg, passing the `options`, and executing immediately
 * if `callback` is passed. If no `callback` is passed a
 ...
 * Examples:
 *
 *     A.updateOne(conditions, updateObject, options, callback) // executes
 *     A.updateOne(conditions, updateObject, options)  // returns Query
 *     A.updateOne(conditions, updateObject, callback) // executes
 *     A.updateOne(conditions, updateObject)           // returns Query
 *
 * @param {Object} conditions
 * @param {Object} update
...
Model.updateOne = function (conditions, update, options, callback) {
  if ('function' == typeof options) {
    callback = options;
    options = null;
  }
    ...
    if ('function' == typeof conditions) {
      var msg = 'Model.updateOne(): First argument must not be a function.\n\n'
              + '  ' + this.modelName + '.updateOne(conditions, update, options, callback)\n'
              + '  ' + this.modelName + '.updateOne(conditions, update, options)\n'
              + '  ' + this.modelName + '.updateOne(conditions, callback)\n'
              + '  ' + this.modelName + '.updateOne(conditions, update)\n'
              + '  ' + this.modelName + '.updateOne(update)\n'
  ...
  var query =
    new Query(conditions, options)
    .select(fields)
    .bind(this, 'updateOne', update);

  if ('undefined' == typeof callback)
    return query;

  this._applyNamedScope(query);
  return query.updateOne(callback);
}
 ...
 * Examples:
 *
 *     A.removeOne(conditions, options, callback) // executes
 *     A.removeOne(conditions, options)  // return Query
 *     A.removeOne(conditions, callback) // executes
 *     A.removeOne(conditions) // returns Query
 *     A.removeOne()           // returns Query
 *
 * @param {Object} conditions
 * @param {Function} callback
 * @api public
...
Model.removeOne = function (conditions, options, callback) {
  if (1 === arguments.length && 'function' == typeof conditions) {
    var msg = 'Model.removeOne(): First argument must not be a function.\n\n'
              + '  ' + this.modelName + '.removeOne(conditions, callback)\n'
              + '  ' + this.modelName + '.removeOne(conditions)\n'
              + '  ' + this.modelName + '.removeOne()\n';
    throw new TypeError(msg)
  }
  ...
  this._applyNamedScope(query);
  return query.removeOne(callback);
}
