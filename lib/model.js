...
/*!
 * get value of the object either raw object or mongoose
 */

function getValue(path, obj) {
    var isDocument = !! obj.$__;
    if (isDocument) return utils.getValue(path, obj);
    return mpath.get(path, obj);
}

/*!
 * set value of the object either raw object or mongoose
 */

function setValue(path, val, obj) {
    var isDocument = !! obj.$__;
    if (isDocument) return utils.setValue(path, val, obj);
    return mpath.set(path, val, obj);
}
...
function populate (model, docs, options, cb) {
  ...
  // get refPk
  var refPk = (schema && schema.options.refPk) || '_id';
  var sourcePk = (schema && schema.options.sourcePk) || '_id';
  ...
  for (i = 0; i < len; i++) {
    ...
    if (!ret || Array.isArray(ret) && 0 === ret.length) {
      ret = getValue(path, doc);
    }

    if (ret) {
      options._docs[id] = Array.isArray(ret) ? ret.slice() : ret;;
      //только те объекты по которым есть что-то
      docOrder[id] = found++;
      docIdOrder[id] = i;
      rawIds.push(ret);
    }
  ...
  // fix to provide way of loading keys not using keys

  match[refPk] || (match[refPk] = { $in: ids });
  //
  var assignmentOpts = {};
  assignmentOpts.sort = options.options && options.options.sort || undefined;
  assignmentOpts.excludeId = new RegExp('\s?-' + refPk + '\s?').test(select) || (select && 0 === select[refPk]);
  assignmentOpts.refPk = refPk;
  ...
  if (assignmentOpts.excludeId) {
    ...
    if ('string' == typeof select) {
      ...
      delete select[refPk];// setValue to undefined or Delete.
    }
  }

  Model.find(match, select, options.options, function (err, vals) {
    if (err) return cb(err);
    var refPk = assignmentOpts.refPk;
    var rawOrder = {};
    var i, val, rawDocs = {}, key;
    var len = vals.length;
    for (i = 0; i < len; i++) {
      val = vals[i];
      key = getValue(refPk, val);
      rawDocs[key] = vals[i];
      rawOrder[key] = i;
    }
...
function assignVals (opts) {
  ...
  var docIdOrder = opts.docIdOrder;
  var docOrder = opts.docOrder;
  var rawOrder = opts.rawOrder;
  var docs = opts.docs;
  var path = opts.path;
  var options = opts.options;

  assignRawDocsToIdStructure(rawIds, rawDocs, rawOrder, options);

  var rawIdsKeys = Object.keys(docOrder);
  var rawPos, docPos;
  for (var i = rawIdsKeys.length - 1; i >= 0; i--) {
    rawPos = docOrder[rawIdsKeys[i]];
    docPos = docIdOrder[rawIdsKeys[i]];
    utils.setValue(path, rawIds[rawPos], docs[docPos], function(val) {
...
function assignRawDocsToIdStructure (rawIds, vals, rawOrder, options, recursed) {
  ...
  for (var i = 0; i < rawIds.length; ++i) {
    ...
    if (Array.isArray(id)) {
      // handle [ [id0, id2], [id3] ]
      assignRawDocsToIdStructure(id, vals, rawOrder, options, true);
      newOrder.push(id);
      continue;
    }
