...
Model._applyValidation = function( update, cb ){
  ...
  for( var matchName in update){

    // Clean up position indexes ('some.4.thing' or 'some.$.thing')
    parts = matchName.replace(/(\.[0-9]+\.)|(\.\$\.)/, '.').split('.');
 ...
 if( currSchema.path(currPath )){
      handled = true;
      validatePath( currSchema, currPath, matchName, update[matchName] );
    }
  }
