...
Model.prototype._populate = function populate (schema, oid, query, fn) {
  ...
  function ensureKeyExists(){
    if(!useId && query.fields){ 
      if(Array.isArray(query.fields))
        query.fields.push(key)
      else if("string"== typeof query.fields){
        query.fields = query.fields.split(' ').concat(key);
      } else if('object' == typeof query.fields){
        //determine whether use include or exlude
        var _fields = query.fields
        for (var prop in _fields){
          if(_fields[prop]) _fields[key] = 1 // add to include
          else delete _fields[key]; // remove from excluded
        }
      }
    }
  }
  ...
  if (!Array.isArray(oid)) {
    var key = schema.options.refPk || ( useId = true, "_id");

    ensureKeyExists();

    var conditions = query.conditions ? utils.clone(query.conditions) : {};
    if(!conditions[key])
      conditions[key] = oid;
    else // condition already defined
      if(conditions[key] != oid) 
  ...
  var key = schema.caster.options.refPk || ( useId = true, "_id");

  ensureKeyExists();

  var model = this.model(schema.caster.options.ref)
    , conditions = query && query.conditions || {};
  conditions[key] || (conditions[key] = { $in: oid });
  ...
  model.find(conditions, query.fields, query.options, function (err, docs) {
    ...
    var arr = [];
    oid.forEach(function (_key) {
      if (_key in docHash) arr.push(docHash[_key]);
    });
