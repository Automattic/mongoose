...
Model.prototype.save = function save (fn) {
  ...
  if (this.isNew) {
    ...
    if (delta) {
      // handle weird -native MongooseNumber serialization issues
      // since >= 0.9.7-3-5
      var id = this._doc._id
        , _id = id.valueOf ? id.valueOf() : id;
      this.collection.update({ _id: _id }, delta, options, complete);
    } else {
      complete(null);
    }
  }
