...
Model = this.Model = Class({
  ...
  _cast: function(type,val){
    switch(type){
      ...
      case Number: 
        ...
        break;
      case ObjectID:
        val = (val instanceof ObjectID) ? val : ObjectID.createFromHexString(val);
        break;
  ...
  save: function(fn){
    ...
    for(var i=0,l=this._arrays.length; i < l; i++){
      if(this._casters[this._arrays[i]]){
        cast = this._casters[this._arrays[i]];
