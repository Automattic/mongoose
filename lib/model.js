...
var Class = require('./support/class/lib/class').Class,
 //   Sugar = require('./sugar').Sugar,
    ...
    Manager = new Class({
        ...
        compile : function(id,doc,store,name){
        
            var schema = doc.prototype.schema,
                collectionName = doc.prototype.name || name,
                static = {
            ...
            if(store.collections[collectionName]) doc.collection = store.collections[collectionName];
            else{
              store.collection(collectionName,function(aCollection){
                doc.collection = aCollection;
   // for now             collection.createIndex(doc.prototype.indexes,function(err,resp){}); // ensures indexes
                doc.emit('onLoad',aCollection);
              })
            }
