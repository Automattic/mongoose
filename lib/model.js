...
Model._applyUpdateValidation = function( update, cb ){
    ...
    for( var keyToUpdate in toCycle){ 
      ...
      (function(){ // Make a separate context
        ...
        function explore( obj, schema, path ){
          ...
          // if Path is empty: go through the object considering
          // each key as a path. Seems awkward to have this here,
          // but this is how this function could *start*
          if( path == '' ){

            console.log('Path is empty, calling itself for each key');

            for( var k in obj ){
              explore( obj[k], schema, k );
            }

          // If path is NOT empty, get the type: 
          // * If it's a "baddy", give up. 
          // * If it's a schema, set it as schema and re-start exploring. 
          // * If it's nothing, keep looking appending the extra path
          } else {

            var pathType = getPathType( schema.path(path));
            console.log('Path is not empty, switch started, pathType: %j' , pathType);

            switch (pathType.type) {
              case 'mixed':
              case 'external':
                console.log('External schema, exit');
                return;
              break;
              ...
              case 'present':
                console.log('Path found: run validation');
                validatePath( schema, path, keyToUpdate + '.' + path, obj );
              break;

              case 'notFound':
                console.log('Path not found, calling itself for each key');
                for( var k in obj ){
                  explore( obj[k], schema, path + '.' + k );
                }
  ...
  function validatePath( schemaType, currPath, keyToUpdate, value ){
    ...
    if (validating[currPath]) return;
    validating[currPath] = true;
    total++;

    process.nextTick(function(){
      schemaType.paths[currPath].doValidate(value, function (err) {
        if( err ){
          errorsFound = true;
