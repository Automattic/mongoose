...
var object = require('../utils/object'),
      ...
      for(prop in type){
        ...
        if(isObject(type[prop])){
          ...
          to.__defineGetter__(prop,(function(){
            var s = store, key = prop, get = getter;
            return function(){
              return (function(){
                if(get instanceof Function) return get.call(scope,s[key]);
                else return s[key];
              }).call(scope);
            }
          })());
          ...
          to.__defineSetter__(prop,(function(){
            var s = store, key = prop, t = type, set = setter;
            return function(val){
              return (function(){
                  if(set instanceof Function) s[key] = set.call(scope,val);
                  else s[key] = val;
      ...
      for(prop in getters){
        if(!obj[prop]){
          if(!isObject(getters[prop])){
            var func = (function(){
              var g = getters[prop], s = scope;
              return function(){ return g.apply(s,arguments); };
            })();
            if(!isSet) addGetter(obj,prop,func);
            else addSetter(obj,prop,func);
          } else {
    ...
    Model.prototype = {
      ...
      compile : function(name,store){
        ...
        model = (function(){
          return function(obj){
            if(!(obj._id instanceof ObjectID)) obj._id = new ObjectID();
        ...
        addMethods(model.prototype,{ 
          ...
          save : function(callback){
            ...
            if(meta.saving){
             ...
             model.save(this.__meta__.doc,function(){
               meta.saving = false;
