    ...
    Model.prototype = {
      ...
      compile : function(name,store){
        ...
        addMethods(model.prototype,{ 
          ...
          save : function(){ 
            var self = this;
            if(this.__meta__.saving == true){
              this.__meta__.saveAgain = true;
            } else {
             model.save(object.clone(this)).run(function(doc){ 
                if(!self._id) self._id = doc._id;
                self.__meta__.saving = false;
                if(self.__meta__.saveAgain){
                  self.__meta__.saveAgain = false;
                  self.save();
                } 
              }); 
            }
        ...
        if(!model.__meta__.type._id) model.__meta__.type._id = Object;
