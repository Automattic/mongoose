var QueryPromise = require('./queryPromise').QueryPromise,
  ...
  QueryWriter.prototype = {
        ...
        this.callback = function(err,cursor){
          if(err) self.promise.error(err);
            ...
            if(err) self.promise.error(err);
              ...
              if(doc != null) callback.call(self.promise, ((hydrate) ? new model(doc) : doc));
    ...
    toArray : function(callback,hydrate){
      var model = this.model, self = this, arr = [];
      this.callback = function(err,cursor){
        if(err) self.promise.error(err);
        else cursor.each(function(err,doc){
          if(err) self.promise.error(err);
          else{
            if(doc != null) arr.push((hydrate) ? new model(doc) : doc);
            else {
              callback.call(self.promise, arr);
    ...
    get : function(callback,hydrate){ // action
        var model = this.model, self = this, fn = callback;
        this.callback = function(err,cursor){ 
          if(err) self.promise.error(err);
          else cursor.nextObject(function(err,doc){
            if(err) self.promise.error(err);
            else if(doc != null) fn.call(self.promise, ((hydrate) ? new model(doc) : doc) );
            else fn.call(self.promise, null);
            self.promise.complete();
          });
        };
        return this.process();
  ...
  QueryWriter.prototype.has = function(modifier){
    if(!this.args.length) this.args.push({});
    for(i in modifier){
      if(Object.prototype.toString.call(this.args[0][i]) != '[object Object]') this.args[0][i] = {};
      this.args[0][i]['$in'] = modifier[i];
    }
    return this;
  };
  
  ['nin','ne','gt','gte','lt','lte',
  ...
  QueryWriter.prototype.all = QueryWriter.prototype.toArray;
