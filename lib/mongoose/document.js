    ...
    if(typeof prop == 'string'){
      ...
      if(obj.hasOwnProperty(prop)){
        ...
        if(schema.paths[curpath].type == 'array'){
          if(override){
            this.set(curpath, new EmbeddedArray(obj[prop], curpath, schema.paths[curpath], this, !override));
          } else {
            val[prop] = new EmbeddedArray(obj[prop], curpath, schema.paths[curpath], this, override);
...
Document.prototype._validate = function(override, fn, args){
  ...
  for(i=0, l=dirty.length; i<l; i++){
    ...
    validators = (typeof def.validators == 'object') ? Object.keys(def.validators) : [];
...
Document.prototype.get = function(path){
  ...
  if(type.type == 'object'){
    // weird need to return the getter
...
var EmbeddedArray = function(arr, path, schema, scope, hydrate){
  ...
  Array.prototype.push.call(this); // hack to initialize 'length' so we can hide it
...
EmbeddedArray.prototype = {
  ...
  set: function(idx, val){
    this[idx] = val;
    this._.scope._.dirty[this._.path+'.'+idx] = true;
  },
