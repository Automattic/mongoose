...
module.exports = {
  ...
  Statics: {
    ...
    find: function(where, hydrate){
      ...
      var self = this
        , where = where || {}
        , casters = this.prototype._casters = this.prototype._casters || {}
        , cast = this.prototype._cast;
      ...
      for (var i in where){
         _writer.where(i, ((!casters[i]) ? where[i] :
          (!Array.isArray(where[i]) && typeof where[i] == 'object') ?
          castsub(i, where[i]) : cast(casters[i], where[i])));
      }
