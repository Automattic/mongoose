...
var Document = this.Base = function(obj,isOld){
  ...
  this._hydrated = {};
  this.isNew = !isOld;
  this._compile(this._schema.paths,)
  this.hydrate(obj);
};
...
Document.prototype._hydrate = function(struct,obj,val,path){
  ...
  for(i=0,l=struct.length; i<l; i++){
    ...
    if(typeof prop == 'string'){
      ...
      if(schema.paths[curpath].type != 'virtual'){
        count++;
        if(obj.hasOwnProperty(prop)){
          hydrated++;
          if(schema.paths[curpath].type == 'array'){
            val[prop] = new EmbeddedArray();
          } else {
            val[prop] = this.set(curpath,obj[prop]); 
          }
      ...
      (Object.prototype.toString.call(obj[prop]) == '[object Object]')){
        hydrated++;
        val[prop] = {};
        var children = this._compile(struct[i][1], obj[prop], val[prop], p.concat(prop));
        count += children[0];
        hydrated += children[1];
      }
    }
  ...
  if(hydrated == count)
    this._hydrated[ (p.length) ? p.join('.') : ''] = true;
...
defineMethod(Model, 'hydrate', function(obj){
  this._hydrate(this._struct,obj,this.__doc);
  return this;
});
