...
Document.prototype._setData = function(struct, obj, val, path, override){
  ...
  for(i=0,l=struct.length; i<l; i++, count++){
    ...
    if (typeof prop === "string") {
      this.set(prop, obj[prop], override);
    } else {
      prop = struct[i][0];
      this._setData(struct[i][1], obj[prop], val[prop], p.concat(prop), override);
    }


    if(typeof prop == 'string'){
      curpath = p.concat(prop).join('.');
      if(obj.hasOwnProperty(prop)){ // If the registered attribute is one of the attributes we want to change
        flag++;
...
Document.prototype.set = function (path, val, override) {
  ...
  for(i=type.setters.length-1; i>=0; i--) val = type.setters[i].apply(this,[val,path,type]);

  if(type.type == 'object'){
    for(prop in val) this.set(path+'.'+prop,val[prop]);
  } else if(type.type == 'array'){
    this._.arrays[path] = new EmbeddedArray(val, path, type, this, !override);
    doc[path] = this._.arrays[path].arr;
    if (override) {
      this._.dirty[path] = [];
    } else {
      this._.hydrated[path] = true;
      this.emit('hydrate', [curpath, doc[path]]);
    }
    ...
    // type.setters are the setters we defined via set() in the type definition
    for(i=type.setters.length-1; i>=0; i--) val = type.setters[i].apply(this,[val,path,type]);
