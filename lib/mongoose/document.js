  ...
  Statics: {
    ...
    drop: function(fn){
      ...
      this._collection.drop(function(err, ok){
        if (err) {
          throw err;
          return self._connection._error(err); // TODO _connection has no method _error
        }
        fn(ok);
      });
    }
  ...
  compileEtters: function(struct,prototype,path,scope){
    ...
    for(var i=0,l=struct.length; i<l; i++){
      ...
      if(typeof prop == 'string'){
        ...
        prop = struct[i][0],
        curpath = p.concat(prop).join('.');
        // Q: Why no setter here?
        // A (nwhite): there is no setter its recursive... for an object. so we create a new object to prototype then call compileEtters again
        Object.defineProperty(prototype, prop, {
