...
defineMethod(Document, '_setData', function(struct, obj, val, path, override){
  ...
  for(i=0,l=struct.length; i<l; i++){
    ...
    if(typeof prop == 'string'){
      ...
      if(obj.hasOwnProperty(prop)){
        ...
        if(!override) this._.hydrated[curpath] = true;
      }
    } else {
      prop = struct[i][0];
      if(obj.hasOwnProperty(prop) &&
...
defineMethod(Document, '_run', function(name, fn, args){
...
defineMethod(Document, '_get', function(path){
...
defineMethod(Document, 'get', function(path){
...
defineMethod(Document, '_set', function(path,val){
  var parts = path.split('.'), doc = this._.doc;
...
defineMethod(Document, 'set', function(path,val){
...
function inherits (ctor, superCtor) {
    Object.defineProperty(ctor,'super_', { value: superCtor, enumerable: false });
