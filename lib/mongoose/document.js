...
var EmbeddedArray = function(arr, path, schema, scope, hydrate){
  ...
  if(schema.options instanceof Array){
    ...
    module.exports.defineMethod(model, '_getters', {});

    for(prop in schema._hooks) module.exports.defineHook(model, prop, schema._hooks[prop]);
    for(prop in schema._methods) module.exports.defineMethod(model, prop, schema._methods[prop]);
    module.exports.compileEtters(schema._struct, model.prototype, model.prototype);
...
EmbeddedArray.prototype.__proto__ = [];
...
EmbeddedArray.prototype.set = function(idx, val){
    ...
    this.scope._.dirty[this.path+'.'+idx] = true;
};
