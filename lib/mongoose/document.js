  ...
  for(i=0,l=struct.length; i<l; i++){
    ...
    if(typeof prop == 'string'){
      ...
      if(obj.hasOwnProperty(prop)){
        hydrated++;
        if(schema.paths[curpath].type == 'array') val[prop] = new EmbeddedArray(obj[prop]);
        else val[prop] = obj[prop]; 
      ...
      (Object.prototype.toString.call(obj[prop]) == '[object Object]')){
        hydrated++;
        val[prop] = {};
        var children = this._hydrate(struct[i][1], obj[prop], val[prop], p.concat(prop));
        count += children[0];
        hydrated += children[1];
      }
    }
...
Document.prototype._mixin = function(struct, obj, path){
  var p = path || [], count = 0, dirty = 0, i, l, prop, schema = this._schema, curpath;
  for(i=0,l=struct.length; i<l; i++){
    prop = struct[i];
    count++;
    if(typeof prop == 'string'){
      ...
      if(obj.hasOwnProperty(prop)){
        dirty++;
        if(schema.paths[curpath].type == 'array') this.set(curpath, new EmbeddedArray(obj[prop]));
        else this.set(curpath, obj[prop]); 
        this._dirty[curpath] = true;
      }
    } else {
      prop = struct[i][0];
      if(obj.hasOwnProperty(prop) &&
      (Object.prototype.toString.call(obj[prop]) == '[object Object]')){
        dirty++;
        if(this.get(curpath) == undefined) this._set(curpath, {});
        var children = this._mixin(struct[i][1], obj[prop], p.concat(prop));
        count += children[0];
        dirty += children[1];
      }
    }
  }
  this._dirty[ (p.length) ? p.join('.') : ''] = true; 
  return [count, dirty];
}

Document.prototype.mixin = function(obj){
  this._mixin(this._schema._struct, obj || {});
  return this;
}
...
defineMethod(Document, 'hydrate', function(obj){
  this._hydrate(this._schema._struct, obj || {}, this.__doc);
  return this;
});
