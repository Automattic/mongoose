...
Document.prototype._setData = function(struct, obj, val, path, override){
  var p = path || [], count = 0, flag = 0, i, l, prop, schema = this._schema, curpath;
  for(i=0,l=struct.length; i<l; i++){
    ...
    if(typeof prop == 'string'){
      ...
      if(obj.hasOwnProperty(prop)){
        ...
        this[override ? '_dirty' : '_hydrated'][curpath] = true;
      }
    } else {
      prop = struct[i][0];
      if(obj.hasOwnProperty(prop) &&
  ...
  if(flag == count) this._hydrated[ (p.length) ? p.join('.') : ''] = true; 
  return [count, flag];
};
