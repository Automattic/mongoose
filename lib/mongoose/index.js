 ...
 * 2. Adds a reference to the schema in this._models
 * 3. Adds a shortcut getter, name, that delegates to this.connection
...
//        type.prototype = definition;
     ...
     * @param {String} subtype is the name of the subtype
     */
    Schema.prototype[name] = function (key, subtype){
      ...
      var path = this.getPath([key]),  i, l = this.paths[key].setups.length;
Schema
      ...
      for(i=0; i<l; i++) this.paths[key].setups[i].apply(this,[key, path]);
      return this;
    };
...
Connection.prototype._compile = function(name){
  ...
  for(prop in schema._statics) model[prop] = schema._statics[prop];
   Doc.compileEtters(schema._struct, model.prototype, model.prototype);
  return model;
};
