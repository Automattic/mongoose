...
var _compileEtters = function(struct,prototype,model,path,scope){
        ...
        get: (function(path,p,struct,scp){
          return function(){
            ...
            if(!(path in scope._getters)){
              var nested = function(){};
               _compileEtters(struct, nested.prototype, model, p, scope);
...
Connection.prototype._compile = function(name){
  ...
  Doc.inherits(model, schema._model);
  ...
  for(prop in schema._statics) model[prop] = schema._statics[prop];
   _compileEtters(schema._struct, model.prototype, model.prototype);
  return model;
};
