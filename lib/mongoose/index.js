  ...
  if(name && this._types[name]) return this._types[name];
    ...
    Schema.prototype[name] = function (key, subtype){
      ...
      if (typeof subtype === "string") {
        subtype = mongoose._models[subtype] || mongoose._types[subtype];
      }
      ...
      if (subtype instanceof Schema) {
        if(name == 'array'){
          subtype._host = this; // TODO When is _host ever used?
        ...
        if(name == 'array' && (typeof subtype == 'string' || subtype instanceof TypeSchema)) {
        }
//        if(name == 'array' && (typeof subtype == 'string' || subtype != undefined)) this.extend(subtype);
        this._struct.push(key);
      }
      var setups = this.paths[key].setups;
      for (var i=0, l=setups.length; i<l; i++) setups[i].apply(this,[key, path]);
      return this;
    };
...
Mongoose.prototype.documentation = function(name, path, callback){
  var model = this._models[name];
  if(!model) return false;
  Documentation.create(model, path, callback);
};
