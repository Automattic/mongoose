...
Model.update = function (conditions, doc, options, callback) {
  ...
  // dont want to allow overwritting entire docs be default
  // set is the default
  // default values should be set
  // if options.upsert is true return the _id (minor)
  // allow queries without pathkeys in root
  //   { $set: { name: 'bob' }}
  //   { $inc: { count: 1 }}
  //   { $pull: {}}
  //   { $pullAll: {}}

  // User.update({ _id: id }, { x: 1, $pull: {y: 'u' } })
