...
Model.prototype.save = function (fn) {
  if (this.isNew) {
    ...
    this.activePaths.map('modify', function (path) {
...
Model.castQuery = function (query) {
  ...
  for (var i in query){
    ...
    else if (query[i].constructor == Object || query[i].constructor == RegExp || Array.isArray(query[i]))
      ret[i] = query[i];
    else
...
Model.find = function (query, fields, options, callback) {
  ...
  return this.findQuery(query, fields, options, callback, function (query) {
    ...
    self.collection.find(casted, this.fields, function (err, docs) {
