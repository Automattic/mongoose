...
Model.find = function (query, fields, options, callback) {
  ...
  return this.findQuery(query, fields, callback, options, function (query) {
    ...
    self.collection.find(casted, query.fields, function (err, docs) {
      if (err) return query.queryComplete(err);
      ...
      for (var i = 0, l = docs.length; i < l; i++){
        ...
        arr[i].init(docs[i], function () {
          --count || query.queryComplete(null, arr);
        });
      }
    });
  });
...
Model.findOne = function (query, fields, options, callback) {
  ...
  return this.findQuery(query, fields, options, callback, function (query) {
    var casted = self.castQuery(query.query);
    self.collection.findOne(casted, query.fields, function (err, doc) {
      if (err) return query.queryComplete(err);
      
      var casted = new self();
      casted.init(doc, function () {
        query.queryComplete(null, casted);
      });
    });
  });
};
...
Model.count = function (query, callback) {
  ...
  return this.query(query, {}, callback, function (query) {
    var casted = self.castQuery(query.query);
    self.collection.count(casted, query.queryComplete);
  });
};
...
Model.update = function (query, doc, options, callback) {
  ...
  return this.query(query, callback, options, function (query) {
    ...
    self.collection.update(castQuery, castDoc, query.options, query.queryComplete);
  });
};
