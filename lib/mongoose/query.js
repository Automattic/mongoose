...
Query.prototype.exec = function(fn, hydrate){
  ...
  util.walkObject(schema, query, function(path, fullpatharr, prop, val){
    var tmpVal
      , toAssign
      , type = schema.paths[path]
      , setters;
    if (typeof type._castSet === "function") {
      ...
      for (var i = setters.length-1; i >= 0; i--) {
        try {
          ...
          var ret = setters[i](val,path,type);
          // Coercion failure
          if (Error == ret) {
      ...
      tmpVal = model._schema.paths[path]._castSet(tmpVal);
      toAssign[fullpatharr[l]] = tmpVal;
    }
  });
