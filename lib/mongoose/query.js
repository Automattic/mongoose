...
Query.prototype.update = function (doc, callback) {
  ...
  // this only works if everything is $set only
  // we don't want this anymore. we want doc to
  // end up just like the result of a save() call
  // with what is sent to the db
  try {
    this.cast(model, doc);
  } catch (err) {
    return callback(err);
