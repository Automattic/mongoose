...
function NamedScope (name, block) {
  this.name = name;
  this.block = block;
};
...
NamedScope.prototype.decorate = function (model) {
  var name = this.name
    , block = this.block
    , query = this.query;
  if (block) {
    if (block.length === 0) {
      Object.defineProperty(model, name, {
        get: function () {
          return block.call(query);
        }
      });
    } else {
      model[name] = function () {
        return block.apply(query, arguments);
      };
    }
  } else {
    Object.defineProperty(model, name, {
...
Schema.prototype.namedScope = function (name, fn) {
  var namedScopes = this.namedScopes || (this.namedScopes = {});
  return namedScopes[name] = new NamedScope(name, fn);
};
