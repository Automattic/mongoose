...
Schema.prototype._compile = function(path){
  for(p in this.paths){
    var path = (path) ? p : path + '.' + p,
        opts = this.paths[p].options;
    if(opts instanceof Schema){
      opts._compile(path);
      for(embedPath in opts.paths)
        this.paths[path+'.'+embedPath] = opts.paths[embedPath];
    }
  }
  return this;
};
...
Schema.prototype.methods = function(obj){
  for(method in obj){
    this._overrides[method] = obj[method];
  }
  return this;
};
