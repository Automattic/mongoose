...
var walkObject = exports.walkObject = function(schema, obj, fn, path, fullpatharr) {
  ...
  for (var key in obj) if (obj.hasOwnProperty(key)) {
    val = obj[key];
    fullpatharr = fullpatharr.concat(key);
    if (/^\$/.test(key)) { // Skip mongodb "reserved" keys
      curpath = path;
    } else {
      curpath = path.concat(key);
    }
    if ('[object Object]' == val.toString()) {
      walkObject(schema, val, fn, curpath, fullpatharr);
    } else {
//      console.log(schema.paths);
      fn(curpath.join('.'), fullpatharr, schema.paths[curpath.join('.')], val);
    }
  }
};
