...
var walkObject = exports.walkObject = function(schema, obj, fn, path) {
  var path = path || []
  ...
  for (var i = 0, len = keys.length; i < len; ++i) {
    ...
    if ('[object Object]' == toString.call(val)) {
      walkObject(schema, val, fn, path);
    } else {
      curpath = path.concat(key).join('.');
      console.log(schema.paths);
      fn(curpath, schema.paths[curpath], val);
    }
  }
};
