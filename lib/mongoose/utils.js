...
/**
 * Copies static methods (eg: `find`) from one constructor to the other
 *
 * @param {Function} from
 * @param {Function} to
 * @api private
...
exports.copyStatics = function(ctorFrom, ctorTo){
  for (var i in ctorFrom){
    if (ctorFrom.__lookupGetter__(i))
      ctorTo.__defineGetter__(i, ctorFrom.__lookupGetter__(i));
    else if (typeof ctorFrom[i] == 'function')
      ctorTo[i] = ctorFrom[i];
  }
};
