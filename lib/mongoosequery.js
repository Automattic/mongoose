 ...
 * ####Example:
 *
 *     var query = new MongooseQuery(model.collection);
 *     query.setOptions({ lean : true});
 *     query.where('age').gte(21).exec(callback);
 *
 * @param {Object} Mongoose collection Object
...
function MongooseQuery(collection, options, model, conditions) {
  ...
  this.mongooseCollection.conn.on('open', function() {
    ...
    // we now have version information for the mongod instance. Set the within
    // parameters appropriately
    if (self.mongooseCollection.conn.__mongodVersion &&
      self.mongooseCollection.conn.__mongodVersion[0] >= 2 &&
      self.mongooseCollection.conn.__mongodVersion[1] >= 4) {
      mquery.use$geoWithin = true;
    }
    // this means that the use$geoWithin setting has been checked, used to
    // determine if we need to walk the conditions and check for within() calls
    // that came prior to connecting
    mquery.$geoWithinSet = true;
...
MongooseQuery.prototype.within = function () {
  if (!mquery.$geoWithinSet) {
    this.__checkWithin = true;
  }
  return MongooseQuery.base.within.apply(this, arguments);
}
  ...
  if (!callback) {
    ...
    if (!this._collection) {
      this.events.once('connected', function(self) {
        if (self.__checkWithin) {
          // a within call came before version was checked, validate the query
          var keys = Object.keys(self._conditions);
          var l = keys.length;
          for (var i=0; i < l; i++) {
            if (self._conditions[keys[i]].$within) {
              self._conditions[keys[i]].$geoWithin = self._conditions[keys[i]].$within;
              delete self._conditions[keys[i]]["$within"];
            }
          }
          self.__checkWithin = false;
        }
        self.find({}, callback);
      });
      return this;
...
/**
 * legacy mongoose support. Basically just syntax sugar
 */
...
/**
 * Just exposes the use$geoWithin for the testing rig
 */

MongooseQuery.use$geoWithin = function () {
  return mquery.use$geoWithin;
}
