...
function saveSubdocsPostSave(options) {
  if (this.$isSubdocument) {
    return;
  }
...
function saveSubdocsPostDeleteOne(options) {
  const removedSubdocs = this.$__.removedSubdocs;
  if (!removedSubdocs?.length) {
    return;
  }
...
saveSubdocsPostSave[symbols.middlewareOptions] = true;
saveSubdocsPostDeleteOne[symbols.middlewareOptions] = true;
