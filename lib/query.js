...
Query.prototype.update = function update (doc, callback) {
  ...
  // Note: this would ideally go higher up in the function. However,
  // there are things I don't quite understand above (e.g. the
  // throw of casteDoc which happens AS WELL as callback(err) (?!?).
  // 
  // So I am placing it here, since it's a safe spot to run tests.
  this.model._applyUpdateValidation(this._updateArg, function(err) {
    console.log("Validation issues FROM QUERY (UPDATE)! ============================");
    console.log(err);
...
Query.prototype._findAndModify = function (type, callback) {
  ...
  this.model._applyUpdateValidation(this._updateArg, function(err) {
    console.log("Validation issues FROM QUERY (FINDONEANDUPDATE)! ============================");
    console.log(err);

    if( err ){
      console.log("I AM HEREEEEEEEEEEEEEE");
      process.nextTick(promise.error.bind(promise, err));
    } else {
    // SORRY, UNINTENDED ORIGINAL CODE BELOW. WILL INDENT WHEN POSITION IS FINAL
