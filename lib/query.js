...
Query.prototype.countDocuments = function(conditions, options) {
  ...
  if (canMerge(conditions)) {
    ...
    this.error(
      new ObjectParameterError(conditions, 'filter', 'countDocuments')
    );
  }
...
function _completeOneLean(schema, doc, path, res, opts) {
  if (opts.lean?.virtuals) {
    applyVirtuals(schema, doc, opts.lean.virtuals, {
...
function _completeManyLean(schema, docs, path, opts) {
  if (opts.lean?.virtuals) {
    applyVirtuals(schema, docs, opts.lean.virtuals, {
      skipPopulateVirtuals: true
    });
  }
...
function _update(query, op, filter, doc, options) {
  ...
  if (filter instanceof Query ||
      (filter != null && filter.toString() !== '[object Object]')) {
