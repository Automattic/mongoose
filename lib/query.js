...
async function _executePostHooks(query, res, error, op) {
  ...
  const filter = shouldRunMiddleware(query.options, 'post') ? null : isBuiltInMiddleware;
  const opts = error ? { error, filter } : { filter };
...
function _executePreHooks(query, op) {
  ...
  const filter = shouldRunMiddleware(query.options, 'pre') ? null : isBuiltInMiddleware;
