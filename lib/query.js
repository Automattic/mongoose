...
Query.prototype._optionsForExec = function(model) {
  const options = Query.base._optionsForExec.call(this);
...
function completeOne(model, doc, res, fields, self, pop, callback) {
  ...
  const casted = helpers.createModel(model, doc, fields);
...
Query.prototype._walkUpdatePath = function _walkUpdatePath(obj, op, pref) {
  let prefix = pref ? pref + '.' : '', keys = Object.keys(obj), i = keys.length, hasKeys = false, schema, key, val;

  const useNestedStrict = this.schema.options.useNestedStrict;
  ...
  while (i--) {
    ...
    if (val && val.constructor.name === 'Object') {
      ...
      if (schema && schema.caster && op in castOps) {
        ...
        const v = this.model.schema._getPathType(pathToCheck);
        const _strict = 'strict' in this._mongooseOptions ?
      ...
      const checkPath = (key === '$each' || key === '$or' || key === '$and') ?
        pref : prefix + key;
      schema = this._getSchema(checkPath);

      const pathDetails = this.model.schema._getPathType(checkPath);
      const isStrict = 'strict' in this._mongooseOptions ?
        this._mongooseOptions.strict :
...
Query.prototype._applyPaths = function applyPaths() {
  ...
  let fields = this._fields, exclude, keys, ki;
  ...
  const analyzeSchema = function(schema, prefix) {
    prefix || (prefix = '');
