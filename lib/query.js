...
Query.prototype.clone = function() {
  ...
  q._mongooseOptions = clone(this._mongooseOptions);
  // clone() retains class instances, so populate options need their own copies.
  const populate = q._mongooseOptions.populate;
  if (populate != null) {
    for (const path of Object.keys(populate)) {
      populate[path] = _clonePopulateOptions(populate[path]);
    }
  }
...
Query.prototype._findOneAndReplace = async function _findOneAndReplace() {
  ...
  const modelOpts = { skipId: true, middleware: this.options.middleware };
  ...
  try {
    ...
    if (runValidators) {
      await update.validate({ middleware: this.options.middleware });
...
Query.prototype.validate = async function validate(castedDoc, options, isOverwriting) {
  ...
  if (isOverwriting) {
    await castedDoc.$validate({ middleware: this.options.middleware });
  } else {
...
Query.prototype._replaceOne = async function _replaceOne() {
  ...
  const options = this._optionsForExec(this.model);
  this[queryUpdateSymbol] = new this.model(this[queryUpdateSymbol], null, { skipId: true, middleware: this.options.middleware });
  removeUnusedArrayFilters(this[queryUpdateSymbol], options);
...
Query.prototype.exec = async function exec(op) {
  ...
  return traceQuery(async function maybeTracedQueryExec() {
    ...
    try {
      await _this._hooks.execPre('exec', _this, [], { filter: buildMiddlewareFilter(_this.options, 'pre') });
    } catch (err) {
    ...
    await _this._hooks.execPost('exec', _this, [], { filter: buildMiddlewareFilter(_this.options, 'post') });
...
function _clonePopulateOptions(options) {
  const copy = new PopulateOptions({ ...options });
  if (Array.isArray(copy.populate)) {
    copy.populate = copy.populate.map(_clonePopulateOptions);
  }
  return copy;
}
