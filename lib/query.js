...
Query.prototype._getUpdateKeySchemaAndPath = function(pathString){
  ...
  for (var i = 0; i < l; i++) {
    ...
    if (part == '$' || part.match(/^[0-9]+$/)) continue;

    pathString =  pathString === '' ? part : pathString + '.' + part;

    l && console.log(" -- pathString is: " + pathString );
    l && console.log(" -- Part: " + part );
      
    pathType = this._getPathType(schema.paths[pathString]);
  ...
  r.schema = schema;
  l && console.log("(Required paths: %j", schema._requiredpaths );
...
Query.prototype._applyUpdateValidation = function( obj, cb ){
  ...
  while (i--) {
    ...
    for (var keyToUpdate in keyValuePairs) { 
      ...
      explore( s.schema, s.pathString, keyValuePairs, op, s );  
    }     
  ...
  function explore(schema, pathPrefix, obj, op, opts){
    var paths
      , pathString
      , path
      , pathType
    ...
    console.log("Exploring schema. Prefix: %s, obj:", pathPrefix);
    console.log(obj);

    // get the paths within the right prefix
    paths = Object.keys(schema.paths).filter( function(item){
    ...
    while (i--){
      ...
      switch(pathType.type){
        ...
        case 'schema':
          ...
          if( ! Array.isArray(val))
            throw new Error("Schema operator needs an array as parameter in the update object");
  ...
  function applyDefaultsAndSetters(obj, pathString, path){
  ...
  function getUpdateValue(obj, pathString){
    var split = pathString.split('.')
      , l = split.length
      , i, k, _obj = obj
    ;
  
    console.log("---------Getting updateValue key: '%s', split: '%j', obj:", pathString, split);
    console.log(obj);
