...
function buildNearParams(args, sphere) {
  ...
  if (args.length === 1) {
    if (Array.isArray(args[0])) {
      params.push({ center: args[0], spherical: sphere });
    } else if (typeof args[0] === 'string') {
      // just passing a path
      params.push(args[0]);
    } else if (utils.isObject(args[0])) {
      if (typeof args[0].spherical !== 'boolean') {
        args[0].spherical = sphere;
      }
      params.push(args[0]);
    } else {
    ...
    if (typeof args[0] === 'number' && typeof args[1] === 'number') {
      params.push({ center: [args[0], args[1]], spherical: sphere });
    } else if (typeof args[0] === 'string' && Array.isArray(args[1])) {
      params.push(args[0]);
      params.push({ center: args[1], spherical: sphere });
    } else if (typeof args[0] === 'string' && utils.isObject(args[1])) {
      params.push(args[0]);
      if (typeof args[1].spherical !== 'boolean') {
        args[1].spherical = sphere;
      }
      params.push(args[1]);
    } else {
      throw new TypeError('invalid argument');
    ...
    if (typeof args[0] === 'string' && typeof args[1] === 'number'
      ...
      params.push(args[0]);
      params.push({ center: [args[1], args[2]], spherical: sphere });
    } else {
      throw new TypeError('invalid argument');
    }
  } else {
...
Query.prototype.near = function() {
  ...
  // Delegate argument parsing to helper for clarity, preserving behavior.
  const params = buildNearParams(arguments, sphere);
