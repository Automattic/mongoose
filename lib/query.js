  ...
  const cb = (err, docs) => {
    ...
    if (!mongooseOptions.populate) {
      ...
      // call _completeManyLean here?
        ...
        // callback(null, docs) :
        completeMany(_this.model.schema, docs, null, completeManyOptions, callback);
    }
    ...
    for (const key of Object.keys(doc)) {
      if (Array.isArray(doc[key])) {
        doc[key].forEach((item) => {
          for (const k in item) {
            if (typeof item[k] === 'object' && item[k] != null && Object.keys(item[k]).length) {
              _completeOneLean(item[k], res, opts);
            }
            item = opts.lean.transform(item);
          }
        });
      } else if (typeof doc[key] === 'object' && doc[key] != null && Object.keys(doc[key]).length) {
        _completeOneLean(doc[key], res, opts);
      }
    }
    doc = opts.lean.transform(doc);
    if (callback) {
      return callback(null, doc);
    } else {
      return;
...
/*
function _completeManyLean(model, docs, fields, userProvidedFields, opts, callback) {
  console.log('this', model.schema.childSchemas);
  if (opts.lean && opts.lean.transform) {
    if (Array.isArray(docs)) {
        ...
        for (const key of Object.keys(doc)) {
          if (Array.isArray(doc[key])) {
            doc[key].forEach((item) => {
              for (const k in item) {
                if (typeof item[k] === 'object' && item[k] != null && Object.keys(item[k]).length) {
                  _completeManyLean(model, item[k], fields, userProvidedFields, opts);
                }
                item = opts.lean.transform(item);
              }
            });
          } else if (typeof doc[key] === 'object' && doc[key] != null && Object.keys(doc[key]).length) {
            _completeManyLean(model, doc[key], fields, userProvidedFields, opts);
          }
          doc = opts.lean.transform(doc);
        }
      });
      ...
      for (const key of Object.keys(docs)) {
        ...
        } else if (typeof docs[key] === 'object' && docs[key] != null && Object.keys(docs[key]).length) {
          _completeManyLean(model, docs[key], fields, userProvidedFields, opts);
        }
        docs = opts.lean.transform(docs);
      }
    }
  }
...
function _completeManyLean(schema, docs, path, opts, callback) {
  if (opts.lean && opts.lean.transform) {
    for (let i = 0; i < schema.childSchemas.length; i++) {
      ...
      console.log('childPath', childPath, 'schema', _schema);
      let doc = mpath.get(childPath, docs);
      console.log('before if', doc)
      if (doc == null) {
        continue;
      }
     doc = opts.lean.transform(doc[0]);
     console.log('doc', doc);
