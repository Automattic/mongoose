...
Query.prototype.find = function (criteria, callback) {
  ...
  } else if (criteria && criteria.constructor.name == 'Object') {
...
Query.prototype.elemMatch = function (path, criteria) {
  ...
  if (path.constructor.name == 'Object') {
    ...
    block = path;
    path = this._currPath;
  } else if (criteria.constructor.name == 'Object') {
...
Query.prototype.fields = function () {
  ...
  if (!arg0) return this;
  if (arg0.constructor.name == 'Object' || Array.isArray(arg0)) {
...
Query.prototype.update = function (doc, callback) {
  ...
  var model = this.model
    , options = this._optionsForExec(model)
    , useSet = model.options['use$SetOnSave'] // deprecated?
    , castQuery
    , castDoc
...
Query.prototype._castUpdate = function (obj) {
    ...
    if ('$' !== path[0]) {
      ...
      while (ii--) {
        ...
        if (schema.path(subpath)) {
          // Use the model for $set and $inc
          if ('$set' === method) {
            doc.set(subpath, subval);
            toSet.push([ subpath, doc.get(subpath) ]);
            continue;
          }
          ...
          // Allow MongoDB to perform the other atomic operations.
          // We don't need a model for those, right?
          toAtomic.push([ method, val ]);
        }
      }
    }
  }
  ...
  if (toSet.length) {
    toSet.map(function(path){
      delta['$set'][path[0]] = path[1];
    });
  }
  if (toAtomic.length) {
    toAtomic.map(function(atomic){
      delta[atomic[0]] = atomic[1];
    });
  }
  return delta;
};
