 ...
 *     query.box('loc', { ll : lowerLeft, ur : upperRight })
 *
...
Query.prototype.box = function (path, ll, ur) {
  if (Array.isArray(path)) {
    ur = ll;
    ll = path;
    if (!Array.isArray(ur)) {
      throw new Error("Invalid box() parameters");
    }
    path = null;
    ...
    ur = path.ur;
    path = null;
  } else if (!Array.isArray(ll) && utils.isObject(ll)) {
    ur = ll.ur;
    ll = ll.ll;
  }

  if (path) {
    return this.where(path).box(ll, ur);
  } else {
...
/**
 * Alias for [circle](#query_Query-circle)
 *
