...
/**
 * $push, $addToSet, $set, and $pushAll require casting docs
 * to real Documents for Update operations.
 * @private
 */

var castOps = {};
;('$push $pushAll $addToSet $set').split(' ').forEach(function (op) {
  castOps[op] = 1;
});
    ...
    while (ii--) {
      ...
      if (!pathschema) {
        ...
        while (p--) {
          ...
          if (foundschema && foundschema.caster) {
            pathschema = foundschema;
            break search;
          }
        }
      }
      ...
      if (pathschema) {
        ...
        var casted = pathschema.castForQuery(subval);
