...
Query.prototype._castUpdate = function (obj) {
  ...
  while (i--) {
    ...
    // handle $set sugar where the path is passed directly
    // Model.update({..}, { path: 'value' }, cb);
    if ('$' !== path[0]) {
      var o = {}
      o[path] = val;
      val = o;
    ...
    var subpaths = Object.keys(val);
    var ii = subpaths.length;
    var method = path;

    while (ii--) {
      var subpath = subpaths[ii];
      var subval = val[subpath];
      var pathschema = schema.path(subpath);
      var $each = false;

      if (!pathschema) {
        pathschema = (function search (subpath, schema) {
          // look for arrays of embedded docs
          var parts = subpath;
          var p = parts.length + 1;

          search:
          while (p--) {
            var trypath = parts.slice(0, p).join('.');
            var foundschema = schema.path(trypath);
            if (foundschema) {
              // is ArrayType?
              if (foundschema.caster) {
                if ('comments' === trypath) console.error('COMMENTS', p, parts.length, parts);
                // need to continue until all parts have been tried
                // once we find the array, we need to check if there are
                // document paths to look up for casting
                // Also we need to handle array.$.path too (schema.path doesn't work here)
                if (p !== parts.length) {
                  if ('$' === parts[p]) {
                    // need to recurse again starting with the remainder paths
                    // to handle things like comments.$.comments.$.title
        ...
        })(subpath.split('.'), schema)
      }

      ret[method] || (ret[method] = {});
      if (subpath in ret[method]) {
        throw new Error('duplicate path `' + subpath +'` in '+ method +' operation');
      }

      haskeys = true;

      if (pathschema) {
        if ('$addToSet' === method && subval.$each) {
          subval = subval.$each;
          $each = true;
        }

        // cast embedded documents for ops that add data to MongoDB
        // so they get ObjectIds
        if (pathschema.caster &&
            method in castOps &&
            ('Object' === subval.constructor.name || Array.isArray(subval))) {
          var tmp = pathschema.cast(subval);
        ...
        var casted = method in numberOps
          ? Number(subval)
          : pathschema.castForQuery(subval)
        ...
        if ($each) {
          ...
          ret[method][subpath] = casted;
        }

      } else {
        // adding data to a non-existing schema path
