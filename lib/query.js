...
Query.prototype._optionsForExec = function(model) {
  ...
  if (!options.safe && model.schema.options.safe) {
    options.safe = model.schema.options.safe;
  }

  if (!options.readPreference && model.schema.options.read) {
...
function tryCastFind(query) {
  try {
    query.cast(this.model);
    query._castError = null;
  } catch (err) {
    query._castError = err;
  }
}
...
function tryCastFindOne(query) {
  try {
    query.cast(query.model);
    query._castError = null;
  } catch (err) {
    query._castError = err;
  }
...
function tryCastCursor(query) {
  try {
    query.cast(this.model);
  } catch (err) {
    return (new QueryCursor(query, query.options))._markError(err);
  }
}
