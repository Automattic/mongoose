...
Query.prototype.findAndCount = async function findAndCount() {
  if (this.options.sort == null) {
    throw new MongooseError('Query.prototype.findAndCount() requires sort. Use `sort()` to set a sort order before calling `findAndCount()`.');
  }
  if (this.options.limit == null) {
    throw new MongooseError('Query.prototype.findAndCount() requires limit. Use `limit()` to set a limit before calling `findAndCount()`.');
  }

  this.op = 'find';

  const countQuery = this.clone().countDocuments().skip(0).limit(null);

  return Promise.all([this.exec(), countQuery.exec()]);
};
