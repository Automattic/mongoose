...
Query.prototype._findOneAndUpdate = wrapThunk(function(callback) {
  ...
  if (runValidators) {
    this.validate(this._update, options, false, err => {
      if (err) {
        return callback(err);
      }
      ...
      this._collection.collection.findOneAndUpdate(this._conditions, this._update, options).
        then(res => {
...
function _updateThunk(op, callback) {
  ...
  if (runValidators) {
    this.validate(this._update, options, isOverwriting, err => {
      ...
      this._collection.collection[op](castedQuery, this._update, options).then(
 ...
 * MongoDB will replace the existing document and will not accept any [atomic](https://docs.mongodb.com/manual/tutorial/model-data-for-atomic-operations/#pattern) operators (`$set`, etc.)
 *
...
Query.prototype.exec = async function exec(op) {
  ...
  let res;
  const asyncThunks = new Set([
    '_findOne',
    '_find',
    '_deleteMany',
    '_deleteOne'
  ]);
  if (asyncThunks.has(thunk)) {
    let error = null;
    try {
    ...
    await _executePostHooks(this, res, error);
  } else {
...
function _executePostHooks(query, res, error) {
  return new Promise((resolve, reject) => {
    const opts = error ? { error } : {};

    const queryMiddleware = query.model.schema.s.hooks.filter(hook => {
    ...
    queryMiddleware.execPost(query.op, query, [res], opts, (error) => {
      if (error) {
        return reject(error);
      }
...
function _executePreHooks(query) {
  return new Promise((resolve, reject) => {
    const queryMiddleware = query.model.schema.s.hooks.filter(hook => {
      const contexts = _getContexts(hook);
      if (hook.name === 'validate') {
    ...
    queryMiddleware.execPre(query.op, query, [], (error) => {
      if (error != null) {
        return reject(error);
      }
      resolve();
