...
Query.prototype._findOneAndReplace = wrapThunk(function(callback) {
  ...
  castedDoc.validate(err => {
    ...
    if (castedDoc.hasOwnProperty('_id')) {
      delete castedDoc._id;
    }
...
function _updateThunk(op, callback) {
  ...
  // Sending an `_id` to `replaceOne()` causes an error
  if (op === 'replaceOne' && this._update.hasOwnProperty('_id')) {
    delete this._update._id;
  }
