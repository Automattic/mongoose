...
Query.prototype.exec = async function exec(op, callback) {
  ...
  const [err, res] = await execPromise.
    then(res => ([null, res]), err => ([err, null]));
  if (err != null) {
    const CustomError = err.constructor;
    if (callback != null) {
      return callback(err);
    }
    let _err = null;
    if (err.copy instanceof Function && err instanceof MongooseError) {
      _err = new CustomError();
      _err.copy(err);
    } else if (err._args) {
      _err = new CustomError(...err._args);
    } else {
      _err = new CustomError(err.message);
    }
