...
/* TODO:
  1) Make the code work as is (partial)
  2) Add queue of defaults/setters/validations to apply, THEN apply them
  3) Make the code work again
  5) Handle case as in https://github.com/LearnBoost/mongoose/pull/1256
  6) Party
*/

Query.prototype._getPathType = function(path){
  
  // not in schema at all
  if (! path) 
    return { type: 'notFound', object: {} };

  // mixed
  if (path.caster && path.caster.casterConstructor && path.caster.casterConstructor.name === 'Mixed') 
     return { type: 'mixed', object: path.schema };      // -- d: [ ] TODO: CHECK
  if (path.options && path.options.type.name === 'Mixed' ) 
    return { type: 'mixed', object: path.schema };       // e: { type: Schema.Types.Mixed} TODO: CHECK

  // not here (ref:)
  if (path.options && path.options.ref)
    return { type: 'external', object: {} };             // d: { type: Schema.Types.ObjectId, ref: 'Two' },
  if (path.caster && path.caster.options && path.caster.options.ref)
    return { type: 'external', object: {} };             //  i: [{ type: Schema.Types.ObjectId, ref: 'Two' }]

  // a schema of EMBEDDED documents
  if (path.schema)
     return { type: 'schema', object: path.schema };     // f: [Two] ---- f: [ { a: something, b:else } ]

  // Array of basic types
  if (path.caster)
     return { type: 'simpleArray', object: {} };         // [ { type: String } ]

  // nothing special I could work out.
...
Query.prototype._getUpdateKeySchemaAndPath = function(pathString){
  var r = {}
    , schema = this.model.schema
    , part, parts = pathString.split('.')
    , i, l = parts.length
    , schemaPathString = ''
    , pathType
    , dontCheck = false
  ...
  console.log("To start with, pathString is '%s'", pathString);
  
  // Fish out the pair schema + path, by going through the
  // path and ending up with schematype + path
  for (var i = 0; i < l; i++) {
    part = parts[i];


    // ignore $ and numbers (people.$.name)
    if (part == '$' || part.match(/^[0-9]+$/)){
      continue;
    }

    schemaPathString =  schemaPathString === '' ? part : schemaPathString + '.' + part;

    l && console.log(" -- pathString is: " + schemaPathString );
    l && console.log(" -- Part: " + part );
      
    pathType = this._getPathType(schema.paths[schemaPathString]);
    l && console.log("pathType here is: " + pathType.type );

    switch(pathType.type) {
       
      case 'schema':
        // it's a schema: swap the schema variable, and
        // set the path to start from scratch

        l && console.log(" --  It's a schema! Switching schema"); 
        schema = pathType.object;
        schemaPathString = '';
        
      break;

      case 'external':
        // ref objects won't be validated since they won't even
        // make it onto the object and sholdn't be in an
        // update anyway
        l && console.log(" --  It's an EXTERNAL schema!"); 
        dontCheck = true;
        continue;
      break;

      case 'mixed':
        // something.mixed: {...}  will be validated,
        // something.mixed.a won't
  
        if( i != l-1){
          // the path is not at its last element: ignore
          l && console.log(" --  It's a mixed schema, but the path goes on: ignore!"); 
          dontCheck = true;
          continue;
        } else {
          // it's a root-level mixed, it might have validation
          l && console.log(" --  It's a mixed schema, root level, set it"); 
          schema = pathType.object;
          schemaPathString = '';
        }
      break;
      ...
      case 'simpleArray':
        // nothing to do, it's just a valid path within the schema
      break;
      ...
      default:
          l && console.log(" --  Nothing special found, will continue adding and looking"); 
      break;
    };
  }

  // FIXME: Take 
  // r.schemaPathString = pathString;

  r.prevPathString = '';
  if (schemaPathString.length){
     r.prevPathString = pathString.substring(0, pathString.length - schemaPathString.length - 1);
  }

  r.dontCheck = dontCheck;
  r.schemaPathString = schemaPathString;
  r.pathType = pathType.type;
  r.schema = Object.keys(schema.paths).join(', ');

  l && console.log("=================================================================" );
  l && console.log("In the end, _getSchemaAndPath returned: %j", r);
...
Query.prototype._applyUpdateValidation = function( obj, cb ){
   ...
   var self = this
     ...
     , total = 0, validating = {}
     , handled = false
     , errorsFound = false, errors = {}
     , pathsToCkeck = {}
   ;

  // Validation is not switched off -- calling the callback straight
  if(! this.options.enableUpdateValidation && ! require('mongoose').get('enableUpdateValidation')) {
    console.log("...........................Validation is turned off!");
    cb( null );
    return;
  }

  console.log("VALIDATION STARTED!");
  // console.log( require('util').inspect( this.model.schema, true, 5 ) );

  // Scan the update array, make a list of matching paths
  while (i--) {

    op = ops[i];
    keyValuePairs = obj[op];

    l && console.log("Analysing op: " + op );
    l && console.log("keyValuePairs for the op are: %j" , keyValuePairs );

    if ( !(op in castOps) ) { continue; } // TODO: check for $each and add $unset support (simplifying things for now)

    for (var keyToUpdate in keyValuePairs) { 
      var s = this._getUpdateKeySchemaAndPath(keyToUpdate);

      

      // explore  the surrounding paths as well
      var paths = Object.keys(s.schema.paths).filter( function(item){
        if (!s.schemaPathString) return true;
        if (item == s.schemaPathString) return true;
        console.log("Comparing %s and %s", item.substr(0, s.schemaPathString.length + 1), s.schemaPathString + '.' );
        return item.substr(0, s.schemaPathString.length + 1) === s.schemaPathString + '.';
      });
      ...
      explore( s.schema, keyValuePairs, op, s );  
    }     
    console.log("It's all over:");
    console.log(require('util').inspect( keyValuePairs, true, 5) );
  }

  // There was nothing to check, simply return the callback with no errors
  if( ! handled ){
    complete();
  }


  function explore(schema, obj, op, opts){
    var paths
      , pathString
      , path
      , pathType
    ...
    console.log("Exploring schema.  obj:");
    console.log(obj);


    console.log("Full list of paths:");
    console.log(Object.keys( schema.paths) );

    // get the paths within the right prefix
    paths = Object.keys(schema.paths);

    console.log("===========================Paths for this schema:");
    console.log(paths);
    ...
    while (i--){
      pathString = paths[i];
      path = schema.paths[ pathString ];
      pathType = self._getPathType(path); 
      val = getUpdateValue(obj, pathString);

      console.log("About to start the switch. Val for '%s': '%j'", pathString, val);
      ...
      switch(pathType.type){
        ...
        case 'schema':
          console.log('2 -- It\'s a schema!');
          
          val = getUpdateValue(obj, pathString);
          if( Array.isArray(val)){
            for (var ii = 0; ii < val.length; ii++){
              explore(pathType.object, '', val[ii], '$set');
            }
          }
        break;

        case 'simpleArray':
          console.log('3 -- It\'s a simple array!');

          if( ! Array.isArray(val) )
            throw new Error("Expecting an array in object...");

            for (var ii = 0; ii < val.length; i++)
              applyDefaultsAndSetters(obj, pathString, path, op);
        break;

        case 'mixed':
          console.log('4 -- It\'s mixed!');
        ...
        case 'field':
          console.log('4 -- It\'s just a bloody field, finally!');
          applyDefaultsAndSetters(obj, pathString, path);
        break;
  ...
  function applyDefaultsAndSetters(obj, pathString, path, fullPathString){
    var split = pathString.split('.')
      , l = split.length - 1
      , i, k, _obj = obj
    ;
  
    console.log("Entered applyDefaultsAndSetters...");
    console.log("Obj: '%j', pathString: '%s'", obj, pathString);
 
    for (i = 0; i < l; i++){
      k = split[i];
      console.log("K IS: %j", k );

      if( typeof (_obj[k]) === 'undefined' ){
        console.log("Path '%s' didn't exist, adding it" , k);
        _obj[k] = {}
      }
      _obj = _obj[k];
    }
    ...
    k = split[i];
    console.log("Now assigning %j[%s], ", _obj, k);
    
    // actually applying defaults and setters
    if (typeof(_obj[k]) === 'undefined')
      _obj[k] = path.getDefault({}, true );
    
    _obj[k] = path.applySetters(_obj[k], {}, true, undefined );
    
    // toValidate.push( obj[k], path, 

    console.log("END RESULT after the cure:");
    console.log(require('util').inspect(obj, true, 5));
  }

  function getUpdateValue(obj, pathString, pathPrefix){
    var split = pathString.split('.')
      , l = split.length
      , i, k, _obj = obj
    ;
  
    console.log("---------Getting updateValue key: '%s', split: '%j', pathPrefix: '%j', obj:", pathString, split, pathPrefix);
    console.log(obj);
    ...
    for (i = 0; i < l; i++){
      ...
      console.log("_obj: '%j'", _obj );
    }
    console.log("Returning: '%j'" , _obj);
    return _obj;
  }


  function validatePath( schemaType, currPath, errorKey, value ){

    var v;

    if( schemaType.paths[currPath].caster && schemaType.paths[currPath].caster.doValidate) v = schemaType.paths[currPath].caster;
    else v = schemaType.paths[currPath];

    // No validators defined
    if( ! v ) return;

    if (validating[errorKey]){  return; }
    validating[errorKey] = true;
    ...
    process.nextTick(function(){
      ...
      // Run the validation. Note that unlike "normal" validation calls,
      // the scope is set to the value itself. People who turn on validation
      // for updates will need to check typeof(this) 
      v.doValidate( value, function (err) {

        if( err ){
          errorsFound = true;

          // Ensure that we were passed an error, or make one up
          // on the spot
          if ( 'string' === typeof err) {
            err = new ValidatorError(keytoUpdate, err);
          }
          errors[errorKey] = err;
        }          
        --total || complete();
...
Query.prototype.update = function update (doc, callback) {
  ...
  this._applyUpdateValidation(castedDoc, function(err) {
...
Query.prototype._findAndModify = function (type, callback) {
  ...
  this._applyUpdateValidation(castedDoc, function(err) {
    ...
    if( err ){
      process.nextTick(promise.error.bind(promise, err));
    } else {
    // SORRY, UNINDENTED ORIGINAL CODE BELOW. WILL INDENT WHEN POSITION IS FINAL
...
/*
      ...
      (function(){ // Make a separate context

        l && console.log("OK, entering recursive exploration of update parameter");

        // It starts with the current path
        explore( currValue, currSchema, currPath, keyToUpdate, keyValuePair, keyToUpdate  );

        function explore( obj, schema, path, pathIndex, updateObject, updateKey ){

          l && console.log("Explore entered. Obj: '%j', path: '%s', pathIndex: '%s', updateObject: '%j', updateKey: '%s'" , obj, path, pathIndex, updateObject, updateKey );

          var pathType = getPathType( schema.path(path));
          l && console.log('Switch started, pathType: %j' , pathType.type);

          switch (pathType.type) {

            case 'mixed':
              l && console.log("It's mixed: run validation in case the mixed element had them, but quit at this point");
              validatePath( schema, path, pathIndex, obj );
              updateObject[updateKey] = schema.paths[path].applySetters(obj, {}, true, obj );              
              handled = true;
              return;
            break;
            
            case 'simpleArray':
              l && console.log("It's an array of basic types (not schemas or mixed). Running validation for every element and quitting");
              if( Array.isArray(obj)){
                for( var k in obj ){
                  l && console.log('Validating %j', obj[k]);
                  validatePath( schema, path, joinDot( pathIndex, k), obj[k] );
                  updateObject[updateKey][k] = schema.paths[path].applySetters(obj[k], {}, true, obj );              
                  handled = true;
                }
              }
              return;
            ...
            case 'external':
              l && console.log('External schema, exit, won\'t validate as even update will simply fail anyway');
              return;
            break;

            case 'schema':
              l && console.log("It's a schema: change the current schema to the new one, re-running explore without path");
              explore( obj, pathType.object, '', pathIndex, obj );
            break;
              
            case 'field':
              l && console.log('Path found: run validation, end of search!');
              ...
              schema.paths[path].applySetters(obj, {}, true, obj );              
              validatePath( schema, path, pathIndex, obj );
              updateObject[updateKey] = schema.paths[path].applySetters(obj, {}, true, obj );              
              handled = true;
              return;
            ...
            case 'notFound':
              l && console.log('Path not found');

              if (Array.isArray(obj) ){
                l && console.log('Value is an array, will explore each item separately, only changing pathIndex');
                for(var k in obj){
                  explore( obj[k], schema, path, joinDot( pathIndex, k), obj, k  );
                }
              } else if( typeof(obj) == 'object'){
                l && console.log('Value is an object, will continue looking for paths');
                for( var k in obj ){
                  explore( obj[k], schema, joinDot( path, k ), joinDot( pathIndex, k ),       obj, k );
                }
              } else {
                l && console.log('Value is not an object nor an array, and it is not in the schema, giving up');
                return;
              }
              break;
          }
...
Query.prototype.___applyUpdateValidation = function( obj, cb ){
  ...
  console.log( require('util').inspect( this.model.schema, true, 4 ) );

  console.log("OBJECT IS:");
  console.log(obj);

  // Update Validation is off: call the callback straight, no errors
  //if( ! this.options.enableUpdateValidation ){
  //  cb( null );
  //  return;
  //}
  ...
  while (i--) {
    ...
    // only consider $push or $pushAll or $addToSet or $set -- $pop and $inc are
    // uncheckable
    if (op in castOps ){ // TODO: check for $each and add $unset support (simplifying things for now)
      _walkUpdatePath(keyValuePair, op);
    }
  }
  ...
  function _walkUpdatePath (obj, op, pref) {
    ...
    console.log("Called _walkUpdatePath: obj:'%j', op:'%s', pref:'%s'", obj, op, pref );
    ...
    while (i--) {
      ...
      if (val && 'Object' === val.constructor.name) {
        ...
        console.log("val IS an object: maybe exploring further, maybe not");

        // watch for embedded doc schemas
        schema = self._getSchema(prefix + key);
        ...
        if (schema && schema.caster && op in castOps) {
          ...
          console.log("schema found, it was a caster type and op was $push or $pushAll or $addToSet or $set");
          console.log("Run validation!");

          if ('$each' in val) {
            obj[key] = {
              $each: _validateUpdateVal(schema, val.$each, op)
            }
          } else {
            obj[key] = _validateUpdateVal(schema, val, op);
          }


        } else {
          console.log("schema not found, or it was not a caster type or !(op was $push or $pushAll or $addToSet or $set)");
          console.log("Actually exploring further...");
          _walkUpdatePath(val, op, prefix + key);
        }

      } else {
        console.log("val is NOT an object: it's a simple type. Run validation!");
        schema  = self._getSchema(prefix + key);

        obj[key] = _validateUpdateVal(schema, val, op, key);
      }
    }
  }
  ...
  function _validateUpdateVal (schema, val, op, $conditional) {

    console.log("CALLED _validateUpdateVal: schema:'%s', val:'%j', op:'%s', $conditional:'%s'", schema.path, val, op, $conditional);
