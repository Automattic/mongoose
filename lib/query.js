...
const queryTracingChannel = diagnosticsChannel.tracingChannel('mongoose:query');
...
Query.prototype.exec = async function exec(op) {
  if (!queryTracingChannel.hasSubscribers) {
    return execQueryWithoutTrace.call(this, op);
  }
  ...
  const context = {
    ...
    class: 'Query',
    method: 'exec',
    operation: typeof op === 'string' ? op : this.op,
    collection,
  ...
  return queryTracingChannel.tracePromise(execQueryWithoutTrace, context, this, op);
};

async function execQueryWithoutTrace(op) {
