...
Query.prototype._find = wrapThunk(function(callback) {
  ...
  this._collection.collection.find(filter, options, (err, cursor) => {
    ...
    if (options.explain) {
      return cursor.explain(res => cb(null, res), err => cb(err));
    }
    try {
...
Query.prototype._findOneAndUpdate = wrapThunk(function(callback) {
  ...
  if (runValidators) {
    this.validate(this._update, options, false, err => {
      ...
      this._collection.collection.findOneAndUpdate(this._conditions, this._update, options).then(
        res => {
          const doc = res.value;
  ...
  if (runValidators) {
    this.validate(this._update, options, isOverwriting, err => {
      ...
      this._collection.collection[op](castedQuery, this._update, options).then(
        ...
        err => { console.log('Caught', err); callback(err); }
      );
    });
    return null;
  }
