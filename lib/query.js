 ...
 *
 * MongoDB 2.4 deprecated the use of `$within`, replacing it with `$geoWithin`.
 * Mongoose uses `$geoWithin` by default (which is 100% backward compatible
 * with $within). If you are running an older version of MongoDB, set this flag
 * to `false` so your `within()` queries continue to work.
 *
 ...
 * ####Example
 *
 *     // FIXME for mongoose 3.7.x
 *     // decide what to name this function in a mongoose context
 *     // add tests
 *     // fix the docs below
 *
 ...
 *     var VideoQ = query.toConstructor();
 *
 *     // VideoQ is now a subclass of query() and works the same way but with the
 *     // default query parameters and options set.
 *
 *     // run a query with the previous settings but filter for movies with names
 *     // that start with "Life".
 *     VideoQ().where({ name: /^Life/ }).exec(cb);
 *
 * New in 3.7.x
 *
 ...
 * ####NOTE:
 *
 * Only use `$where` when you have a condition that cannot be met using other
 * MongoDB operators like `$lt`.
 * **Be sure to read about all of [its
 * caveats](http://docs.mongodb.org/manual/reference/operator/where/) before
 * using.**
 *
 ...
 *
 * When using string syntax, prefixing a path with `-` will flag that path as
 * excluded. When a path does not have the `-` prefix, it is included. Lastly,
 * if a path is prefixed with `+`, it forces inclusion of the path, which is
 * useful for paths excluded at the [schema
 * level](/docs/api.html#schematype_SchemaType-select).
 *
 * ####Example
 *
 ...
 * Cannot be used with `distinct()`.
 *
 * _v2 had slightly different syntax such as allowing arrays of field names.
 * This support was removed in v3._
 *
 * @method select
 * @memberOf Query
 ...
 * ####Preferences:
 *
 *     primary - (default)  Read from primary only. Operations will produce an
 ...
 * [here](http://docs.mongodb.org/manual/applications/replication/#read-preference)
 * and
 ...
 * javascript objects, not [MongooseDocuments](#document-js). They have no
 * `save` method, getters/setters or other Mongoose magic applied.
 *
 * ####Example:
 *
 *     new Query().lean() // true
 ...
 *
 * This is a
 * [great](https://groups.google.com/forum/#!topic/mongoose-orm/u2_DzDydcnA/discussion)
 * option in high-performance read-only scenarios, especially when combined
 * with [stream](#query_Query-stream).
 *
...
/**
 * Declares the query a findOne operation. When executed, the first found
 * document is passed to the callback.
 *
 ...
 * ####Note
 *
 * The operation is only executed when a callback is passed. To force execution
 * without a callback (which would be an unsafe write), we must first call
 * remove() and then execute it by using the `exec()` method.
 *
 *     // not executed
 ...
 * any `options`, and returns the found document (if any) to the callback. The
 * query executes immediately if `callback` is passed.
 *
 * ####Available options
 *
 ...
 * to the callback. Executes immediately if `callback` is passed.
 *
 * ####Available options
 *
 * - `sort`: if multiple docs are found by the conditions, sets the sort order to choose which doc to update
 ...
 * ####Note
 *
 * Passing an empty object `{}` as the doc will result in a no-op unless the
 * `overwrite` option is passed. Without the `overwrite` option set, the update
 * operation will be ignored and the callback executed without sending the
 * command to MongoDB so as to prevent accidentally overwriting documents in the
 * collection.
 *
 * ####Note
 *
 * The operation is only executed when a callback is passed. To force execution
 * without a callback (which would be an unsafe write), we must first call
 * update() and then execute it by using the `exec()` method.
 *
...
Query.prototype.update = function (conditions, doc, options, callback) {
  ...
  // when the user calls update() without a callback, it is assumed that they
  // want to execute an unsafe write. We need to remove this in 4.0 and
  // make them explicitly pass it.
  if (!callback) {
    callback = true;
    if (!options) options = {};
    options.safe = { w : 0 };
 ...
 * separate query is then executed for each path specified for population.
 * After a response for each query has also been returned, the results are
 ...
 * ####Valid options
 *
 *   - `transform`: optional function which accepts a mongoose document. The
 *                  return value of the function will be emitted on `data`.
 *
 * ####Example
 *
 ...
 * ####NOTE:
 *
 * In Mongoose 3.7, `intersects` changed from a getter to a function. If you
 * need the old syntax, use
