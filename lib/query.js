...
/**
 * Validates query filter values against the schema using
 * [`Model.validateFilter()`](https://mongoosejs.com/docs/api/model.html#Model.validateFilter()).
 * Callers should only invoke this when the `validateFilter` query option is enabled.
 *
 * @method _validateFilterConditions
 * @memberOf Query
 * @api private
 * @instance
 */

Query.prototype._validateFilterConditions = async function _validateFilterConditions() {
  if (this.error() != null) {
    return;
  }

  try {
    await this.model.validateFilter(this._conditions, this);
  } catch (err) {
    this.error(err);
  }
};
...
Query.prototype._find = async function _find() {
  ...
  if (this.options.validateFilter) {
    await this._validateFilterConditions();
  }
...
/**
 * Enables validation for query filters. When enabled, Mongoose validates
 * query filter values using [`Model.validateFilter()`](https://mongoosejs.com/docs/api/model.html#Model.validateFilter()).
 * This is a query-level option only and is disabled by default.
 *
 * #### Example:
 *
 ...
 *
 * @param {boolean} [bool=true] when called with no argument, enables filter validation
 * @return {Query} this
 * @api public
 */

Query.prototype.validateFilter = function(val) {
  if (val === undefined) {
    val = true;
  }

  this.options.validateFilter = arguments.length ? !!val : true;
