...
Query.prototype.updateOne = function updateOne (query, doc, options, callback) {
  ...
  if (!castedDoc) {
    process.nextTick(promise.resolve.bind(promise));
    return promise;
  } else if (castedDoc instanceof Error) {
    promise.error(castedDoc);
    return promise;
  }

  castedQuery = castQuery(this);
  if (castedQuery instanceof Error) {
    promise.error(castedQuery);
    return promise;
  }

  opts = this._optionsForExec(model);

  opts.new = 'new' in opts
    ? opts.new
    : true

  opts.upsert = 'upsert' in opts
    ? opts.upsert
    : false
...
Query.prototype.removeOne = function (query, callback) {
  // TODO
