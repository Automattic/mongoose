...
Query.prototype.exec = async function exec(op) {
  ...
  const tracingChannel = getTracingChannel('mongoose:query:exec');
  const useTracing = tracingChannel && tracingChannel.hasSubscribers;
  let tracingCtx;
  if (useTracing) {
    tracingCtx = {
      operation: this.op,
      collection: (this.model && this.model.collection && this.model.collection.name) || '',
  ...
  try {
    ...
    await this._hooks.execPost('exec', this, [res]);
