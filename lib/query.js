...
Query.prototype._applyUpdateValidation = function( obj, cb ){
  ...
  while (i--) {
    ...
    for (var keyToUpdate in keyValuePairs) { 
      ...
      // var pathPrefix = s.schemaPathString;

      // explore  the surrounding paths as well
      /*
      var paths = Object.keys(s.schema.paths).filter( function(item){
        if (!pathPrefix) return true;
        if (item == pathPrefix) return true;
        console.log("Comparing %s and %s", item.substr(0, pathPrefix.length + 1), pathPrefix + '.' );
        return item.substr(0, pathPrefix.length + 1) === pathPrefix + '.';
      });
  ...
  function explore(schema, obj, op, opts){
    ...
    while (i--){
      ...
      val = getUpdateValue(obj, pathString, pathPrefix);
