...
/**
 * Specifies a `$eq` query condition.
 *
 * When called with one object argument with no preceeding `where()`, adds a `$eq` to every
 * property in the object. Otherwise uses the most recent path passed to `where()`.
 *
 * The one object syntax is useful because it protects against [query selector injection attacks](https://thecodebarbarian.com/2014/09/04/defending-against-query-selector-injection-attacks.html).
 *
 * ####Example
 *
 *     Thing.find().eq({ age: 21 });
 *
 *     // or
 *     Thing.find().where('age').eq(21);
 *
 *     // or
 *     Thing.find().eq('age', 21);
 *
 ...
 * @param {String|Object} [path]
 * @param {any} val
 * @see $eq http://docs.mongodb.org/manual/reference/operator/eq/
 * @api public
 */
...
'eq gt gte lt lte ne in nin all regex size maxDistance minDistance'.split(' ').forEach(function(conditional) {
  const $conditional = '$' + conditional;
  Query.prototype[conditional] = function() {
    let path;
    let val;

    if (arguments.length === 1 && typeof arguments[0] === 'object' && arguments[0] != null && this._path == null) {
      val = arguments[0];
      for (const key of Object.keys(val)) {
        this._conditions[key] = this._conditions[key] || {};
        this._conditions[key][$conditional] = val[key];
      }
      return this;
    }
