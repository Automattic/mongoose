    ...
    Query.base.findOne.call(self, {}, function(err, doc) {
      ...
      if (!options.populate) {
        return true === options.lean
          ...
          : completeOne(self.model, doc, projection, self, null, function(error, doc) {
            if (error) {
      ...
      self.model.populate(doc, pop, function(err, doc) {
        ...
        return true === options.lean
          ? promise.complete(doc)
          : completeOne(self.model, doc, projection, self, pop, function(error, doc) {
            if (error) {
              return promise.error(err);
            }
            promise.complete(doc);
...
Query.prototype._count = function(conditions, callback) {
  try {
    this.cast(this.model);
  } catch (err) {
...
Query.prototype.count = function (conditions, callback) {
  ...
  this._count(conditions, callback);
...
function completeOne (model, doc, fields, self, pop, callback) {
  var opts = pop ?
    { populated: pop }
    : undefined;
...
Query.prototype._findAndModify = function (type, callback) {
  ...
  var cb = function(err, doc) {
    ...
    if (!options.populate) {
      return true === options.lean
        ? promise.complete(doc)
        : completeOne(self.model, doc, fields, self, null, function(error, doc) {
          if (error) {
            return promise.error(err);
          }
          promise.complete(doc);
    ...
    self.model.populate(doc, pop, function (err, doc) {
      ...
      return true === options.lean
        ? promise.complete(doc)
        : completeOne(self.model, doc, fields, self, pop, function(error, doc) {
          if (error) {
            return promise.error(err);
          }
          promise.complete(doc);
  ...
  while (i--) {
    ...
    if (val && 'Object' === val.constructor.name) {
      ...
      if (schema && schema.caster && op in castOps) {
        ...
        if (this.model.schema.pathType(prefix + key) === 'adhocOrUndefined' &&
            this.model.schema.pathType(prefix + '0.' + key) === 'adhocOrUndefined') {
          if (strict === 'throw') {
