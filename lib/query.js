 ...
 *     Kitten.find().tailable({ awaitdata: true })
 *     Kitten.find().tailable(true, { awaitdata: true })
 *
...
Query.prototype.tailable = function (v, opts) {
  this.options.tailable = arguments.length ? !!v : true;

  var options = opts || v;
  if (options && options.awaitdata) this.options.awaitdata = true;
...
Query.prototype._applyUpdateValidation = function( update, cb ){

  var cleanKeyToUpdate, part;
  var currSchema, currPathString='';
  var total = 0, validating = {};
  var handled = false;
  var errorsFound = false, errors = {};
  var op;
  var self = this;
  var l = true;
  var keyValuePairs;
  ...
  for( var k in update){
    l && console.log("Analysing: " + k );

    op = k;
    keyValuePairs = update[k];

    l && console.log("keyValuePairs is: %j" , keyValuePairs );

    // For validation, we are only really interested in
    // $set and $push, as we can't do much about other
    // things without having the original object
    if(op !== '$set' && op !== '$push' && op != '$unset') continue;

    // console.log( require('util').inspect( this.schema,true, 5 ) );

    for( var keyToUpdate in keyValuePairs){ 

      // Clean up position indexes ('some.4.thing' or 'some.$.thing')
      cleanKeyToUpdate = keyToUpdate.replace(/(\.[0-9]+\.)|(\.\$\.)/, '.');
      cleanKeyToUpdate = cleanKeyToUpdate.replace(/(\.[0-9]+$)|(\.\$$)/, '').split('.');

      l && console.log("Turned into: " + cleanKeyToUpdate );

      // The model's schema is the "root" schema to consider, with an empty path
      currSchema = this.model.schema;
      currPathString = '';

      // Fish out the pair schema + path, by going through the
      // path and ending up with schematype + path
      var dontCheck = false;
      for (var i = 0, ll = cleanKeyToUpdate.length; i < ll; i++) {
        part = cleanKeyToUpdate[i];

        // currPathString + currPathString + '.' + part
        currPathString = joinDot(currPathString, part);

        l && console.log("currPathString is: " + currPathString );

        // console.log(currSchema.paths[part]);
        l && console.log(" -- Part: " + part );
        l && console.log(" -- currPathString: " + currPathString );
      
        var pathType = getPathType( currSchema.paths[currPathString]);
        l && console.log("pathType here is: " + pathType.type );
        // It's a straight schema: reset the currPathString, set the new schema
        if( pathType.type == 'schema' ){
          l && console.log(" --  It's a schema! Switching schema"); 
          currSchema = pathType.object;
          currPathString = '';

          console.log("New schema:");
          console.log(currSchema);

        // It's a path but it's not here (reference to external schema): skip
        } else if( pathType ==  'external' ) {
          l && console.log(" --  It's an EXTERNAL schema!"); 
          dontCheck = true;
          continue;

        // It's a path but it's mixed and the path is not at its last
        // element: ignore
        } else if( pathType == 'mixed'){
          l && console.log(" --  It's a mixed schema!"); 

          if( i != l-1 ){
            l && console.log(" --  It's a mixed schema, but the path goes on: ignore! (updating an inner part of a mixed)"); 
            dontCheck = true;
            continue;
          } else {
            l && console.log(" --  It's a mixed schema, last path! Switch to it"); 
            currSchema = pathType.object;
          }


        } else {
          l && console.log(" --  Nothing special, add the next chunk, keep looking"); 
        }
      };

      // The path was marked as something not to check: skip
      if( dontCheck ){
         l && console.log("Skipping path");
         continue;
      }

      // At this point, we have 1) The schema 2) The operation 3) The value
      // We need to cycle the values and run a validator every time the
      // value is a valid path, STOPPING if the schema type is mixed
      // (as that would be silly)
      l && console.log("=================================================================" );
      l && console.log("keytoUpdate is: %j", keyToUpdate );
      l && console.log("keyValuePairs is:");
      l && console.log( require('util').inspect( keyValuePairs, true, 5 ) );
      ...
      (function(){ // Make a separate context
        ...
        explore( currSchema, currPathString, keyToUpdate,    keyValuePairs, keyToUpdate  );


        function explore( schema, path, fullPathIndex, updateObject, updateKey ){

          var obj = updateObject[updateKey];

          l && console.log("Explore entered. Obj: '%j', path: '%s', fullPathIndex: '%s', updateObject: '%j', updateKey: '%s'" , obj, path, fullPathIndex, updateObject, updateKey );
          ...
          switch (pathType.type) {
            ...
            case 'mixed':
              ...
              validatePath( schema, path, fullPathIndex, obj );
            ...
            case 'simpleArray':
              ...
              if( Array.isArray(obj)){
                for( var k in obj ){
                  ...
                  validatePath( schema, path, joinDot( fullPathIndex, k), obj[k] );
            ...
            case 'schema':
              ...
              explore( pathType.object, '', fullPathIndex, updateObject, updateKey );
            break;
            ...
            case 'notFound':
              ...
              if (Array.isArray(obj) ){
                l && console.log('Value is an array, will explore each item separately, only changing fullPathIndex');
                for(var k in obj){
                  explore( schema, path, joinDot( fullPathIndex, k), obj, k  );
                }
              } else if( typeof(obj) == 'object'){
                l && console.log('Value is an object, will continue looking for paths');
                for( var k in obj ){
                  explore( schema, joinDot( path, k ), joinDot( fullPathIndex, k ),       obj, k );
                }
              } else {
  ...
  function validatePath( schemaType, currPathString, errorKey, value ){

    var v;

    if( schemaType.paths[currPathString].caster && schemaType.paths[currPathString].caster.doValidate) v = schemaType.paths[currPathString].caster;
    else v = schemaType.paths[currPathString];
  ...
  function getPathType(path){
    ...
    if( path.caster && path.caster.casterConstructor && path.caster.casterConstructor.name === 'Mixed' )
      return { type: 'mixed', object: {} };          // -- d: [ ] 
    if( path.options && path.options.type.name === 'Mixed' )
      return { type: 'mixed', object: {} };          // e: { type: Schema.Types.Mixed}
