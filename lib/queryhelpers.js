...
exports.applyPaths = function applyPaths(fields, schema) {
  ...
  if (fields) {
    ...
    while (keyIndex--) {
      ...
      exclude = !field; // indicates to exclude as opposed to exclusive projection
      break;
    }
  }
  ...
  function analyzePath(path, type) {
    const plusPath = '+' + path;
    const hasPlusPath = fields && plusPath in fields;
    const minusPath = '-' + path;
    const hasMinusPath = fields && minusPath in fields;
    ...
    if (exclude === false && type.selected && !fields[path]) {
      delete fields[path];
      return;
    }
