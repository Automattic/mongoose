...
Schema.interpretAsType = function (path, obj) {
  if (obj.constructor) {
    var constructorName = obj.constructor.name ||
      Function.toString.call(obj.constructor).trim().match(/^function\s*([^\s(]+)/)[1];
  ...
  var name = 'string' == typeof type
    ...
    : (type.name || Function.toString.call(type).trim().match(/^function\s*([^\s(]+)/)[1]);
