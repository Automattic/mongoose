...
const hasPositionalSyntaxNumberRE = /\.\d+/;
const hasPositionalSyntaxNumberMiddleRE = /\.\d+\./g;
const hasPositionalSyntaxNumberEndRE = /\.\d+$/;
...
function _pathToPositionalSyntax(path) {
  if (path.indexOf('.') === -1 || !hasPositionalSyntaxNumberRE.test(path)) {
    return path;
  }
  return path
    .replace(hasPositionalSyntaxNumberMiddleRE, '.$.')
    .replace(hasPositionalSyntaxNumberEndRE, '.$');
}
