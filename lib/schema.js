...
Schema.interpretAsType = function(path, obj, options) {
  ...
  const type = obj[options.typeKey] && (options.typeKey !== 'type' || !obj.type.type)
      ? obj[options.typeKey]
      : {};
...
Schema.prototype.eachPath = function(fn) {
  const keys = Object.keys(this.paths), len = keys.length;
...
Schema.prototype.requiredPaths = function requiredPaths(invalidate) {
  ...
  let paths = Object.keys(this.paths), i = paths.length, ret = [];
...
Schema.prototype.setupTimestamp = function(timestamps) {
  if (timestamps) {
    const createdAt = timestamps.createdAt || 'createdAt', updatedAt = timestamps.updatedAt || 'updatedAt', schemaAdditions = {};
    ...
    const genUpdates = function() {
      const now = new Date();
      const updates = {$set: {}, $setOnInsert: {}};
...
function getPositionalPathType(self, path) {
  ...
  let last = subpaths.length - 1, subpath, i = 1;
...
Schema.prototype.indexes = function() {
  ...
  const collectIndexes = function(schema, prefix) {
    ...
    prefix = prefix || '';
    let key, path, index, field, isObject, options, type;
  ...
  function fixSubIndexPaths(schema, prefix) {
    let subindexes = schema._indexes, len = subindexes.length, indexObj, newindex, klen, keys, key, i = 0, j;
