...
function _schemaTypeToJSONSchema(schemaType, isRequired, options) {
  ...
  if (schemaType.instance === 'Number') {
    ...
    if (embeddedSchemaType.schema) {
      ...
      if (useBsonType) {
        additionalProperties.additionalProperties = { bsonType: bsonType.length === 1 ? bsonType[0] : bsonType };
      } else {
        additionalProperties.additionalProperties = { type: type.length === 1 ? type[0] : type };
      }
    }
  } else {
