...
function Schema(obj, options) {
  ...
  if (timestamps) {
    var createdAt = timestamps.createdAt || 'createdAt',
        ...
        fieldType = timestamps.fieldType || Date,
        dateFunc = timestamps.dateFunc,
        schemaAdditions = {};
    
    schemaAdditions[updatedAt] = fieldType;

    if (!this.paths[createdAt]) {
      schemaAdditions[createdAt] = fieldType;
    }

    this.add(schemaAdditions);

    this.pre('save', function(next) {
      var defaultTimestamp = dateFunc ? dateFunc() : new Date();
    ...
    var genUpdates = function() {
      var now = dateFunc ? dateFunc() : new Date();
