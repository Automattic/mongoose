...
Schema.prototype.virtual = function(name, options) {
  ...
  let cur = parts[0];
  for (let i = 0; i < parts.length - 1; ++i) {
    const schemaType = this.paths[cur];
    if (schemaType != null && (schemaType.$isMongooseDocumentArray || schemaType.$isSingleNested)) {
      const remnant = parts.slice(i + 1).join('.');
      if (schemaType.schema.pathType(remnant) === 'adhocOrUndefined') {
        schemaType.schema.virtual(remnant, options);
      }
      break;
    } else if (schemaType != null && schemaType.$isSchemaMap && parts[i + 1] === '$*' && schemaType.$__schemaType.schema != null) {
      const remnant = parts.slice(i + 2).join('.');
      if (remnant.length > 0 && schemaType.$__schemaType.schema.pathType(remnant) === 'adhocOrUndefined') {
        schemaType.$__schemaType.schema.virtual(remnant, options);
      }
      break;
    }
    cur += '.' + parts[i + 1];
