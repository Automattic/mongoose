...
Schema.prototype._getDocumentMiddleware = function _getDocumentMiddleware() {
  return this.s.hooks.
    ...
    filter(hook => {
      // Custom save methods may delegate to $save(), which still needs built-in hooks.
      // Other custom methods must not run query-specific built-in hooks.
      if (this.methods[hook.name] && hook.name !== 'save') {
...
Schema.prototype.virtual = function(name, options) {
  ...
  if (utils.hasUserDefinedProperty(options, ['ref', 'refPath'])) {
    ...
    virtualPreInit[symbols.builtInMiddleware] = true;
    this.pre('init', virtualPreInit);
    ...
    function virtualPreInit(obj, opts) {
      if (mpath.has(name, obj)) {
        ...
        if (opts?.hydratedPopulatedDocs && !options.count) {
          ...
          if (!Array.isArray(populatedVal) && !populatedVal.$__ && modelNames?.length === 1) {
            ...
            this.$$populatedVirtuals[name] = PopulateModel.hydrate(populatedVal, null, { middleware: opts.middleware });
          } else if (Array.isArray(populatedVal) && modelNames?.length === 1) {
            const PopulateModel = this.db.model(modelNames[0]);
            for (let i = 0; i < populatedVal.length; ++i) {
              if (!populatedVal[i].$__) {
                populatedVal[i] = PopulateModel.hydrate(populatedVal[i], null, { hydratedPopulatedDocs: true, middleware: opts.middleware });
              }
            }
...
Schema.prototype.remove = function(path) {
  ...
  if (Array.isArray(path)) {
    path.forEach(function(name) {
      ...
      // A document array registers its element paths under `subpaths` and a
      // single nested subdocument registers its paths under `singleNestedPaths`.
      // Left behind, they keep answering `path()` and `pathType()`, and queries
      // on the removed path keep being cast.
      _deleteSubpaths(this, name);
      _deletePath(this, name);
...
function _deleteSubpaths(schema, name) {
  const prefix = name + '.';
  for (const subpath of Object.keys(schema.subpaths)) {
    if (subpath === name || subpath.startsWith(prefix)) {
      delete schema.subpaths[subpath];
    }
  }
  for (const subpath of Object.keys(schema.singleNestedPaths)) {
    if (subpath === name || subpath.startsWith(prefix)) {
      delete schema.singleNestedPaths[subpath];
    }
  }
}
