    ...
    if (isNested) {
      for (let i = 0; i < schemaType._presplitPath.length - 1; ++i) {
        ...
        if (jsonSchemaForPath.properties[subpath] == null) {
          jsonSchemaForPath.properties[subpath] = {
...
function _schemaTypeToJSONSchema(schemaType, isRequired, options) {
  ...
  if (schemaType.instance === 'Number') {
    ...
    if (schemaType.schema) {
      // DocumentArray
      if (useBsonType) {
        additionalProperties.items = { bsonType: ['object', 'null'], ...schemaType.schema.jsonSchema(options) };
      } else {
        additionalProperties.items = { type: ['object', 'null'], ...schemaType.schema.jsonSchema(options) };
      }
    } else {
      // Primitive array
      ...
      if (useBsonType) {
        ...
        additionalProperties.items = { type, ...convertedSchemaType.additionalProperties };
      }
    }
  }
