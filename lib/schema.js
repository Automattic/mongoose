...
function Schema(obj, options) {
  if(traverseObjectForProjection(obj)) {
    throw new MongooseError('Schema level projection not supported');
  }
...
function traverseObjectForProjection(obj) {
  if (obj == null) {
    console.log('yo')
    return false;
  }
  ...
  for (const key in obj) {
    if(obj[key] == null) {
      ...
      console.log('key', key, ' The obj', obj[key])
      return traverseObjectForProjection(obj[key])
    } else if (key == 'select' && typeof key == 'boolean') {
      console.log('hello')
      return true;
    }
  }
