...
Schema.prototype.registerHooks = function () {
  var self = this;
  var pathKeys = Object.keys(this.paths);

  this.post('validate', function (doc) {
    if (!doc.isNew) { return; }
    var path;
    pathKeys.filter(function (path) {
      return '2dsphere' === self.paths[path]._index;
    }).forEach(function (pathKey) {
      path = doc.get(pathKey);

      if (Array.isArray(path) && path.length === 0) {
        doc.set(pathKey, void 0);
      }
    });
  });
};
