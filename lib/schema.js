...
Schema.prototype.removeIndex = function removeIndex(index) {
  if (arguments.length > 1 || Object.keys(arguments[0]).length > 1 && typeof arguments[0] !== 'string') {
    throw new Error('removeIndex() takes only 1 parameter');
  }
  for (let i = 0; i < this._indexes.length; i++) {
    for (let j = 0; j < this._indexes[i].length; j++) {
      if (Object.keys(this._indexes[i][j]) == index || util.isDeepStrictEqual(this._indexes[i][j], index)) {
