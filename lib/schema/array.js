  ...
  if (Array.isArray(value)) {
    ...
    if (this.caster) {
      try {
        ...
        var indices = [];
        var vl = this.options.arrayDimension || 1;
        for (var x=0; x < vl; x++) {
          indices.push(0);
        }

        // this is a kludge to deal with the multidimensional case, where this
        // is called before the arrays value has been initialized to anything
        if (vl > 1) {
          vl = value[0] == undefined ? 0 : vl;
        }
        ...
        while(vl > indices[0]) {
          value._setValueByIndex(indices, this.caster.cast(value._getValueByIndex(indices), doc, init));
          ...
          while(incLen <= indices[incIndex]) {
            ...
            // decrease the incIndex and increment it
            incIndex--;
            if (incIndex < 0) {
