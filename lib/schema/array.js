...
SchemaArray.prototype.cast = function(value, doc, init, prev, options) {
  ...
  if (Array.isArray(value)) {
    ...
    // Use options.path if provided and it's not just the subdocument's own path.
    // When options.path equals doc.$basePath, it means we're casting a direct field of the subdoc.
    let path = options.path && options.path !== doc?.$basePath ? options.path : this.path;
