...
SchemaArray.prototype._castForQuery = function(val, context) {
  ...
  if (val &&
      embeddedSchemaType?.discriminators &&
      typeof embeddedSchemaType?.schema?.options?.discriminatorKey === 'string') {
    if (embeddedSchemaType.discriminators[val[embeddedSchemaType.schema.options.discriminatorKey]]) {
      embeddedSchemaType = embeddedSchemaType.discriminators[val[embeddedSchemaType.schema.options.discriminatorKey]];
    } else {
      const constructorByValue = getDiscriminatorByValue(embeddedSchemaType.discriminators, val[embeddedSchemaType.schema.options.discriminatorKey]);
