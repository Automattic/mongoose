...
SchemaDocumentArray.prototype.doValidate = async function doValidate(array, scope, options) {
  ...
  for (let i = 0; i < array.length; ++i) {
    ...
    if (options?.validateModifiedOnly && !doc.$isNew && !doc.$isModified()) {
      continue;
    }

    const subdocOptions = doc.$isNew && options?.validateModifiedOnly ?
      { ...options, validateModifiedOnly: false } :
      options;
    promises.push(doc.validate(null, subdocOptions));
  }

  let subdocumentError;
  try {
    await Promise.all(promises);
  } catch (err) {
    subdocumentError = err;
  }

  if (schemaTypeError != null) {
    throw schemaTypeError;
  }
  if (subdocumentError != null) {
    throw subdocumentError;
  }
};
...
SchemaDocumentArray.prototype.doValidateSync = function(array, scope, options) {
  ...
  let resultError = schemaTypeError;
  ...
  for (let i = 0, len = count; i < len; ++i) {
    ...
    const subdocOptions = doc.$isNew && options?.validateModifiedOnly ?
      { ...options, validateModifiedOnly: false } :
      options;
    const subdocValidateError = doc.$__validateSync(subdocOptions);
