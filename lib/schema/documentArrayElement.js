...
SchemaDocumentArrayElement.prototype.doValidate = async function doValidate(value, scope, options) {
  ...
  // The parent document array validates each of its subdocuments, so when it
  // already did, this element's own validators are all that is left to run.
  if (options?.skipSubdocumentValidation) {
    if (options.skipSchemaValidators) {
      return;
    }
    return SchemaType.prototype.doValidate.call(this, value, scope, options);
  }
...
SchemaDocumentArrayElement.prototype.doValidateSync = function doValidateSync(value, scope, options) {
  ...
  if (options?.skipSubdocumentValidation) {
    ...
    return SchemaType.prototype.doValidateSync.call(this, value, scope, options);
  }
