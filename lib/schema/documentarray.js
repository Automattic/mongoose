  ...
  function cb(err) {
    ...
    for (let i = 0, len = count; i < len; ++i) {
      ...
      if (!(doc instanceof Subdocument)) {
        ...
        doc = array[i] = new Constructor(doc, array, undefined, i);
      }
...
DocumentArrayPath.prototype.getDefault = function(scope) {
  ...
  for (let i = 0; i < ret.length; ++i) {
    ...
    const _subdoc = new Constructor({}, ret, undefined, i);
    _subdoc.init(ret[i]);
    _subdoc.isNew = true;
  ...
  for (let i = 0; i < len; ++i) {
    ...
    if (value[i] instanceof Subdocument) {
      ...
      if (init) {
        ...
        subdoc = new Constructor(null, value, selected, i);
        ...
        if (prev && subdoc && utils.deepEqual(subdoc.toObject(_opts), value[i])) {
          ...
          try {
            subdoc = new Constructor(value[i], value, undefined, i);
            // if set() is hooked it will have no return value
            // see gh-746
            value[i] = subdoc;
          } catch (error) {
