...
class SchemaMap extends SchemaType {
  ...
  cast(val, doc, init, prev, options) {
    ...
    if (init) {
      ...
      // Use the map's calculated full path for passing to nested casts
      const mapFullPath = map.$__path;
      ...
      if (val instanceof global.Map) {
        for (const key of val.keys()) {
          ...
          if (_val == null) {
            ...
            _val = map.$__schemaType.cast(_val, doc, true, null, { ...options, path: mapFullPath + '.' + key });
          }
          map.$init(key, _val);
        }
      } else {
