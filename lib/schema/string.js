...
function SchemaString(key, options) {
  const defaultOptionsKeys = Object.keys(SchemaString.defaultOptions);
  for (const optionName of defaultOptionsKeys) {
    if (SchemaString.defaultOptions.hasOwnProperty(optionName) && !options.hasOwnProperty(optionName)) {
      options[optionName] = SchemaString.defaultOptions[optionName];
    }
  }
  this.enumValues = [];
  this.regExp = null;
...
SchemaString.setDefaultOption = function setDefaultOption(optionName,value) {
  SchemaString.defaultOptions[optionName] = value;
};
