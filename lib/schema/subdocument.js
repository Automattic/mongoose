...
SchemaSubdocument.prototype.cast = function(val, doc, init, priorVal, options) {
  ...
  if (init) {
    subdoc = new Constructor(void 0, selected, doc, false, { defaults: false, path: options?.path });
    delete subdoc.$__.defaults;
    // Don't pass `path` to $init - it's only for the subdocument itself, not its fields.
    // Subdocuments use relative paths, not absolute paths, for change tracking.
    // `options.path` contains the absolute path.
