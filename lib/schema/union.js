...
class Union extends SchemaType {
  ...
  applySetters(val, doc, init, prev, options) {
    // Setters on the union path itself run first, so they can change which entry
    // in the union the value resolves to.
    val = this._applySetters(val, doc, init, prev, options);
