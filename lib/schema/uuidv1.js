...
const UUID_FORMAT = /[0-9a-f]{8}-[0-9a-f]{4}-1[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}/i;
...
function SchemaUUIDv1(key, options) {
  SchemaType.call(this, key, options, 'UUIDv1');
...
SchemaUUIDv1._cast = function(value, doc, init) {
  if (value === null) {
    return value;
  }
  ...
  if (typeof value === 'string') {
    if (UUID_FORMAT.test(value)) {
      ...
      throw new CastError('uuidv1', value, this.path);
    }
  }
  ...
  throw new CastError('SchemaUUIDv1', value, this.path);
};
...
SchemaUUIDv1.set = SchemaType.set;
...
SchemaUUIDv1.cast = function cast(caster) {
...
SchemaUUIDv1._checkRequired = v => UUID_FORMAT.test(v);
...
SchemaUUIDv1.checkRequired = SchemaType.checkRequired;
...
SchemaUUIDv1.prototype.checkRequired = function checkRequired(value, doc) {
...
SchemaUUIDv1.prototype.cast = function(value, doc, init) {
  ...
  if (typeof this._castFunction === 'function') {
    ...
    castFn = SchemaUUIDv1.cast();
  }

  try {
    return castFn(value);
  } catch (error) {
    throw new CastError('SchemaUUIDv1', value, this.path, error, this);
  }
};
...
SchemaUUIDv1.prototype.$conditionalHandlers =
