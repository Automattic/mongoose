...
const CastError = error.CastError;
const ValidatorError = error.ValidatorError;
...
function SchemaType(path, options, instance) {
  ...
  for (let i in options) {
    if (this[i] && typeof this[i] === 'function') {
      ...
      const opts = Array.isArray(options[i])
          ? options[i]
          : [options[i]];
...
SchemaType.prototype.required = function(required, message) {
  ...
  const msg = message || errorMessages.general.required;
...
SchemaType.prototype.getDefault = function(scope, init) {
  const ret = typeof this.defaultValue === 'function'
...
SchemaType.prototype.applySetters = function(value, scope, init, priorVal, options) {
  let v = value, setters = this.setters, len = setters.length, caster = this.caster;
...
SchemaType.prototype.applyGetters = function(value, scope) {
  let v = value, getters = this.getters, len = getters.length;
...
SchemaType.prototype.doValidate = function(value, fn, scope) {
  let err = false, path = this.path, count = this.validators.length;
...
SchemaType.prototype.doValidateSync = function(value, scope) {
  let err = null, path = this.path, count = this.validators.length;
