...
/**
 * Adds validator(s) to this schematype.
 *
 * Validators must return `Boolean`. False is interpreted as validation failure.
 *
 * ####Examples:
 *
 *     function validator () { ... }
 ...
 *
 * Validation occurs `pre('save')` or whenever you manually execute [#document_Document-validate](document#validate).
 *
 * If validation fails during `pre('save')` and no callback was passed to receive the error:
 *
 *     doc.save()
 *
 * an `error` event will be emitted on your Models associated db [#connection_Connection](connection), passing the validation error object along.
 *
