...
function applyHooks(model, schema, options) {
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (!objToDecorate[pointCut]) {
      continue;
    }
    if (objToDecorate[pointCut].$isWrapped) {
      continue;
    }
    if (!objToDecorate[pointCut].$originalFunction) {
      objToDecorate[newName] = objToDecorate[pointCut];
    }
    objToDecorate[pointCut] = (function(_newName) {
      return function wrappedPointCut() {
        ...
        if (fn && this.constructor.$wrapCallback) {
        ...
        return utils.promiseOrCallback(fn, cb => {
          ...
          this[_newName].apply(this, args);
        });
      };
    })(newName);
    objToDecorate[pointCut].$originalFunction = newName;
    objToDecorate[pointCut].$isWrapped = true;
    ...
    for (j = 0; j < _len; ++j) {
      args = toWrapEl.pre[j];
      args[0] = newName;
      objToDecorate.$pre.apply(model.prototype, args);
    }
    ...
    for (j = 0; j < _len; ++j) {
      ...
      args[0] = newName;
      objToDecorate.$post.apply(model.prototype, args);
    }
  }
}
