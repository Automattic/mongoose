...
const documentArrayParent =
  require("../../../helpers/symbols").documentArrayParent;
...
const methods = {
  ...
  _cast(value, index) {
    ...
    if (
      ...
      if (
        typeof value[Constructor.schema.options.discriminatorKey] ===
          "string" &&
        Constructor.discriminators[
          value[Constructor.schema.options.discriminatorKey]
        ]
      ) {
        Constructor =
          Constructor.discriminators[
  ...
  /**
   * Pulls items from the array automatically.
   *
   * @param {Object} [args...]
   * @api public
   * @method pull
  ...
  create(obj) {
    ...
    if (
      ...
      if (
        ...
        Constructor.discriminators[
          obj[Constructor.schema.options.discriminatorKey]
        ]
      ) {
        Constructor =
          Constructor.discriminators[
...
function _updateParentPopulated(arr) {
  ...
  const populatedPaths = Object.keys(parent.$__.populated).filter((p) =>
    p.startsWith(arr[arrayPathSymbol] + ".")
  );
  ...
  for (const path of populatedPaths) {
    ...
    parent.$__.populated[path].value = arr.map((val) =>
      val.$populated(remnant)
    );
  }
}
