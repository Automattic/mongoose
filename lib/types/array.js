...
MongooseArray.prototype.$pushAll = function (array) {
  var values = array.map(this._cast, this);
...
MongooseArray.prototype.$pullAll = function (values) {
  if (values && values.length) {
    ...
    while (i--) {
      ...
      if (mem instanceof EmbeddedDocument) {
        if (values.some( function (v) { return v.equals(mem); } )) {
          [].splice.call(oldArr, i, 1);
        }
      } else if (~values.indexOf(mem)) [].splice.call(oldArr, i, 1);
    }
    if (values[0] instanceof EmbeddedDocument) {
      this._registerAtomic('$pullDocs', values.map( function (v) { return v._id; } ));
    } else {
