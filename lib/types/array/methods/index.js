...
const methods = {
  ...
  _registerAtomic(op, val, atomics) {
    ...
    if (atomics === undefined) {
      atomics = this[arrayAtomicsSymbol];
    }
    ...
    if (op === '$pullAll' || op === '$addToSet') {
      ...
      if (val != null && utils.hasUserDefinedProperty(val, '$each')) {
        ...
        if (atomics.$push == null || atomics.$push.$each == null) {
          atomics.$push = { $each: val };
  ...
  push() {
    ...
    _checkManualPopulation(this, values, schemaType);

    values = [].map.call(values, this._mapCast, this);
    let ret;
    const atomics = this[arrayAtomicsSymbol];
    methods._markModified.call(this);
    if (isOverwrite) {
      atomic.$each = values;
    ...
    methods._registerAtomic.call(this, '$push', atomic, atomics);
    return ret;
  },
...
function _checkManualPopulation(arr, docs, schemaType) {
  if (schemaType == null) {
    schemaType = arr[arraySchemaSymbol];
  }
  const ref = arr == null ?
    null :
    schemaType && schemaType.caster && schemaType.caster.options && schemaType.caster.options.ref || null;
  if (arr.length === 0 &&
      docs.length !== 0) {
