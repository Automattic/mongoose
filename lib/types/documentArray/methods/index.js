...
const methods = {
  ...
  id(id) {
    ...
    if (schemaType && schemaType.casterConstructor && schemaType.casterConstructor.schema) {
    ...
    let casted;
    try {
      if (isBsonType(castedId, 'ObjectId')) {
        casted = castedId.toString();
      } else if (idSchemaType == null) {
    ...
    for (const val of this) {
      ...
      } else if (casted != null && isBsonType(_id, 'ObjectId')) {
        if (casted == _id.toString()) {
          return val;
        }
      } else if (castedId == _id) {
        return val;
      }
    }
