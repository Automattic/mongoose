...
function documentArrayProxy(arr, values, path, doc) {
  let arrayAtomics = {};
  ...
  if (Array.isArray(values)) {
    if (values[arrayPathSymbol] === path &&
      ...
      arrayAtomics = Object.assign({}, values[arrayAtomicsSymbol]);
    }
    values.forEach(v => {
  ...
  const proxy = new Proxy(arr, {
    ...
    set: function(target, prop, value) {
      ...
      } else if (prop === arrayAtomicsSymbol) {
        arrayAtomics = value;
      } else {
        arr[prop] = value;
      }
 ...
 */
function set(target, i, val, skipModified) {
  if (skipModified) {
    target.__array[i] = val;
    return target;
  }
  const value = CoreDocumentArray.prototype._cast.call(target, val, i);
  target.__array[i] = value;
  CoreDocumentArray.prototype._markModified.call(target, i);
  return target;
}
...
function MongooseDocumentArray(values, path, doc) {
  ...
  return documentArrayProxy(arr, values, path, doc);
}
