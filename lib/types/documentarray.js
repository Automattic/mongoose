...
MongooseDocumentArray.prototype._cast = function (value) {
  ...
  while(value.length > indices[0]) {
    ...
    if (val instanceof this._schema.casterConstructor) {
      if (!(val.__parent && val.__parentArray)) {
        // value may have been created using array.create()
        val.__parent = this._parent;
        val.__parentArray = this;
      }
      value._setValueByIndex(indices, val);
    } else {
      ...
      val = new this._schema.casterConstructor(val, this);
      value._setValueByIndex(indices, val);
    }
...
MongooseDocumentArray.prototype.toObject = function (options) {
  ...
  var il = indices.length;
  while(rArr.length > indices[0]) {
    var val = rArr._getValueByIndex(indices);
    // this handles the case where there are undefined values in the
    // array, this should be the only case for which this does not
    // conform to the schema
    if (!val) {
    ...
    rArr._setValueByIndex(indices, objVal);
    // time to deal with incrementing the index array

    var incIndex = il - 1;
    indices[incIndex]++;
    var incLen = rArr._getValueByIndex(indices.slice(0,incIndex)).length;
    ...
    while(incLen <= indices[incIndex]) {
      ...
      // get the length of the dimension we just incremented
      incLen = rArr._getValueByIndex(indices.slice(0,incIndex)).length;
    }
    if (incIndex < 0) {
      // continue to break out after last index has been cast
      break;
...
MongooseDocumentArray.prototype.notify = function notify (event) {
  ...
  return function notify (val) {
    var dim = self._schema.options.arrayDimension || 1;
    ...
    var il = indices.length;
    while(self.length > indices[0]) {
      var val = self._getValueByIndex(indices);
      // this handles the case where there are undefined values in the
      // array, this should be the only case for which this does not
      // conform to the schema
      if (!val) {
      ...
      var incIndex = il - 1;
      indices[incIndex]++;
      var incLen = self._getValueByIndex(indices.slice(0,incIndex)).length;
      ...
      while(incLen <= indices[incIndex]) {
        ...
        // get the length of the dimension we just incremented
        incLen = self._getValueByIndex(indices.slice(0,incIndex)).length;
      }
      if (incIndex < 0) {
        // continue to break out after last index has been cast
        break;
