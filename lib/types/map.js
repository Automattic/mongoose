...
class MongooseMap extends Map {
  ...
  set(key, value) {
    ...
    if (parent != null && parent.$__ != null && !deepEqual(value, priorVal)) {
      ...
      // If parent is a subdocument, it will prepend its $basePath, so we need to pass
      // a path relative to the subdocument, not the full path from root
      let pathToMark = path;
      if (parent.$isSingleNested && parent.$basePath) {
        // Only use relative path if path actually starts with parent's basePath
        if (path.startsWith(parent.$basePath + '.')) {
          pathToMark = path.substring(parent.$basePath.length + 1);
        }
      }
