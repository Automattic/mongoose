...
Subdocument.prototype.markModified = function(path) {
  console.log('made it to sub', new Error().stack);
  Document.prototype.markModified.call(this, path);

  if (this.$__parent && this.$basePath) {
    global.__doc = this.$__parent;
    console.log('first if');
    console.log(this.$__parent.isDirectModified(this.$basePath));
    console.log(this.$basePath);
    console.log('active paths', this.$__parent.$__.activePaths);
    if (this.$__parent.isDirectModified(this.$basePath)) {
      return;
    }
    console.log('made it to 133');
