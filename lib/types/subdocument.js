...
Subdocument.prototype.markModified = function(path) {
  ...
  // If parent is also a subdocument, it will prepend its own $basePath,
  // so pass a path relative to the parent instead of the full path
  let pathToMark = fullPath;
  if (parent.$isSingleNested && parent.$basePath) {
    // Only use relative path if fullPath actually starts with parent's basePath
    if (fullPath.startsWith(parent.$basePath + '.')) {
      pathToMark = fullPath.substring(parent.$basePath.length + 1);
    }
  }
