...
exports.clone = function clone(obj, options, known) {
  ...
  if (Array.isArray(obj)) {
    return cloneArray(obj, options, known);
  }
  ...
  if (obj.constructor) {
    switch (exports.getFunctionName(obj.constructor)) {
      case 'Object':
        return cloneObject(obj, options, known);
      case 'Date':
...
function cloneObject(obj, options, known) {
  ...
  //   return obj._id
  // }
  if(known.visitedNodes.indexOf(obj) === -1){
    known.visitedNodes.push(obj)
    known.clonedCopy.push(ret)
    if (retainKeyOrder) {
      for (k in obj) {
        val = clone(obj[k], options, known);
    ...
    ret = known.clonedCopy[known.visitedNodes.indexOf(obj)]
  }
...
function cloneArray(arr, options, known) {
  var ret = [];
  if(known.visitedNodes.indexOf(arr) === -1){
    known.visitedNodes.push(arr)
    known.clonedCopy.push(ret)
    for (var i = 0, l = arr.length; i < l; i++) {
      ret.push(clone(arr[i], options, known));
    }
  } else {
    ret = known.clonedCopy[known.visitedNodes.indexOf(arr)]
  }
  return ret;
}
