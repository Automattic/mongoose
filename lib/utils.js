...
exports.clonePOJOsAndArrays = function clonePOJOsAndArrays(val, seen) {
  ...
  if (isPOJO(val)) {
    ...
    for (const key of Object.keys(val)) {
      val[key] = exports.clonePOJOsAndArrays(val[key], seen);
    }
    seen.delete(val);
    return val;
  }
  if (Array.isArray(val)) {
    ...
    for (let i = 0; i < val.length; ++i) {
      val[i] = exports.clonePOJOsAndArrays(val[i], seen);
    }
    seen.delete(val);
    return val;
  }
...
exports.merge = function merge(to, from, options, path, seen) {
  options = options || {};
  ...
  while (i < len) {
    ...
    if (to[key] == null) {
      to[key] = exports.clonePOJOsAndArrays(from[key], seen);
      ...
      if (seen && seen.has(from[key])) {
        throw new Error('Mongoose does not support circular references in query filters or updates.');
      }
      seen || (seen = new Set());
      seen.add(from[key]);

      merge(to[key], from[key], options, (path ? path + '.' + key : key), seen);

      seen.delete(from[key]);
