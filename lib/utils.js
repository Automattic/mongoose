...
exports.isCircular = function isCircular(obj) {
  const seenObjectsMap = new Map();

  return detect(obj, '');

  function detect(obj, basePath) {
    if (obj && typeof obj === 'object') {
      if (seenObjectsMap.get(obj)) {
        return { isCircular: true, path: basePath };
      }
      seenObjectsMap.set(obj, true);

      for (const key in obj) {
        if (Object.hasOwnProperty.call(obj, key)) {
          const currentPath = basePath ? `${basePath}.${key}` : key;
          const result = detect(obj[key], currentPath);
