...
describe("model: findOneAndUpdate:", function () {
  ...
  it("executes when a callback is passed", function (done) {
    ...
    M.where("name", "aaron")
      .findOneAndUpdate({ $set: { name: "Aaron" } })
      .findOneAndUpdate(cb);
  ...
  it("executes when a callback is passed to a succeeding function", function (done) {
    ...
    M.where()
      ...
      .exec(cb);
    M.where()
      .findOneAndUpdate({ name: "aaron" }, { $set: { name: "Aaron" } })
      .exec(cb);
    M.where()
      .findOneAndUpdate({ $set: { name: "Aaron" } })
      .exec(cb);
  ...
  it("updates numbers automatically", function (done) {
    let totalDocs = 4;
  ...
  it("honors strict schemas", async function () {
    ...
    const name = Date.now();
    const doc = await S.findOneAndUpdate(
      { name: name },
      { ignore: true },
      { upsert: true, new: true }
    );
    ...
    const doc2 = await S.findOneAndUpdate(
      { name: "orange crush" },
      { ignore: true },
      { upsert: true }
    );
    ...
    const doc3 = await S.findOneAndUpdate(
      { name: "orange crush" },
      { ignore: true }
    );
  ...
  it("returns errors with strict:throw schemas", async function () {
    ...
    const err = await S.findOneAndUpdate(
      { name: name },
      { ignore: true },
      { upsert: true }
    ).then(
    ...
    const err2 = await S.findOneAndUpdate(
      { _id: s._id },
      { ignore: true }
    ).then(
      () => null,
  ...
  it("executes when a callback is passed to a succeeding function", function (done) {
    ...
    M.findByIdAndUpdate(_id, { $set: { name: "Aaron" } }, { new: false }).exec(
      cb
    );
  ...
  it("supports $elemMatch with $in (gh-1091 gh-1100)", function (done) {
    ...
    B.create({ ids: [_id1, _id2] }, function (err, doc) {
      ...
      B.findByIdAndUpdate(doc._id, { title: "woot" }, { new: true })
  ...
  it("supports population (gh-1395)", function (done) {
    ...
    const N = db.model("Test2", {
      a: { type: Schema.ObjectId, ref: "Test1" },
      i: Number,
    });
  ...
  it("return hydrated document (gh-7734 gh-7735)", async function () {
    ...
    fruit = await Fruit.findOneAndUpdate(
      {},
      { $set: { name: "Banana" } },
      { new: true }
    );
      ...
      a2.save(function (error, a2) {
        ...
        User.findOneAndUpdate(
          ...
          function (error, doc) {
            ...
            assert.ok(
              isEqualWith(doc.contacts[0].account, a2._id, compareBuffers)
            );
  ...
  it("doesn't add __v on upsert if `$set` (gh-4505) (gh-5973)", function () {
    ...
    return Account.findOneAndUpdate({}, update, { upsert: true, new: true })
  ...
  it("doesn't add __v on upsert if `$set` with `update()` (gh-5973)", function () {
    ...
    const update = { $set: { name: "test", __v: 1 } };
    return Account.updateOne({}, update, { upsert: true, new: true })
      .then(() => Account.findOne({ name: "test" }))
      .then((doc) => assert.strictEqual(doc.__v, 1));
  });
    ...
    TestModel.create(
      {
        a: 1,
        b: 0,
        ticks: [{ name: "eggs" }, { name: "bacon" }, { name: "coffee" }],
      },
      function (error) {
        assert.ifError(error);
        TestModel.findOneAndUpdate(
          { a: 1 },
          { $pull: { ticks: { $or: [{ name: "eggs" }, { name: "bacon" }] } } },
          function (error) {
            assert.ifError(error);
  ...
  it("cast errors for empty objects as object ids (gh-2732)", function (done) {
    ...
    Breakfast.findOneAndUpdate({}, { base: {} }, {}).exec(function (error) {
      assert.ok(error);
      done();
    });
  });
  ...
  describe("middleware", function () {
    it("works", function (done) {
      ...
      Breakfast.findOneAndUpdate({}, { base: "eggs" }, {}, function (error) {
        assert.ifError(error);
    ...
    it("works with exec()", function (done) {
      ...
      Breakfast.findOneAndUpdate({}, { base: "eggs" }, {}).exec(function (
        error
      ) {
        assert.ifError(error);
        assert.equal(preCount, 1);
  ...
  describe("validators (gh-860)", function () {
      ...
      Breakfast.findOneAndUpdate(
        ...
        function (error, breakfast) {
          ...
          assert.equal(
            ...
            "Validator failed for path `topping` with value `bacon`"
          );
      ...
      Breakfast.findOneAndUpdate(
        ...
        function (error, breakfast) {
          ...
          assert.equal(
            ...
            "Validator failed for path `eggs` with value `softboiled`"
          );
    ...
    it("min/max, enum, and regex built-in validators work", function (done) {
      ...
      Breakfast.findOneAndUpdate(
        ...
        function (error) {
          ...
          assert.equal(
            error.errors.eggs.message,
            "Path `eggs` (3) is less than minimum allowed value (4)."
          );
          ...
          Breakfast.findOneAndUpdate(
            ...
            function (error) {
              ...
              assert.equal(
                error.errors.steak,
                "`tofu` is not a valid enum value for path `steak`."
              );
    ...
    it("validators ignore paths underneath mixed (gh-8659)", function () {
      ...
      const updateOptions = { runValidators: true, upsert: true, new: true };
      return Test.findOneAndUpdate({}, { "n.test": "foo" }, updateOptions).then(
  ...
  describe("bug fixes", function () {
    it("passes raw result if rawResult specified (gh-4925)", function (done) {
      ...
      TestModel.findOneAndUpdate({}, update, options).exec(function (
        error,
        res
      ) {
        assert.ifError(error);
    ...
    it("custom validator on mixed field (gh-4305)", function (done) {
      ...
      Board.findOneAndUpdate({}, update, opts).exec(function (error) {
        assert.ifError(error);
    ...
    it("avoids edge case with middleware cloning buffers (gh-5702)", function (done) {
      ...
      User.create(user, function (error, user) {
        ...
        User.findOneAndUpdate(q, upd, { new: true })
          .lean()
          .exec(function (error) {
            assert.ifError(error);
            done();
  ...
  it("updating embedded discriminator with discriminator key in update (gh-8378)", async function () {
    ...
    doc = await MyModel.findByIdAndUpdate(
      doc._id,
      {
  ...
  it("setDefaultsOnInsert with doubly nested subdocs (gh-8392)", function () {
    ...
    const opts = { upsert: true, new: true };
    return Model.findOneAndUpdate({}, { prop: "foo", L1: {} }, opts)
      .lean()
  ...
  it("supports overwriting nested map paths (gh-10485)", async function () {
    ...
    const res = await Parent.findOneAndUpdate(
      { _id: parent._id, "children.vals.github": { $exists: true } },
      { $set: { "children.$.vals": { telegram: "hello" } } },
      { new: true }
    );
    assert.deepEqual(
      res.toObject().children[0].vals,
      new Map([["telegram", "hello"]])
    );
  });
  ...
  it("supports $set on elements of map of subdocuments (gh-10720)", async function () {
    ...
    const res = await Parent.findOneAndUpdate({ _id: parent.id }, update, {
      new: true,
    });
