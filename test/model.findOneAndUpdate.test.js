...
describe('model: findByIdAndUpdate:', function(){
  ...
  describe('defaults and validators', function() {
    it('applies defaults on upsert', function(done) {
      ...
      Breakfast.findOneAndUpdate({}, { base: 'eggs' }, updateOptions, function(error, breakfast) {
        assert.ifError(error);
    ...
    it('doesnt set default on upsert if query sets it', function(done) {
      ...
      var updateOptions = { upsert: true, setDefaultsOnInsert: true };
      Breakfast.findOneAndUpdate({ topping: 'sausage' }, { base: 'eggs' }, updateOptions, function(error, breakfast) {
        assert.ifError(error);
        assert.equal('eggs', breakfast.base);
    ...
    it('properly sets default on upsert if query wont set it', function(done) {
      ...
      var updateOptions = { upsert: true, setDefaultsOnInsert: true };
      Breakfast.findOneAndUpdate({ topping: { $ne: 'sausage' } }, { base: 'eggs' }, updateOptions, function(error, breakfast) {
        assert.ifError(error);
        assert.equal('eggs', breakfast.base);
        assert.equal('bacon', breakfast.topping);
        db.close();
    ...
    it('runs validators if they\'re set', function(done) {
      var db = start();
      ...
      Breakfast.findOneAndUpdate({}, { topping: 'bacon', base: 'eggs' }, updateOptions, function(error) {
        assert.ok(!!error);
    ...
    it('validators handle $unset and $setOnInsert', function(done) {
      ...
      Breakfast.findOneAndUpdate({}, { $unset: { steak: '' }, $setOnInsert: { eggs: 'softboiled' } }, updateOptions, function(error) {
        assert.ok(!!error);
        assert.equal(2, Object.keys(error.errors).length);
        assert.equal('steak', Object.keys(error.errors)[0]);
        assert.equal('eggs', Object.keys(error.errors)[1]);
    ...
    it('min/max, enum, and regex built-in validators work', function(done) {
      ...
      var updateOptions = { runValidators: true };
      Breakfast.findOneAndUpdate({}, { $set: { steak: 'ribeye', eggs: 3, bacon: '3 strips' } }, updateOptions, function(error) {
        assert.ok(!!error);
        assert.equal(1, Object.keys(error.errors).length);
        ...
        Breakfast.findOneAndUpdate({}, { $set: { steak: 'tofu', eggs: 5, bacon: '3 strips' } }, updateOptions, function(error) {
          assert.ok(!!error);
          assert.equal(1, Object.keys(error.errors).length);
          assert.equal('steak', Object.keys(error.errors)[0]);
          ...
          Breakfast.findOneAndUpdate({}, { $set: { steak: 'sirloin', eggs: 6, bacon: 'none' } }, updateOptions, function(error) {
            assert.ok(!!error);
            assert.equal(1, Object.keys(error.errors).length);
    ...
    it('multiple validation errors', function(done) {
      ...
      var updateOptions = { runValidators: true };
      Breakfast.findOneAndUpdate({}, { $set: { steak: 'tofu', eggs: 2, bacon: '3 strips' } }, updateOptions, function(error) {
        assert.ok(!!error);
        assert.equal(2, Object.keys(error.errors).length);
    ...
    it('validators ignore $inc', function(done) {
      ...
      var updateOptions = { runValidators: true };
      Breakfast.findOneAndUpdate({}, { $inc: { eggs: 1 } }, updateOptions, function(error) {
        assert.ifError(error);
        db.close();
        done();
      });
