...
/* Helper for generating type test test/types/middleware.preposttypes.test.ts,
 * only active when "GEN_TYPE_TEST" is true
 */
const GEN_TYPE_TEST = false;
const typeTestText = [];
if (GEN_TYPE_TEST) {
  typeTestText.push('/* The following tests were generated by test/model.middleware.preposttypes.test.js (with GEN_TYPE_TEST hardcoded set to true) */');
}
function jsfy(obj) {
...
describe('pre/post hooks, type of this', function() {
  ...
  it('dynamic type of this in pre/post hooks', async function() {
    ...
    function registerHooks(expThisType /* QUERY, DOC, UNION, NEVER  */, method /* save, updateOne etc. */, options/* ? {document,query} */) {
      ...
      for (const hook of ['pre', 'post']) {
        // only for type test generation ----------------------
        if (GEN_TYPE_TEST) {
          const thisTypeStr = ['Query<any, any>', 'HydratedDocument<IDocument>', 'Query<any, any>|HydratedDocument<IDocument>', 'never'][expThisType];
          if (hook === 'pre') {
            typeTestText.push(`schema.${hook}(${jsfy(method).replace(/"/g, '')}${options ? ', ' + jsfy(options) : ''}, function() {
  ExpectType<${thisTypeStr}>(this);
});`);
          } else if (hook === 'post') {
            typeTestText.push(`schema.${hook}(${jsfy(method).replace(/"/g, '')}${options ? ', ' + jsfy(options) : ''}, function(res) {
  ExpectType<${thisTypeStr}>(this);
  ExpectNotType<Query<any, any>>(res);
    ...
    if (GEN_TYPE_TEST) {
      typeTestText.push('/* end of generated tests */');
      console.log(typeTestText.join('\n\n'));
    }
  });
});
