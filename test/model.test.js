...
describe("Model", function () {
    ...
    describe("array", function () {
      describe("defaults", function () {
        ...
        it("empty", function (done) {
          ...
          const DefaultZeroCardArray = db.model(
            "Test",
            DefaultZeroCardArraySchema
          );
  ...
  describe("validation", function () {
    it("works", function (done) {
      ...
      const TestValidation = db.model(
        "Test",
        new Schema({
          simple: { type: String, required: true },
          scope: {
            type: String,
            validate: [dovalidate, "scope failed"],
            required: true,
          },
          asyncScope: {
            type: String,
            validate: [dovalidateAsync, "async scope failed"],
            required: true,
          },
        })
      );
    ...
    it("custom messaging", function (done) {
      ...
      const TestValidationMessage = db.model(
        "Test",
        new Schema({
    ...
    it("with Model.schema.path introspection (gh-272)", function (done) {
      ...
      const IntrospectionValidation = db.model(
        "Test",
        IntrospectionValidationSchema
      );
      ...
      doc.save(function (err) {
        assert.equal(
          ...
          'Name cannot be greater than 1 character for path "name" with value `hi`'
        );
        assert.equal(err.name, "ValidationError");
        assert.ok(
          err.message.indexOf("Test validation failed") !== -1,
          err.message
        );
        done();
      });
    ...
    it("of required undefined values", function (done) {
      const TestUndefinedValidation = db.model(
        "Test",
        new Schema({
          simple: { type: String, required: true },
        })
    ...
    it("save callback should only execute once (gh-319)", function (done) {
      ...
      post.save(function (err) {
        ...
        assert.equal(
          ...
          "Validator failed for path `password` with value `short`"
        );
        assert.equal(
        ...
        assert.equal(
          ...
          "Validator failed for path `username` with value `nope`"
        );
        ...
        assert.equal(
          post.errors.password.message,
          "Validator failed for path `password` with value `short`"
        );
        assert.equal(
          post.errors.email.message,
          "Validator failed for path `email` with value `too`"
        );
        assert.equal(
          post.errors.username.message,
          "Validator failed for path `username` with value `nope`"
        );
        done();
      });
    ...
    it("nested", function (done) {
      const TestNestedValidation = db.model(
        "Test",
        new Schema({
          nested: {
    ...
    it("of nested subdocuments", function (done) {
      ...
      const TestSubdocumentsValidation = db.model(
        "Test",
        new Schema({
          items: [Subdocs],
        })
      ...
      post.save(function (err) {
        ...
        assert.ok(err instanceof ValidationError);
        assert.ok(
          err.errors["items.0.subs.0.required"] instanceof ValidatorError
        );
        assert.equal(
          err.errors["items.0.subs.0.required"].message,
          "Path `required` is required."
        );
        assert.ok(
          post.errors["items.0.subs.0.required"] instanceof ValidatorError
        );
        assert.equal(
          post.errors["items.0.subs.0.required"].message,
          "Path `required` is required."
        );
        ...
        post.save(function (err) {
          ...
          assert.equal(
            err.errors["items.0.required"].message,
            "Path `required` is required."
          );
    ...
    it("without saving", function (done) {
      const TestCallingValidation = db.model(
        "Test",
        new Schema({
    ...
    it("when required is set to false", function (done) {
      ...
      const TestV = db.model(
        ...
        new Schema({
          result: {
            type: String,
            validate: [validator, "chump validator"],
            required: false,
          },
        })
      );
    ...
    describe("middleware", function () {
      ...
      it("async", function (done) {
        ...
        AsyncValidationMiddlewareSchema.pre(
          "validate",
          true,
          function (next, done) {
            const _this = this;
      ...
      it("complex", function (done) {
        ...
        ComplexValidationMiddlewareSchema.pre(
          "validate",
          true,
          function (next, done) {
            const _this = this;
  ...
  describe("defaults application", function () {
    ...
    it("subdocument", function (done) {
      ...
      const TestSubdocumentsDefaults = db.model(
        "Test",
        new Schema({
          items: [Items],
        })
    ...
    it("allows nulls", function (done) {
      const T = db.model(
        "Test",
        new Schema({ name: { type: String, default: null } })
      );
      const t = new T();
      ...
      it("array of Mixed on existing doc can be pushed to", function (done) {
        ...
        doodad.save(function (err) {
          ...
          DooDad.findById(doodad._id, function (err, doodad) {
            ...
            assert.deepEqual(doodad.nested.arrays.toObject(), [
              ["+10", "yup", date],
            ]);
      ...
      it('props can be set directly when property was named "type"', function (done) {
        ...
        doodad.save(function (err) {
          ...
          DooDad.findById(doodad._id, function (err, doodad) {
            ...
            assert.deepEqual(doodad.nested.array.toObject(), [
              { x: 1 },
              { x: 2 },
              { x: 3 },
            ]);
  ...
  it("changing a number non-automatically (gh-203)", function (done) {
    const post = new BlogPost();
  ...
  describe("saving embedded arrays", function () {
    it("of Numbers automatically", function (done) {
    ...
    it("of Strings automatically", function (done) {
    ...
    it("of Buffers automatically", function (done) {
  ...
  it("updating an embedded array document to an Object value (gh-334)", function (done) {
    ...
    instance.set({
      name: "name-value",
      arrData: [{ name: "arrName1", subObj: { subName: "subName1" } }],
    });
    instance.save(function (err) {
      assert.ifError(err);
  ...
  it("removing a subdocument automatically", function (done) {
    const post = new BlogPost();
    post.title = "hahaha";
    post.comments.push({ title: "woot" });
    post.comments.push({ title: "aaaa" });
  ...
  describe("#exec()", function () {
    it.skip("count()", function (done) {
      BlogPost.create(
        { title: "interoperable count as promise" },
        function (err) {
          assert.ifError(err);
          const query = BlogPost.count({
            title: "interoperable count as promise",
          });
        ...
        return BlogPost.create({ title: "interoperable findOne as promise 2" })
          .then((doc) => {
            ...
            return BlogPost.findOne({
              title: "interoperable findOne as promise 2",
            }).exec();
          })
          .then((found) => {
            assert.equal(found.id, created.id);
          ...
          function (err, d1, d2, d3) {
            ...
            P.create(
              ...
              function (err) {
                ...
                const promise = BlogPost.find({ title: /^then promise/ })
                  .select("_id")
                  .exec();
                promise
                  .then(function (blogs) {
  ...
  describe("backward compatibility", function () {
    it("with conflicted data in db", function (done) {
      const M = db.model(
        "Test",
        new Schema({ namey: { first: String, last: String } })
      );
  ...
  describe("setting an unset value", function () {
    it("is saved (gh-742)", function (done) {
      const DefaultTestObject = db.model(
        "Test",
        new Schema({
  ...
  describe("unsetting a default value", function () {
    it("should be ignored (gh-758)", function (done) {
      const M = db.model(
        "Test",
        new Schema({ s: String, n: Number, a: Array })
      );
  ...
  describe("Skip setting default value for Geospatial-indexed fields (gh-1668)", function () {
    ...
    it("Doc with 2dsphere indexed field without initial value can be updated", async function () {
      ...
      const personDoc = await Person.findByIdAndUpdate(p._id, updates, {
        new: true,
      });
  ...
  it("save max bson size error with buffering (gh-3906)", async function () {
    ...
    const error = await test.save().then(
      () => null,
      (err) => err
    );
  ...
  describe("insertMany()", function () {
    ...
    it("insertMany() with nested timestamps (gh-12060)", async function () {
      ...
      await Test.insertMany([{ child: { name: "test2" } }], {
        timestamps: false,
      });
  ...
  describe("bug fixes", function () {
    ...
    it("insertMany() ordered option for constraint errors (gh-3893)", async function () {
      ...
      const error = await Movie.insertMany(arr, { ordered: false }).then(
        () => null,
        (err) => err
      );
    ...
    it("insertMany() ordered option for single validation error", async function () {
      ...
      const arr = [{ foo: "Star Wars" }, { foo: "The Fast and the Furious" }];
      await Movie.insertMany(arr, { ordered: false });
    ...
    it("insertMany() sets `isNew` for inserted documents with `ordered = false` (gh-9677)", async function () {
      ...
      await Movie.init();
      const err = await Movie.insertMany(arr, opts).then(
        () => err,
        (err) => err
      );
      assert.ok(err);
    ...
    it("insertMany() with error handlers (gh-6228)", async function () {
      ...
      let threw = false;
      try {
        await Movie.insertMany([{ name: "Star Wars" }, { name: "Star Wars" }]);
      } catch (error) {
        assert.ok(error);
        threw = true;
      }
    ...
    describe("3.6 features", function () {
      ...
      describe("watch()", function () {
        ...
        it("watch() (gh-5964)", async function () {
          ...
          assert.equal(
            changeData.fullDocument._id.toHexString(),
            doc._id.toHexString()
          );
        });
      ...
      describe("sessions (gh-6362)", function () {
        ...
        beforeEach(async function () {
          ...
          const mongo36 =
            version[0] > 3 || (version[0] === 3 && version[1] >= 6);
          if (!mongo36) {
            this.skip();
          }
        });
        ...
        it("startSession() before connecting", async function () {
          ...
          const sessionPromise = MyModel.startSession({
            causalConsistency: true,
          });
    ...
    it("method with same name as prop should throw (gh-4475)", function (done) {
      ...
      try {
        ...
        assert.equal(
          error.message,
          "You have a method and a property in " +
    ...
    describe("bulkWrite casting", function () {
      ...
      it("throws an error if no update object is provided (gh-8331)", async function () {
        ...
        assert.equal(
          ...
          "Hafez",
          "Document data is not wiped if no update object is provided."
        );
      });
      ...
      it("casts according to child discriminator if `discriminatorKey` is present (gh-8982)", async function () {
        ...
        await Person.bulkWrite([
          {
            updateOne: {
              filter: { __t: "Worker", age: "5" },
              update: { age: "6" },
            },
          },
          {
            updateMany: {
              filter: { __t: "Worker", age: "10" },
              update: { age: "11" },
            },
          },
          {
            replaceOne: {
              filter: { __t: "Worker", age: "15" },
              replacement: { name: "Hafez3", age: "16" },
            },
          },
          ...
          {
            insertOne: {
              document: { __t: "Worker", name: "Hafez6", age: "30" },
            },
          },
        ]);
        ...
        return Test.bulkWrite([
          {
            updateOne: {
              filter: {},
              update: {
                $set: { "someArray.1": { __proto__: null, message: "test" } },
              },
            },
          },
        ]);
    ...
    it("remove with cast error (gh-5323)", function (done) {
      ...
      const Model = db.model("Test", schema);
      const arr = [{ name: "test-1" }, { name: "test-2" }];
      ...
      Model.create(arr, function (error) {
        ...
        Model.remove([], function (error) {
          assert.ok(error);
          assert.ok(
            error.message.indexOf("must be an object") !== -1,
            error.message
          );
    ...
    describe("Model.syncIndexes()", () => {
      ...
      it("when two different models connect to the same collection, syncIndexes(...) respects the last call", async () => {
        ...
        // Assert
        assert.deepStrictEqual(
          userIndexesBeforeSync.map((index) => index.name),
          ["_id_"]
        );

        assert.deepStrictEqual(
          orderIndexesBeforeSync.map((index) => index.name),
          ["_id_", "name_1"]
        );
      ...
      it("when two models have the same collection name, same field but different options, syncIndexes(...) respects the last call", async () => {
        ...
        // Assert
        const userCustomIdIndex = userIndexes.find(
          (index) => index.key.customId === 1
        );
        assert.strictEqual(userCustomIdIndex.unique, undefined);

        const orderCustomIdIndex = orderIndexes.find(
          (index) => index.key.customId === 1
        );
      ...
      it("when called with different key order, it treats different order as different indexes (gh-8135)", async () => {
        ...
        const compoundIndex = indexesAfterSync.find(
          (index) => index.key.name === 1 && index.key.age === -1
        );
      ...
      it("adding discriminators should not drop the parent model's indexes", async () => {
        ...
        // Assert
        const actorIdIndex = indexes.find(
          (index) => index.name === "actorId_1"
        );
      ...
      it("syncing model with multiple discriminators works", async () => {
        ...
        assert.deepStrictEqual(
          eventIndexes.map((index) =>
            pick(index, ["key", "unique", "partialFilterExpression"])
          ),
          [
            { key: { _id: 1 } },
            {
      ...
      it("syncing main model does not sync discrimator indexes", async () => {
        ...
        const droppedByClickEvent = await ClickEvent.syncIndexes();
        const eventIndexesAfterSyncingClickEvents =
          await ClickEvent.listIndexes();
        ...
        assert.deepStrictEqual(
          eventIndexesBeforeSyncingClickEvents.map((index) =>
            pick(index, ["key", "unique", "partialFilterExpression"])
          ),
          [
            { key: { _id: 1 } },
        ...
        assert.deepStrictEqual(
          eventIndexesAfterSyncingClickEvents.map((index) =>
            pick(index, ["key", "unique", "partialFilterExpression"])
          ),
          [
            { key: { _id: 1 } },
      ...
      it("syncing discriminator does not attempt to sync parent model's indexes", async () => {
        ...
        assert.deepStrictEqual(
          eventIndexesAfterSyncingEverything.map((index) =>
            pick(index, ["key", "unique", "partialFilterExpression"])
          ),
          [
            { key: { _id: 1 } },
    ...
    it("createCollection() enforces expires (gh-11229)", async function () {
      ...
      const collectionInfo = await Test.db.db
        .listCollections()
        .toArray()
        .then((collections) =>
          collections.find((coll) => coll.name === "TestGH11229Var2")
        );
  ...
  it("Model.events() (gh-7125)", async function () {
    ...
    await Model.aggregate([{ $group: { fail: true } }])
      .exec()
      .catch(() => {});
    assert.equal(called.length, 1);
  ...
  it("set $session() before pre validate hooks run on bulkWrite and insertMany (gh-7769)", async function () {
    ...
    await SampleModel.insertMany([{ name: "foo" }, { name: "bar" }], {
      session,
    });
    assert.strictEqual(sessions[0], session);
  ...
  describe("exists() (gh-6872) (gh-8097) (gh-11138)", function () {
    ...
    it("returns `null` if no doc exists", async function () {
      ...
      const existingDocumentWithStrict = await Model.exists(
        { otherProp: "foo" },
        { strict: false }
      );
  ...
  it("Model.validate() (gh-7587)", async function () {
    ...
    err = await Model.validate({ age: null }, ["age"]).then(
      () => null,
      (err) => err
    );
    assert.ok(err);
    ...
    err = await Model.validate({ name: {} }, ["name"]).then(
      () => null,
      (err) => err
    );
    assert.ok(err);
    ...
    obj = { name: { first: "foo" } };
    err = await Model.validate(obj, ["name"]).then(
      () => null,
      (err) => err
    );
    assert.ok(err);
    ...
    err = await Model.validate(obj, ["comments"]).then(
      () => null,
      (err) => err
    );
    assert.ok(err);
  ...
  it("Model.validate(...) validates paths in arrays (gh-8821)", async function () {
    ...
    const err = await User.validate({ friends: [null, "A"] }).catch(
      (err) => err
    );
  ...
  it("Model.validate(...) uses object as context by default (gh-10346)", async () => {
    ...
    const err1 = await User.validate({ name: "John" }).then(
      () => null,
      (err) => err
    );
    assert.ok(err1);

    const err2 = await User.validate({ name: "Sam" }).then(
      () => null,
      (err) => err
    );
    assert.ok(err2 === null);
  ...
  it("bulkWrite can overwrite schema `strict` option for filters and updates (gh-8778)", async function () {
    ...
    const users = [{ notInSchema: 1 }, { notInSchema: 2 }, { notInSchema: 3 }];
    ...
    await User.bulkWrite(
      [
        {
          updateOne: {
            filter: { notInSchema: 1 },
            update: { notInSchema: "first" },
          },
        },
        {
          updateMany: {
            filter: { notInSchema: 2 },
            update: { notInSchema: "second" },
          },
        },
        {
          replaceOne: {
            filter: { notInSchema: 3 },
            replacement: { notInSchema: "third" },
          },
        },
      ],
      { strict: false }
  ...
  it("cast errors have `kind` field (gh-8953)", async function () {
    ...
    const err = await User.findOne({ _id: "invalid" }).then(
      () => null,
      (err) => err
    );
  ...
  describe("returnOriginal (gh-9183)", function () {
    ...
    it("Setting `returnOriginal` works", async function () {
      ...
      const user2 = await User.findByIdAndUpdate(createdUser._id, {
        name: "Hafez2",
      });
      assert.equal(user2.name, "Hafez2");

      const user3 = await User.findOneAndReplace(
        { _id: createdUser._id },
        { name: "Hafez3" }
      );
  ...
  describe("buildBulkWriteOperations() (gh-9673)", () => {
      ...
      const desiredWriteOperations = [
        ...
        { insertOne: { document: users[1] } },
        {
          updateOne: {
            ...
            update: { $set: { name: "I am the updated third name" } },
          },
        },
      ];

      assert.deepEqual(writeOperations, desiredWriteOperations);
    });
    ...
    it("accepts `timestamps: false` (gh-12059)", async () => {
      ...
      // Act
      const writeOperations = User.buildBulkWriteOperations(
        [newUser, userToUpdate],
        { timestamps: false, skipValidation: true }
      );

      // Assert
      const timestampsOptions = writeOperations.map(
        (writeOperationContainer) => {
          const operationObject =
            writeOperationContainer.updateOne ||
            writeOperationContainer.insertOne;
    ...
    it("accepts `timestamps: true` (gh-12059)", async () => {
      ...
      const writeOperations = User.buildBulkWriteOperations(
        [newUser, userToUpdate],
        { timestamps: true, skipValidation: true }
      );
  ...
  describe("bulkSave() (gh-9673)", function () {
    it("saves new documents", async function () {
      ...
      assert.deepEqual(
        ...
        ["Hafez1_gh-9673-1", "Hafez2_gh-9673-1"]
      );
    });
    ...
    it("throws an error on failure", async () => {
      ...
      const err = await User.bulkSave(users).then(
        () => null,
        (err) => err
      );
      assert.ok(err);
    ...
    it("sets `isNew` to false when a document succeeds and `isNew` does not change when some fail", async () => {
      ...
      const err = await User.bulkSave([user1, user2]).then(
        () => null,
        (err) => err
      );
    ...
    it("works if some document is not modified (gh-10437)", async () => {
      ...
      const err = await User.bulkSave([user]).then(
        () => null,
        (err) => err
      );
      assert.ok(err == null);
    ...
    it("accepts `timestamps: false` (gh-12059)", async () => {
      ...
      const userToUpdate = await User.create({
        name: "Hafez",
        createdAt: new Date("1994-12-04"),
        updatedAt: new Date("1994-12-04"),
      });
      userToUpdate.name = "John Doe";
  ...
  it("saves all error object properties to paths with type `Mixed` (gh-10126)", async () => {
    ...
    function assertErrorProperties(user) {
      ...
      assert.deepEqual(user.err.metadata, {
        reasons: ["Cloudflare is down", "DNS"],
      });
    }
  });
  ...
  it("respects `hydrate()` projection (gh-11375)", function () {
    const PieSchema = Schema({
      filling: String,
      hoursToMake: Number,
      tasteRating: Number,
    });
    const Test = db.model("Test", PieSchema);
    const doc = Test.hydrate(
      { filling: "cherry", hoursToMake: 2 },
      { filling: 1 }
    );
  ...
  it("sets index collation based on schema collation (gh-7621)", async function () {
    let testSchema = new Schema({ name: { type: String, index: true } });
