...
describe("model: update:", function () {
          ...
          BlogPost.update({ _id: post._id }, update2, function (err) {
            ...
            BlogPost.findById(post, function (err) {
              ...
              BlogPost.update({ _id: post._id }, update3, function (err) {
                ...
                assert.ok(
                  /Invalid atomic update value for \$pull\. Expected an object, received string/.test(
                    err.message
                  )
                );
  ...
  it("$addToSet with $ (gh-479)", function (done) {
    ...
    BlogPost.update(
      { _id: post._id, "comments.body": "been there" },
      update,
      function (err) {
        assert.ifError(err);
        BlogPost.findById(post, function (err, ret) {
          ...
          assert.strictEqual(
            ret.comments[0].comments[0].body,
            "The Ring Of Power"
          );
          done();
        });
      ...
      BlogPost.update(
        ...
        function (err) {
          ...
          BlogPost.findById(post, function (err, ret) {
            ...
            assert.equal(
              ret.date.toString(),
              new Date(update.$set.date).toString()
            );
  ...
  it("works with nested positional notation", function (done) {
    ...
    BlogPost.update({ _id: post._id }, update, function (err) {
      ...
      BlogPost.findById(post, function (err, ret) {
        ...
        assert.equal(
          ret.comments[0].comments[0].date.toString(),
          new Date("11/5/2011").toString()
        );
    ...
    Project.create({ name: "my project" }, function (err, project) {
      ...
      Project.update(
        ...
        function (err) {
          ...
          Project.update(
            { _id: pid, "components._id": comp._id },
            { $push: { "components.$.tasks": task } },
            function (err) {
              assert.ifError(err);
  ...
  describe("mongodb 2.4 features", function () {
    ...
    before(async function () {
      ...
      mongo24_or_greater =
        version[0] > 2 || (version[0] === 2 && version[1] >= 4);
    });
  ...
  describe("mongodb 2.6 features", function () {
    ...
    before(async function () {
      ...
      mongo26_or_greater =
        version[0] > 2 || (version[0] === 2 && version[1] >= 6);
    });
  ...
  describe("defaults and validators (gh-860)", function () {
    ...
    it("global validators option (gh-6578)", async function () {
      ...
      const updateOptions = { runValidators: true };
      const error = await Breakfast.update(
        {},
        { $unset: { steak: 1 } },
        updateOptions
      ).catch((err) => err);
    ...
    it("required and single nested (gh-4479)", function (done) {
      ...
      Company.update({}, update, options, function (error) {
        assert.ok(error);
        assert.equal(
          error.errors["file.name"].message,
  ...
  it("handles empty update with promises (gh-2796)", function (done) {
    ...
    M.create({}, function (error, doc) {
      assert.ifError(error);
      M.updateOne({ _id: doc._id }, { notInSchema: 1 })
        .exec()
        .then(function (data) {
  ...
  describe("middleware", function () {
    it("can specify pre and post hooks", function (done) {
      ...
      const gnr = new Band({
        members: ["Axl", "Slash", "Izzy", "Duff", "Adler"],
      });
  ...
  describe("bug fixes", function () {
    ...
    it("replaceOne", function (done) {
      ...
      Model.create({ name: "abc", age: 1 }, function (error, m) {
        assert.ifError(error);
        Model.replaceOne({ name: "abc" }, { name: "test" }).exec(function (
          err
        ) {
          assert.ifError(err);
    ...
    it("mixed nested type casting (gh-3337)", function (done) {
      ...
      Model.create({}, function (error, m) {
        ...
        Model.updateOne({ _id: m._id }, update).exec(function (error, res) {
          assert.ifError(error);
          assert.equal(res.modifiedCount, 1);
    ...
    it("$push with buffer doesnt throw error (gh-3890)", function (done) {
      ...
      const propValue = Buffer.from(
        "aa267824dc1796f265ab47870e279780",
        "base64"
      );
    ...
    it("updates child schema timestamps with $push (gh-4049)", function (done) {
      ...
      b2.save(function (err, doc) {
        ...
        const opts = { new: true };
        Parent.findOneAndUpdate(query, update, opts).exec(function (
          error,
          res
        ) {
          assert.ifError(error);
    ...
    it("update with buffer and exec (gh-4609)", function (done) {
      ...
      m.save(function (error, m) {
        assert.ifError(error);
        m.update({ $push: { arr: { ip: Buffer.alloc(1) } } }).exec(function (
          error
        ) {
          assert.ifError(error);
          done();
    ...
    it("single nested schema with geo (gh-4465)", function (done) {
      ...
      Container.update({}, { address: { geo: [-120.24, 39.21] } }).exec(
        function (error) {
          assert.ifError(error);
          done();
        }
    ...
    it("findOneAndUpdate with timestamps (gh-5045)", function (done) {
      ...
      User.findOneAndUpdate(
        ...
        function (error) {
          assert.ifError(error);
          User.update({ username: "test" }, { createdAt: new Date() }).exec(
            function (error) {
              assert.ifError(error);
              done();
            }
    ...
    it("doesn't skip casting the query on nested arrays (gh-7098)", async function () {
      ...
      assert.deepStrictEqual(inserted.xyz, [
        [0, 1],
        [2, 3],
        [4, 5],
      ]);
      assert.deepStrictEqual(inserted.nested.xyz, [
        [0, 1],
        [2, 3],
        [4, 5],
      ]);
      ...
      assert.deepStrictEqual(updated.xyz, [
        [0, 1],
        [200, 3],
        [4, 5],
      ]);
      assert.deepStrictEqual(updated.nested.xyz, [
        [0, 1],
        [200, 3],
        [4, 5],
      ]);
    ...
    it("update validators with nested required (gh-5269)", function (done) {
      ...
      Parent.update(
        ...
        function (error) {
          ...
          assert.ok(
            error.errors["d.d1"].message.indexOf("Path `d1` is required") !==
              -1,
    ...
    it("cast error in update conditions (gh-5477)", async function () {
      ...
      let error = await Model.update(q, u, o).then(
        () => null,
        (err) => err
      );
      assert.ok(error);
      ...
      error = await Model.updateMany(q, u, o).then(
        () => null,
        (err) => err
      );
      assert.ok(error);
    ...
    it("single embedded schema under document array (gh-4519)", function (done) {
      ...
      Group.update({}, update, opts, function (error) {
        assert.ok(error);
        assert.ok(
          error.errors["users.0.permission.read"],
...
describe("model: updateOne: ", function () {
  ...
  it("overwrite an array with empty (gh-7135)", async function () {
    ...
    let err = await TestModel.updateOne(
      {},
      { $set: { anArray: [{}] } },
      { runValidators: true }
    ).then(
  ...
  it("moves $set of immutable properties to $setOnInsert (gh-8467) (gh-9537)", async function () {
    ...
    await Model.updateOne(
      {},
      { name: "John", age: 20, child: { name: "test" } },
      _opts
    );
    ...
    await Model.updateOne(
      {},
      { name: "new", age: 29, child: { name: "new" } },
      _opts
    );
    assert.equal(doc.age, 20);
    assert.equal(doc.name, "John");
  ...
  describe("mongodb 42 features", function () {
    ...
    it("update pipeline timestamps (gh-8524)", async function () {
      const Cat = db.model(
        "Test",
        Schema({ name: String }, { timestamps: true })
      );
  ...
  it("update validators respect storeSubdocValidationError (gh-9172)", async function () {
    ...
    const err = await Model.updateOne(
      {},
      { nested: { arr: [{}] } },
      { runValidators: true }
    ).catch((err) => err);
  ...
  describe("converts dot separated paths to nested structure (gh-10200)", () => {
    ...
    function assertDocumentStructure(payment) {
      ...
      assert.equal(
        payment.externalServiceResponse.resultDetails.clearingInstituteName,
        "Our local bank"
      );
      assert.deepEqual(
