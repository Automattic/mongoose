...
describe("types array", function () {
    ...
    it("works", function (done) {
      ...
      p.save(function (err) {
        ...
        Painting.findById(p, function (err, doc) {
          ...
          assert.equal(doc.colors.length, 2);
          assert.ok(
            !("$set" in doc.colors.$atomics()),
            "invalid $atomic op used"
          );
          doc.save(function (err) {
            assert.equal(err, null);
  ...
  describe("options", function () {
    ...
    beforeEach(function () {
      ...
      docArrOptions = Object.assign(
        {},
        mongoose.Schema.Types.DocumentArray.options
      );
    ...
    it("castNonArrays (gh-7371) (gh-7479)", function () {
      ...
      const bothSchema = new Schema({
        arr: { castNonArrays: true, type: [String] },
        docArr: { castNonArrays: true, type: [{ name: String }] },
      });
  ...
  describe("slice", function () {
    ...
    it("with push (gh-8655)", function () {
      ...
      assert.ok(
        slicedNames.indexOf("test2") !== -1 &&
          slicedNames.indexOf("test4") !== -1
      );
    });
  });
  ...
  describe("setting a doc array", function () {
    it("should adjust path positions", function (done) {
      ...
      const d = new D({
        em1: [{ name: "pos0" }, { name: "pos1" }, { name: "pos2" }],
      });
  ...
  describe("removing from an array automatically using MongooseArray#remove", function () {
    let B;
  ...
  it('test "castNonArrays" property option', function () {
    const Model = db.model(
      "Test",
      new Schema({
        x1: { castNonArrays: false, type: [String] },
        x2: { castNonArrays: true, type: [String] },
        x3: { type: [String] },
      })
    );
  ...
  it("`castNonArrays` on specific paths takes precedence over global option", function () {
    ...
    const userSchema = new Schema({
      friendsNames: { type: [String], castNonArrays: true },
    });
